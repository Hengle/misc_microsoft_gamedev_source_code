...
BAkDefaultLowLevelIO::BAkDefaultLowLevelIO( )
{
}
...
AKRESULT BAkDefaultLowLevelIO::Init( )
{
	m_szBasePath[0] = NULL;
    m_szBankPath[0] = NULL;
...
void BAkDefaultLowLevelIO::Term( )
{
}
...
//-----------------------------------------------------------------------------
AKRESULT BAkDefaultLowLevelIO::Open( 
    ...
    )
{
	...
	ULARGE_INTEGER Temp;
	Temp.LowPart = gXFS.getFileSize( out_fileDesc.hFile,(LPDWORD)&Temp.HighPart );
...
//-----------------------------------------------------------------------------
AKRESULT BAkDefaultLowLevelIO::Close(
    const AkFileDesc & in_fileDesc      ///< File descriptor.
    )
{
   gXFS.setOwnerThread(in_fileDesc.hFile, GetCurrentThreadId());
   if ( !gXFS.closeHandle( in_fileDesc.hFile ) )
    {
...
//-----------------------------------------------------------------------------
AKRESULT BAkDefaultLowLevelIO::Read(
    ...
    )
{
    ...
    if ( !io_transferInfo.pOverlapped->hEvent )
    {
        ...
        {
            gXFS.setOwnerThread(io_fileDesc.hFile, GetCurrentThreadId());
            ...
            if ( !gXFS.setFilePointer( io_fileDesc.hFile, 
                                      io_transferInfo.pOverlapped->Offset,
                                      NULL,
                                      FILE_BEGIN ) )
            {
                return AK_Fail;
        ...
        bOk = gXFS.readFile( io_fileDesc.hFile,
                          out_pBuffer,
...
//     - AK_Fail:        an error occured.
//-----------------------------------------------------------------------------
AKRESULT BAkDefaultLowLevelIO::Write(
    AkFileDesc &    io_fileDesc,        ///< File descriptor.
   ...
   /* BOOL bOk;
    if ( !io_transferInfo.pOverlapped->hEvent )
    {
        ...
        if ( !io_transferInfo.bIsSequential )
        {
            ...
            iOffset.LowPart = io_transferInfo.pOverlapped->Offset;
            iOffset.HighPart = io_transferInfo.pOverlapped->OffsetHigh;
            if ( !gXFS.setFilePointerEx( io_fileDesc.hFile, 
                                      iOffset,
                                      NULL,
                                      FILE_BEGIN ) )
            {
...
//     - AK_Fail:          an error occured.
//-----------------------------------------------------------------------------
AKRESULT BAkDefaultLowLevelIO::GetAsyncResult(
    AkFileDesc &    io_fileDesc,        ///< File descriptor.
    AkIOTransferInfo & io_transferInfo  ///< Platform-specific IO operation info. 
    )
{
    ...
    BOOL bOk;
    gXFS.setOwnerThread(io_fileDesc.hFile, GetCurrentThreadId());
    bOk = gXFS.getOverlappedResult( io_fileDesc.hFile, 
...
//-----------------------------------------------------------------------------
AkUInt32 BAkDefaultLowLevelIO::GetBlockSize(
    const AkFileDesc &  in_fileDesc     ///< File descriptor.
    )
{
    return m_uBlockSize;
...
//-----------------------------------------------------------------------------
#ifndef AK_OPTIMIZED
AKRESULT BAkDefaultLowLevelIO::GetDeviceDesc(
...
AKRESULT BAkDefaultLowLevelIO::OpenFile( 
    AkLpCtstr       in_pszFilename,     // File name.
    AkOpenMode      in_eOpenMode,       // Open mode.
...
// String overload.
AKRESULT BAkDefaultLowLevelIO::GetFullFilePath(
    AkLpCtstr           in_pszFileName, // File name.
    AkFileSystemFlags * in_pFlags,      // Special flags. Can pass NULL.
...
AKRESULT BAkDefaultLowLevelIO::SetDevice(
...
AKRESULT BAkDefaultLowLevelIO::SetBasePath(
...
AKRESULT BAkDefaultLowLevelIO::SetBankPath(
...
AKRESULT BAkDefaultLowLevelIO::SetAudioSrcPath(
...
AKRESULT BAkDefaultLowLevelIO::SetLangSpecificDirName(
