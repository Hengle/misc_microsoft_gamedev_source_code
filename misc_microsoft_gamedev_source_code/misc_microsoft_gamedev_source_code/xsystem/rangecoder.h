...
// Range coding is similiar to arithmetic coding, except flushes are done per-byte instead of per-bit. 
...
public:
   ...
   uint getFreq (uint totFreq) 
   {
      ...
      if (tmp >= totFreq)
      {
         // umm, this is BAD! the stream is corrupted
         return 0;
      }
   ...
   bool decode(uint cumFreq, uint freq, uint totFreq) 
   {
      ...
      mLow  += cumFreq * mRange;
      mRange *= freq;
      while ((mLow ^ mLow + mRange) < Top || mRange < Bottom && ((mRange = -mLow & (Bottom - 1)), 1))
      {
         const int b = mTraits.inputByte();
         if (b < 0)
            return false;
