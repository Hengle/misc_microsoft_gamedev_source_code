...
namespace Rendering
{
   ...
   public class BRenderDebugSphere : BRenderPrimitive
   {
      ...
      {
         createSphere(rad, 3, color, Vector3.Empty);
      }
      public BRenderDebugSphere(float rad, int numSubdivisions, int color)
      {
         createSphere(rad, numSubdivisions, color, Vector3.Empty);
      }
      public BRenderDebugSphere(float rad, int numSubdivisions, int color, Vector3 staticOffset)
      {
         createSphere(rad, numSubdivisions, color, staticOffset);
      }
      private void createSphere(float rad, int numSubdivisions, int color, Vector3 staticOffset)
      {
         ...
         */
         ...
         {
            m_verts[i].x += staticOffset.X;
            m_verts[i].y += staticOffset.Y;
            m_verts[i].z += staticOffset.Z;
         }
      ...
      public void render(bool wireframe, bool alpha, bool dontResetShaders)
      {
   ...
   public class BRenderDebug2DQuad : BRenderPrimitive
   {
      ...
      {
         init(min, max, color, solid,Vector3.Empty);
      }
      public BRenderDebug2DQuad(Vector2 min, Vector2 max, int color, bool solid, Vector3 staticTranslationOffset)
      {
         init(min, max, color, solid,staticTranslationOffset);
      }

      public void init(Vector2 min, Vector2 max, int color, bool solid, Vector3 staticTranslationOffset)
      {
         mMeshType = (int)eMeshType.cMeshDebug;
         ...
         if (solid)
         {
            cubeVertices = new VertexTypes.Pos_Color[]
		      {
			      new VertexTypes.Pos_Color(min.X + staticTranslationOffset.X,staticTranslationOffset.Y,min.Y+ staticTranslationOffset.Z,  color ),
			      new VertexTypes.Pos_Color(min.X + staticTranslationOffset.X,staticTranslationOffset.Y,max.Y+ staticTranslationOffset.Z,  color ),
			      new VertexTypes.Pos_Color(max.X + staticTranslationOffset.X,staticTranslationOffset.Y,min.Y+ staticTranslationOffset.Z,  color ),

               new VertexTypes.Pos_Color(max.X + staticTranslationOffset.X,staticTranslationOffset.Y,min.Y+ staticTranslationOffset.Z,  color ),
               new VertexTypes.Pos_Color(min.X + staticTranslationOffset.X,staticTranslationOffset.Y,max.Y+ staticTranslationOffset.Z,  color ),
               new VertexTypes.Pos_Color(max.X + staticTranslationOffset.X,staticTranslationOffset.Y,max.Y+ staticTranslationOffset.Z,  color ),
		      };
         }
         else
         {
      ...
      public void render(bool alpha, bool twoSided, bool dontResetShaders)
      {
         if (!dontResetShaders)
         {
            BRenderDevice.getDevice().VertexShader = null;
   ...
   //----------------------------
   public class BRenderDebugFullscreenQuad : BRenderPrimitive
   {
      int mColor = Color.White.ToArgb();
      int mWidth = 256;
      int mHeight = 256;
      public BRenderDebugFullscreenQuad(int orthoScreenWidth, int orthoScreenHeight)
      {
         init(orthoScreenWidth, orthoScreenHeight);
      }

      private void init(int orthoScreenWidth, int orthoScreenHeight)
      {
         mMeshType = (int)eMeshType.cMeshDebug;

         mWidth = orthoScreenWidth;
         mHeight = orthoScreenHeight;


         int numVertsPerQuad = 4;


         VertexTypes.PosW_uv0[] g_QuadVertices = new VertexTypes.PosW_uv0[]{
                                    //  x      y      z    w      tu    tv   tu1    tv1
            new VertexTypes.PosW_uv0(0,       mHeight, 0.0f, 1.0f,  0.0f, 1.0f) ,
            new VertexTypes.PosW_uv0(mWidth,  mHeight, 0.0f, 1.0f,  1.0f, 1.0f) ,
            new VertexTypes.PosW_uv0(mWidth,  0,       0.0f, 1.0f,  1.0f, 0.0f) ,
            new VertexTypes.PosW_uv0(0,       0,       0.0f, 1.0f,  0.0f, 0.0f) ,
	      };
         mVB = new VertexBuffer(typeof(VertexTypes.PosW_uv0), numVertsPerQuad, BRenderDevice.getDevice(), 0, VertexTypes.PosW_uv0.FVF_Flags, Pool.Managed);

         using (GraphicsStream stream = mVB.Lock(0, 0, LockFlags.None))
         {
         ...
         int[] inds = new int[] { 2, 1, 0, 0, 3, 2 };
         mIB = new IndexBuffer(BRenderDevice.getDevice(), sizeof(int) * 6, 0, Pool.Managed, false);
         using (GraphicsStream stream = mIB.Lock(0, 0, LockFlags.None))
         {
            stream.Write(inds);
            mIB.Unlock();
         }
      }
      ~BRenderDebugFullscreenQuad()
      {
         destroy();
      }
      public void render()
      ...
      public void render(bool dontResetShaders)
      {
         Viewport oldVP = BRenderDevice.getDevice().Viewport;
         Viewport vp = new Viewport();
         vp.X = 0;
         vp.Y = 0;
         vp.Width = (int)mWidth;
         vp.Height = (int)mHeight;
         vp.MinZ = 0.0f;
         vp.MaxZ = 1.0f;
         ...
         BRenderDevice.getDevice().VertexDeclaration = VertexTypes.PosW_uv0.vertDecl;
         BRenderDevice.getDevice().VertexFormat = VertexTypes.PosW_uv0.FVF_Flags;
         BRenderDevice.getDevice().SetStreamSource(0, mVB, 0);
         BRenderDevice.getDevice().Indices = mIB;
         ...
         BRenderDevice.getDevice().Viewport = oldVP;
      }
   }
