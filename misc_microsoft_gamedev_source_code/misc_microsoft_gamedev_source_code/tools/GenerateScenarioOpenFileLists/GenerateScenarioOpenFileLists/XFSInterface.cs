...
static public class XFSInterface
{
   ...
   static bool ensureXFS()
   {
      ...
      if (mutex == null)
      {
         try
         {
            System.Diagnostics.Process.Start(@"..\" + mXFSPath);
         }
         catch
         {
            return false;
   ...
   static public void safeOpenFileList()
   {
      connectSendMsg("savefileopenlog");
   }

   static public void clearOpenFileList()
   {
      connectSendMsg("clearFileOpenLog");
   }
}