...
const float	BObstructionManager::cWorldObstructionDensityFactor = 0.25f;			// factor to estimate # of objects in world based on number of tiles

const long  BObstructionManager::cMetersPerSmallestQuadCellSide = 8;					// Size of smallest quad node in meters (must be ^ of 2?)
const long  BObstructionManager::cTilesPerSmallestQuad = 4;								// Size of the smallest quad node in Tiles
const long  BObstructionManager::cMetersToQuadCellShift = 3;							// This the N in 2^N = cMetersPerSmallestQuadCellSide; 
const long  BObstructionManager::cMetersToQuadCellULANDMask = 0x7FFFFFF8;			// Mask to round down a coord in meters to the Upper/Left edge of a quad cell
const long  BObstructionManager::cMetersToQuadCellLRORMask  = 0x00000007;		   // Mask to round up a coord in meters to the Lower/Right edge of a quad cell
...
const uint8 cMaximumSupportedPlayers = 7;
...
bool BObstructionManager::initialize(void)
{
   ...
   mTileWidth  = gTerrainSimRep.getNumXTiles();
	...
	for (long qNum = 0; qNum < cNumQuadTrees; qNum++)
	{
		...
		mQTLevelInfo[qNum] = new BOPQuadTreeLevelInfo[mNumQuadTreeLevels];

		// Allocate Storage for the Node Data

		mQuadTreeNodes[qNum] = new BOPQuadTreeNode[mNumQuadTreeNodes];
		...
		for (n = 0; n < mNumQuadTreeNodes; n++)
		{
			...
			mQuadTreeNodes[qNum][n].mReserved = NULL;
	...
	// Setup the Expanded Hull Manager

	mHullManager = new BOPHullArrayManager(1024, 1024);											

   // Enable or disable pathing updates
   bool enablePathingUpdates=true;
...
void BObstructionManager::deinitialize(void)
{
	...
	for (long n= 0; n < cNumQuadTrees; n++)
	{
		...
		{
			delete[] mQTLevelInfo[n];
			mQTLevelInfo[n] = NULL;
		}

		if (mQuadTreeNodes[n] != NULL)
		{
			delete[] mQuadTreeNodes[n];
			mQuadTreeNodes[n] = NULL;
		}
	}
...
void BObstructionManager::generateWorldBoundries(void)
{
	...
	// Need to disable quadpather updating
	...
	for (long x = 0; x < mWidth; x+=cMetersPerSmallestQuadCellSide)
	{
		BOPObstructionNode* EdgeOb1 = mNodeManager->getNode();
		resetObstructionNode(EdgeOb1);
		setAlignedObstruction(EdgeOb1, (float)x, 0.0f-edgeThickness, min((float) (x+cMetersPerSmallestQuadCellSide), mfWidth), 0.0f+cEdgeOfWorldThickness);
		...
		setAlignedObstruction(EdgeOb2, (float)x, (float) mHeight-cEdgeOfWorldThickness, min((float) (x+cMetersPerSmallestQuadCellSide), mfWidth), (float) mHeight+edgeThickness);
	...
	for (long z = 0; z < mHeight; z+=cMetersPerSmallestQuadCellSide)
	{
		BOPObstructionNode* EdgeOb1 = mNodeManager->getNode();
		resetObstructionNode(EdgeOb1);
		setAlignedObstruction(EdgeOb1, 0.0f-edgeThickness, (float)z, 0.0f+cEdgeOfWorldThickness, min((float) (z+cMetersPerSmallestQuadCellSide), mfHeight) );
		EdgeOb1->mType = cObsNodeTypeEdgeofMap;
		EdgeOb1->mProperties |= cObsPropertyUpdatePatherQuadTree;
		...
		BOPObstructionNode* EdgeOb2 = mNodeManager->getNode();
		resetObstructionNode(EdgeOb2);
		setAlignedObstruction(EdgeOb2,  (float) mWidth-cEdgeOfWorldThickness, (float)z, (float) mWidth+edgeThickness, min((float) (z+cMetersPerSmallestQuadCellSide), mfHeight) );
		EdgeOb2->mType = cObsNodeTypeEdgeofMap;
		EdgeOb2->mProperties |= cObsPropertyUpdatePatherQuadTree;
...
void BObstructionManager::addNodeToList(BOPQuadTreeNode* theQuadNode, BOPObstructionNode* theNode)
{
   ...
   BASSERTM(theNode->mType==cObsNodeTypeUnit || theNode->mType==cObsNodeTypeDoppleganger || theNode->mType==cObsNodeTypeTerrain || 
      theNode->mType==cObsNodeTypeEdgeofMap || theNode->mType==cObsNodeTypeMesh, "Invalid obstruction node type being added.");
	...
	long listType = theQuadNode->mListType;
...
/*
   ...
   if (clearExisting)
   {
      ...
      emptyVirtualQuadtreeNodes(cObsTypeNonSolid,        	         (float) x1, (float) z1, (float) (x2 + 1), (float) (z2 + 1), cObsNodeTypeTerrain);
   }
*/
         ...
         while (scanForObs)
         {
            ...
            for (x = 0; x < cTilesPerSmallestQuad; x++)
            {
               for (z = 0; z < cTilesPerSmallestQuad; z++)
               {
                  ...
                  if (oType == cIsNonSolid)
                     obsType = cObsTypeNonSolid;
...
//==============================================================================
void BObstructionManager::findObstructionsOnPoint(const BVector thePoint, float expansionRadius, long QuadTreesToScan, long nodeTypes, long lPlayerID, bool ignoreCurrentList, BObstructionNodePtrArray& theList)
{
	long	incomingSize = 0;
   if (ignoreCurrentList)
   {
...
//	theData.mSEntityIgnoreListSize      = mEntityIgnoreListSize;			
...
//	mEntityIgnoreList						= theData.mSEntityIgnoreList;				
//	mEntityIgnoreListSize				= theData.mSEntityIgnoreListSize;			
...
/*
//==============================================================================
// BObstructionManager::findUnitObstructions
//==============================================================================
void BObstructionManager::findUnitObstructions(const BConvexHull &theHull, bool ignoreNonCollideable, bool ignoreFlying, CHECKUNIT_CALLBACK checkUnitCB, void *checkUnitParam)
{
   ignoreFlying;

	long QuadTreesToScan = cIsNewTypeCollidableMovingUnit | cIsNewTypeCollidableStationaryUnit | 
								  cIsNewTypeSolidNonMoveableUnits |	cIsNewTypeNonSolid;

	if (!ignoreNonCollideable)
		QuadTreesToScan |= cIsNewTypeNonCollidableUnit;

   #ifdef USE_COLLIDABLE_AIR_UNITS
      if(!ignoreFlying)
         QuadTreesToScan |= cIsNewTypeStationaryAirUnit | cIsNewTypeMovingAirUnit;
   #endif

	mFoundObstructionResults.setNumber(0);

	long NodeTypes = cObsNodeTypeUnit;
...
// BObstructionManager::findUnitObstructions
//==============================================================================
void BObstructionManager::findUnitObstructions(const float minX, const float minZ, const float maxX, const float maxZ, bool ignoreNonCollideable, bool ignoreFlying)
{
   ignoreFlying;
...
// BObstructionManager::findUnitObstructions
//==============================================================================
void BObstructionManager::findUnitObstructions(const BVector p0, const BVector p1, bool ignoreNonCollideable, bool ignoreFlying)
{
   ignoreFlying;
...
//==============================================================================
// BObstructionManager::findUnitObstructionsQuadHull
//==============================================================================
void BObstructionManager::findUnitObstructionsQuadHull(const BOPQuadHull *pHull, bool ignoreNonCollideable, bool ignoreFlying)
{
   ignoreFlying;
...
bool BObstructionManager::updatePlayerOwnedMasks()
{
   ...
   #ifdef USE_BLOCKER_UNITS
      findObstructions(0.0f, 0.0f, mfWidth, mfHeight, 0.0f, cIsNewTypeBlockerUnit, cObsNodeTypeUnit, -1L, false, obstructions);
      lObCount = obstructions.getNumber();
      for (long n = 0; n < lObCount; n++)
      {
         BOPObstructionNode *pnode = obstructions[n];
         if (pnode->mPlayerID)
         {
            this->updateObstructionLocation(pnode, cObsTypeBlockerUnit, true);
         }
      }
   #endif
...
bool BObstructionManager::begin(ObsBeginType mode, float radius, long QuadTreeToScan, long validNodeTypes, long lPlayerID, 
																	float ExpansionScale, const BTypedID *ignoreList, long ignoreListSize)
{
	mode;
	mRadius = radius;

	return(begin(cBeginNone, 0, 0, QuadTreeToScan, validNodeTypes, lPlayerID, ExpansionScale, ignoreList, ignoreListSize));
...
*/
   ...
   if (mQuadTreesToScan == -1)
   {
	...
	/*mEntityIgnoreList = ignoreList;
	mEntityIgnoreListSize = ignoreListSize;

	if (mEntityIgnoreList != NULL && mEntityIgnoreListSize > 0 )
	{
		for (long n = 0; n < mEntityIgnoreListSize; n++)
		{
			if (mEntityIgnoreList[n].getType() == BEntity::cUnit)
			{
				theUnit = theWorld->getUnit(mEntityIgnoreList[n].getID());
				if (theUnit != NULL  && theUnit->getObstructionData() != NULL)
				{
					theUnit->getObstructionData()->mProperties |= cObsPropertyInIgnoreList;
				}
			}
		}
	}*/
...
bool BObstructionManager::end()
{
	...
	/*if (mEntityIgnoreList != NULL && mEntityIgnoreListSize > 0 )
	{
		for (long n = 0; n < mEntityIgnoreListSize; n++)
		{
			...
			{
				theUnit = theWorld->getUnit(mEntityIgnoreList[n].getID());
				if (theUnit != NULL && theUnit->getProtoObject()->getRadiusX() != NULL)
				{
					theUnit->getObstructionData()->mProperties &= ~cObsPropertyInIgnoreList;
				}
			}
	...
	mEntityIgnoreList = NULL;
	mEntityIgnoreListSize = 0;
   ...
   for (long curTree=0, treeFlags=QuadTreesToRender; treeFlags != 0 && curTree < cNumQuadTrees; curTree++, treeFlags>>=1)
   {
      if ((treeFlags & 0x0001) == 0)
		...
		for (long n = 0; n < quadNode->mNumObstructions; n++)
		{
            ...
            gTerrainSimRep.addDebugLineOverTerrain(mTempVectors[0], mTempVectors[1], cDWORDRed, cDWORDRed, 0.2f);
...
// output:  none
//==============================================================================
void BObstructionManager::addUnitIgnoreList(const BDynamicSimLongArray &IDList)
{
...
//==============================================================================
void BOPQuadHull::suggestPlacement(const BVector vLocation, float fTestRadiusX, float fTestRadiusZ, BVector &vSuggestion) const
{
#if 0
...
#ifdef USE_BLOCKER_UNITS
      BObstructionManager::cIsNewTypeBlockerUnit              | // Unit that can't move and blocks all units including flying ones
#endif
      BObstructionManager::cIsNewTypeCollidableNonMovableUnit | // Unit that can't move                                                       
      BObstructionManager::cIsNewTypeNonSolid                 | // Non-Solid means you can path across it, but you can't build on it.  
      BObstructionManager::cIsNewTypeBlockLandMovement        | // Terrain that blocks Land-based movement
      BObstructionManager::cIsNewTypeBlockAllMovement         | // Terrain that blocks all types of movement, Now includes the edge of the map
      BObstructionManager::cIsNewTypeBlockNonAir;               // Terrain that blocks all but air movement

   // Check 
   for (long lDir = 0; lDir < 4; lDir++)
   {
      switch (lDir)
      {
         case 0:
