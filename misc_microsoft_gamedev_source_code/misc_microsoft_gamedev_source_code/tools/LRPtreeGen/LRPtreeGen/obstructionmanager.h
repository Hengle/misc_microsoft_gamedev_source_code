...
#define USE_BLOCKER_UNITS
#define USE_COLLIDABLE_AIR_UNITS
...
//		const BTypedID*			mSEntityIgnoreList;									// BTypedID array of units to ignore
//		long							mSEntityIgnoreListSize;								// Size of above array
};
...
//      void                    suggestPlacement(const BVector vLocation, float fTestRadiusX, float fTestRadiusZ, BVector &vSuggestion) const;
...
//      bool                    createSimpleHull(const BVector* points);
	   ...
	   BOPQuadHull*				mNextNode;					// Pointer for linking nodes in the free node list
		...
		float							mRadius;						// Redius the hull has been expanded by
...
typedef struct BOPQuadTreeNode
{
	...
	long						mNumObstructions;			// Number of valid entries in the Obstruction List
	long						mListType;					// Which type of list is present
	long						mReserved;					// Padding to round out struct to 16 bytes (Compiler would do it anyway...)
...
class BObstructionManager
{
   public:
      ...
      enum
      {
         ...
         cObsTypeNonSolid,                      // Non-Solid means you can path across it, but you can't build on it.  
      ...
      enum
      {
         ...
         cIsNewTypeNonSolid                     = 0x0010,      // Non-Solid means you can path across it, but you can't build on it.  
         ...
         cIsNewTypeAllCollidableUnits           = 0x000E,      // All BUnits that are collidable (movable and non-movable)
         cIsNewTypeAllCollidableSquads          = 0x0300,      // All BSquads that are collidable (movable and non-movable)

         cRenderNodesMode1                      = cIsNewTypeAnyUnit,                // Used by Diagnostic obstruction Render (debug) display
         cRenderNodesMode2                      = cIsNewTypeAllCollidableSquads,
         cRenderNodesMode3                      = cIsNewTypeBlockLandUnits,
      };
...
//		void							updatePathingInformation(BOPObstructionNode *theNode);
		...
		bool							begin(ObsBeginType mode, long modeval1, long modeval2, long QuadTreeToScan, long validNodeTypes, long lPlayerID, 
																	float ExpansionScale, const BTypedID *ignoreList = NULL, long ignoreListSize = 0);
...
//      void							findObstructions(float x1, float z1, float x2, float z2, float expansionRadius, long QuadTreesToScan, long nodeTypes, long lPlayerID, bool ignoreCurrentList, BObstructionNodePtrArray &theList);
      ...
      //Halwes - 10/9/2006 - nothing ever uses the bool returned from findObstructionsOnPoint and the 360 compiler does not like functions that return Booleans as per
      //                     "Xbox 360 CPU Performance Update" talk from Summer 2006 GameFest.  Also I've been told passing by value is better on 360 so I changed the first
      //                     parameter to do this.
      void							findObstructionsOnPoint(const BVector thePoint, float expansionRadius, long QuadTreesToScan, long nodeTypes, long lPlayerID, bool ignoreCurrentList, BObstructionNodePtrArray& theList);
...
//		void							findUnitObstructions(const BConvexHull &theHull, bool ignoreNonCollideable, bool ignoreFlying=false, CHECKUNIT_CALLBACK checkUnitCB=NULL, void *checkUnitParam=NULL);
//		void							findUnitObstructions(const float minX, const float minZ, const float maxX, const float maxZ, bool ignoreNonCollideable, bool ignoreFlying=false);
//    void                    findUnitObstructions(const BVector p0, const BVector p1, bool ignoreNonCollideable, bool ignoreFlying=false);
//		void							findUnitObstructionsQuadHull(const BOPQuadHull *pHull, bool ignoreNonCollideable, bool ignoreFlying=false);
...
//		long							performNewWallCheck( const BVector &point1, const BVector &point2, long ignoreWallForPlayerID, BWallCheckResultArray &results, bool ignoreWalls=true);
      ...
      // These work in conjunction with findUnitObstructions.
...
//      void                    addUnitIgnore(long id);
//      void                    addUnitIgnoreList(const BDynamicSimLongArray &IDList);
//      void                    clearUnitIgnore(long id);
//      void                    clearUnitIgnoreList(const BDynamicSimLongArray &IDList);
...
//      static void CALLBACK    findBlackTiles(long x, long z, void* pParam);
      ...
      void                    makeValidZ(long &z);
		...
		const BTypedID*			mEntityIgnoreList;									// BTypedID array of units to ignore
		long							mEntityIgnoreListSize;								// Size of above array
