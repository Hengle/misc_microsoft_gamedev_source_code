...
{
   public partial class AnimTagSoundPage : UserControl
   {
      ...
      public void bindData(visualModelAnimAssetTag model, TreeNode treeNode)
      {
         ...
         soundBrowseControl1.FileName = mData.name;
         visibleCheckBox.Checked = mData.checkvisible;
         checkBoxFOW.Checked = mData.checkFOW;
      ...
      private void soundBrowseControl1_ValueChanged(object sender, EventArgs e)
      {
         // early out if currently binding data
         if (mIsBindingData)
            return;
      ...
      private void checkBoxFOW_CheckedChanged(object sender, EventArgs e)
      {
         // early out if currently binding data
         if (mIsBindingData)
            return;
