...
namespace Terrain
{
   public abstract class BTerrainFlightBrush : BTerrainBrush
   {
      public BTerrainFlightBrush() { }
      ~BTerrainFlightBrush() { }
      ...
      public virtual void applyOnBrush(List<int> indexes, ref Vector3 brushApplyPt, ref Vector3 brushApplyNormal, ref BrushInfo brushInfo, bool alternate)
      {

         {
            ...
            for (int i = 0; i < indexes.Count; i++)
            {
               ...
               Vector3 vertCurrentPos = TerrainGlobals.getEditor().getSimRep().getFlightRep().getWorldspacePoint(x, z).toVec3();
   ...
   public class BTerrainFlightHeightBrush : BTerrainFlightBrush
   {
      public BTerrainFlightHeightBrush()
      {
         m_bApplyOnSelection = true;
      }
      ~BTerrainFlightHeightBrush() { }
      ...
      public override void apply(List<VertIndexWeight> verts, float intensity, bool alternate)
      {
         ...
         for (int i = 0; i < verts.Count; i++)
         {
            ...
            // Change the vertex.
            float overideHeight = TerrainGlobals.getEditor().getSimRep().getFlightRep().getJaggedHeight(x, z);
            if (overideHeight != SimFlightRep.cJaggedEmptyValue)
            {
               overideHeight += factor * weight;
            }
            else
            {
               overideHeight = TerrainGlobals.getEditor().getSimRep().getFlightRep().getHeight(x, z) + (factor * weight);
            }

            TerrainGlobals.getEditor().getSimRep().getFlightRep().setJaggedHeight(x, z, overideHeight);
   ...
   public class BTerrainFlightSetHeightBrush : BTerrainFlightBrush
   {
      private float mSampledHeight;
      public BTerrainFlightSetHeightBrush()
      {
         m_bApplyOnSelection = true;
      }
      ~BTerrainFlightSetHeightBrush() { }
   ...
   public class BTerrainFlightSmoothBrush : BTerrainFlightBrush
   {
      public BTerrainFlightSmoothBrush()
      {
         m_bApplyOnSelection = true;
      ...
      public override void apply(List<VertIndexWeight> verts, float intensity, bool alternate)
      {
         ...
         for (int i = 0; i < verts.Count; i++)
         {
            ...
            float curVertPos = TerrainGlobals.getEditor().getSimRep().getFlightRep().getHeight(x, z);
            float curDef = TerrainGlobals.getEditor().getSimRep().getFlightRep().getCompositeHeight(x, z);


            float newDeformation = curDef + ((newVertPos_Y - curVertPos) * weight_scaled);

            TerrainGlobals.getEditor().getSimRep().getFlightRep().setJaggedHeight(x, z, newVertPos_Y);
      ...
      public void applyFastLowPassFilter(int xPos, int zPos, out float newVertPos_Y)
      {
         ...
         if (minX < 0) minX = 0;
         if (minZ < 0) minZ = 0;
         if (maxX >= TerrainGlobals.getEditor().getSimRep().getFlightRep().getNumXPoints()) maxX = (int)(TerrainGlobals.getEditor().getSimRep().getFlightRep().getNumXPoints() - 1);
         if (maxZ >= TerrainGlobals.getEditor().getSimRep().getFlightRep().getNumZPoints()) maxZ = (int)(TerrainGlobals.getEditor().getSimRep().getFlightRep().getNumZPoints() - 1);
         ...
         for (x = minX; x <= maxX; x++)
         {
            for (y = minZ; y <= maxZ; y++)
            {
               newVertPos_Y += TerrainGlobals.getEditor().getSimRep().getFlightRep().getCompositeHeight(x, y);
               numCoefficents++;
            }
         }
   ...
   public class BTerrainFlightEraseOverrideBrush : BTerrainFlightBrush
   {
      public BTerrainFlightEraseOverrideBrush()
      {
         m_bApplyOnSelection = true;
      }
      ~BTerrainFlightEraseOverrideBrush() { }
      ...
      public override void apply(List<VertIndexWeight> verts, float intensity, bool alternate)
      {
         ...
         for (int i = 0; i < verts.Count; i++)
         {
            ...
            TerrainGlobals.getEditor().getSimRep().getFlightRep().setJaggedHeight(x, z, SimFlightRep.cJaggedEmptyValue);
