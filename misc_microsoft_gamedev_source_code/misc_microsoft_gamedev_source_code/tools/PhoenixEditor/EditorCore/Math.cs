...
namespace EditorCore
{
   ...
   static public class BMathLib
   {
      ...
      static public bool ray3AABB(ref Vector3 coord, ref float t, ref Vector3 rayOrig, ref Vector3 rayDir, ref Vector3 boxmin, ref Vector3 boxmax)
      {
         ...
         float[] minB = new float[] { boxmin.X, boxmin.Y, boxmin.Z };
         float[] maxB = new float[] { boxmax.X, boxmax.Y, boxmax.Z }; ;		/*box */
         float[] origin = new float[] { rayOrig.X, rayOrig.Y, rayOrig.Z };
         float[] dir = new float[] { rayDir.X, rayDir.Y, rayDir.Z };
         float[] coordD = new float[] { rayOrig.X, rayOrig.Y, rayOrig.Z };
         {
            bool inside = true;
            ...
            for (int i = 0; i < NUMDIM; i++)
            {
               if (whichPlane != i)
               {
                  coordD[i] = origin[i] + maxT[whichPlane] * dir[i];

                  if (coordD[i] < minB[i] || coordD[i] > maxB[i])
                  {
   ...
   public class floatN
   {
      ...
      public floatN Clone()
      {
         floatN newN = new floatN(mDimensions);
         for (uint i = 0; i < mDimensions; i++)
            newN[i] = mDataMembers[i];
   ...
   public class Set
   {
      ...
      /// <param name="b"></param>
      /// <returns></returns>
      static public List<int> Difference(List<int> a, List<int> b)
      {
      ...
      /// <param name="b"></param>
      /// <returns></returns>
      static public bool IsSubset<T>(List<T> a, List<T> b)
      {
         foreach (T i in a)
         {
