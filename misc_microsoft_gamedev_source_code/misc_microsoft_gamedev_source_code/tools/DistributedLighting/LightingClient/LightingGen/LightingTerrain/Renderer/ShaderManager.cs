...
namespace Rendering
{
   ...
   public class ShaderHandle
   {
      ...
      ~ShaderHandle()
      {
         ...
         catch (System.Exception ex)
         {
            ...
            Console.WriteLine("Failed to destroy shader handle");
      ...
      void loadInternalFromDisk(string filename)
      {
         string errors = "";
         {
            try
            ...
            {
               Console.ForegroundColor = ConsoleColor.Red;
               Console.WriteLine("Shader " + filename + " did not load");
               Console.ForegroundColor = ConsoleColor.White;
               throw (ex);
            }
         }
      ...
      public void reload()
      {
         ...
         loadInternalFromDisk(mFilename);
   ...
   public class ShaderManager
   {
      ...
      public void init()
      {
         ...
         if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "\\shaders"))
         {
            mFileWatcher = new FileSystemWatcher();
            mFileWatcher.Path = AppDomain.CurrentDomain.BaseDirectory + "\\shaders";
            mFileWatcher.Filter = "*.*";
