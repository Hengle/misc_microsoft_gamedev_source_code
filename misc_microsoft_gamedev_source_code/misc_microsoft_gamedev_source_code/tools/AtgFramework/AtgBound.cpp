...
//-----------------------------------------------------------------------------
Bound Bound::operator*( const XMMATRIX World ) const
{
    //$OPTIMIZE: store matrix decomposed    
    XMVECTOR Translation = World.r[3];       
    FLOAT Scale = XMVector3Length( World.r[2] ).x;    
...
VOID Bound::Merge( const Bound& Other )
{
    ...
    FLOAT fCombinedDiameter = vDistance.x + ThisSphere.Radius + OtherSphere.Radius;
    ...
    ThisSphere.Radius = vMaxRadius.x;
