...
public:
    ...
    FLOAT                GetLocalRange() CONST { return m_fRange; };
    VOID                 SetLocalRange( FLOAT fRange );

    FLOAT                GetWorldRange();       
    VOID                 SetWorldRange( FLOAT fRange );
    
    FalloffType          GetFalloff() CONST { return m_FalloffType; };
    VOID                 SetFalloff( FalloffType Falloff ) { m_FalloffType = Falloff; }
...
public:
    ...
    VOID                SetLightViewProjection( const DWORD dwIndex, const XMMATRIX matVP ) { XMStoreFloat4x4( &m_matLightViewProj[dwIndex], matVP ); }
    XMMATRIX            GetLightViewProjection( const DWORD dwIndex ) const { return XMLoadFloat4x4( &m_matLightViewProj[dwIndex] ); }
...
public:
    ...
    FLOAT               GetLocalRange() CONST { return m_fRange; };
    VOID                SetLocalRange( FLOAT fRange ) { m_fRange = fRange; RecalculateBound(); }

    FLOAT               GetWorldRange(); 
    VOID                SetWorldRange( FLOAT fRange );
    
    FLOAT               GetInnerAngle() CONST { return m_fInnerAngle; };
    VOID                SetInnerAngle( FLOAT fInnerAngle ) { m_fInnerAngle = fInnerAngle; RecalculateBound(); }

    FLOAT               GetOuterAngle() CONST { return m_fOuterAngle; };
    VOID                SetOuterAngle( FLOAT fOuterAngle ) { m_fOuterAngle = fOuterAngle; RecalculateBound(); }

    FalloffType         GetFalloff() CONST { return m_FalloffType; };
    VOID                SetFalloff( FalloffType Falloff ) { m_FalloffType = Falloff; }

    FalloffType         GetSpotFalloff() CONST { return m_SpotFalloffType; };
    VOID                SetSpotFalloff( FalloffType Falloff ) { m_SpotFalloffType = Falloff; }

    XMMATRIX            GetLightProjection() { return XMMatrixPerspectiveFovLH( m_fOuterAngle, 1.0f, 0.1f, GetWorldRange() ); }
    XMMATRIX            GetLightView() { XMVECTOR vD; return XMMatrixInverse( &vD, GetWorldTransform() ); }
    XMMATRIX            GetLightViewProjection() { return GetLightView() * GetLightProjection(); }
