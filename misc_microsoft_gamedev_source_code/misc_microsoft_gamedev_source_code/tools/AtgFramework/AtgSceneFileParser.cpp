...
HRESULT SceneFileParser::LoadXATGFile( const CHAR* strFilename, Scene* pScene, Frame* pRootFrame, DWORD dwFlags, DWORD* pLoadProgress )
{
    XMLParser* pParser = new XMLParser();
    SceneFileParser* pXATGParser = new SceneFileParser();

    g_strParseError[0] = '\0';

    pParser->RegisterSAXCallbackInterface( pXATGParser );
    ...
    pXATGParser->m_Context.pCurrentObject = pScene;
    pXATGParser->m_Context.pCurrentParentFrame = NULL;
    pXATGParser->m_Context.pCurrentParentObject = NULL;
    ...
    HRESULT hr = pParser->ParseXMLFile( strFilename );
...
HRESULT SceneFileParser::ElementBegin( const WCHAR* strName, UINT NameLen, const XMLAttribute *pAttributes, UINT NumAttributes )
{
...
VOID SceneFileParser::ProcessRootData()
{
    if( !m_CurrentElementDesc.bEndElement )
    {
        ...
        else if( MATCH_ELEMENT_NAME( L"PhysicalMemoryFile" ) )
        {
            ...
            WideCharToMultiByte( CP_ACP, 0, m_CurrentElementDesc.strElementBody, wcslen( m_CurrentElementDesc.strElementBody ) + 1, strBlobFile, MAX_PATH, NULL, NULL );
            strcat_s( strFileName, strBlobFile );
            HANDLE hFile = CreateFile( strFileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL );
            ...
            assert( dwFileSize > 0 );
            g_pBinaryBlobData = (BYTE*)g_pCurrentScene->GetResourceDatabase()->PhysicalAlloc( dwFileSize, 0, PAGE_READWRITE | PAGE_WRITECOMBINE );
            DWORD dwBytesRead = 0;
            ReadFile( hFile, g_pBinaryBlobData, dwFileSize, &dwBytesRead, 0 );
            CloseHandle( hFile );
            return;
        }
        else if( MATCH_ELEMENT_NAME( L"BundledResources" ) )
        {
            ...
            WideCharToMultiByte( CP_ACP, 0, m_CurrentElementDesc.strElementBody, wcslen( m_CurrentElementDesc.strElementBody ) + 1, strBundleFile, MAX_PATH, NULL, NULL );
    ...
    if( !m_CurrentElementDesc.bEndElement )
    {
        ...
        else if( MATCH_ELEMENT_NAME( L"IndexBuffer" ) )
        {
            ...
            if( strValue != NULL )
            {
                ...
                VOID* pBuffer = g_pCurrentScene->GetResourceDatabase()->PhysicalAlloc( dwBufferSize, 32, PAGE_READWRITE | PAGE_WRITECOMBINE );
        ...
        else if( MATCH_ELEMENT_NAME( L"IBSubset" ) )
        {
            ...
            if( strValue != NULL )
            {
                ...
                switch( pSubsetDesc->GetPrimitiveType() )
                {
        ...
        else if( m_Context.CurrentObjectType == XATG_VERTEXBUFFER )
        {
            ...
            if( MATCH_ELEMENT_NAME( L"Vertices" ) )
            {
                ...
                VOID* pBuffer = g_pCurrentScene->GetResourceDatabase()->PhysicalAlloc( dwVBSize, 32, PAGE_READWRITE | PAGE_WRITECOMBINE );
...
VOID SceneFileParser::ProcessModelData()
{
    ...
    if( !m_CurrentElementDesc.bEndElement )
    {
        ...
        else if( MATCH_ELEMENT_NAME( L"SubsetMaterialMapping" ) )
        {
            ...
            if( strValue != NULL )
            {
                MaterialInstance* pMaterial = (MaterialInstance*)g_pCurrentScene->FindObjectOfType( strValue, MaterialInstance::TypeID );
    ...
    else
    {
        if( MATCH_ELEMENT_NAME( L"Model" ) )
        {
            ...
            g_pCurrentScene->AddObject( pModel );
...
VOID SceneFileParser::ProcessMaterialData()
{
    ...
    if( !m_CurrentElementDesc.bEndElement )
    {
        if( MATCH_ELEMENT_NAME( L"MaterialInstance" ) )
        {
            ...
            {
                BaseMaterial* pBaseMaterial = (BaseMaterial*)g_pCurrentScene->GetResourceDatabase()->FindResourceOfType( strBaseMaterialName, BaseMaterial::TypeID );
                if( pBaseMaterial == NULL )
                {
                    pBaseMaterial = new BaseMaterial();
                    pBaseMaterial->SetName( strBaseMaterialName );
                    g_pCurrentScene->AddObject( pBaseMaterial );
