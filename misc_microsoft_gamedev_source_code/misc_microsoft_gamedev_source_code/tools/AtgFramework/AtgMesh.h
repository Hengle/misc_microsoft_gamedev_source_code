...
const DWORD XBG7_FILE_ID = (((DWORD)'X'<<24)|(((DWORD)'B'<<16))|(((DWORD)'G'<<8))|(7<<0));
...
public:
    ...
    // Access functions
    MESH_FRAME* GetFrame( DWORD i ) { return &m_pFrames[i]; }
    MESH_DATA*  GetMesh( DWORD i )  { return m_pFrames[i].m_pMeshData; }
    ...
    // rendering.
    virtual BOOL RenderCallback( DWORD dwSubset, const MESH_SUBSET* pSubset, DWORD dwFlags ) { return TRUE; }
    ...
    VOID ComputeFrameBoundingBox( const MESH_FRAME* pFrame, const XMMATRIX matParentMatrix, XMVECTOR& vMin, XMVECTOR& vMax );
...
public:
    ...
    MESH_DATA* GetMesh() const { return m_pMeshData; }
};
