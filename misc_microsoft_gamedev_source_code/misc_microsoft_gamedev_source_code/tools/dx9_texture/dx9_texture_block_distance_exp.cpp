...
bool loadTexture2(const char* pSrcFilename, const char* pDstFilename, int quality, int codebookSize, bool virtualCodebook, float bitRate, int method, bool subsampling, bool useAveDelta, bool useVisualQuant)
{
   ...
   BArray2D<uchar> lumaImage(width, height);
   ...
   for (int y = 0; y < height; y++)
   {
      ...
      for (int x = 0; x < width; x++)
      {
         ...
         lumaImage(x, y) = static_cast<uchar>(yCoCg.r);
   ...
   const uint cTilesX = 32;
   const uint cTilesY = 32;
   const uint tileXCells = cellsX / cTilesX;
   const uint tileYCells = cellsY / cTilesY;
         
   BArray2D<BMatrix16x16> tileEigenvectors(cTilesX, cTilesY);
   BArray2D<BVec16> tileEigenvalues(cTilesX, cTilesY);
   
   BArray2D<BVec16Array> tileBasisVecs(cTilesX, cTilesY);
   BArray2D<BVec16> tileMeans(cTilesX, cTilesY);
   BArray2D<uchar> tileDimensions(cTilesX, cTilesY);
   ...
   BArray2D< std::pair<uint, uint> > closestTiles(cTilesX, cTilesY);
   BArray2D<float> closestError(cTilesX, cTilesY);
   
   const uint tileWidth = image.getWidth() / cTilesX;
   const uint tileHeight = image.getHeight() / cTilesY;
   
   BRGBAImage tileImage(image);
   for (uint x = 0; x < cTilesX; x++)
      tileImage.drawLine(x*tileWidth, 0, x*tileWidth, image.getHeight(), BRGBAColor(64,64,64,0));
   for (uint y = 0; y < cTilesY; y++)
      tileImage.drawLine(0, y*tileHeight, image.getWidth() - 1, y*tileHeight, BRGBAColor(64,64,64,0));
     
   BArray2D< BArray2D<float> > projError(cTilesX, cTilesY);
   for (uint y = 0; y < cTilesY; y++)
      for (uint x = 0; x < cTilesX; x++)
         projError(x,y).resize(cTilesX, cTilesY);
            
   for (uint y = 0; y < cTilesY; y++)
   {
      debugPrintf("Finding matches: %i\n", y);
      
      for (uint x = 0; x < cTilesX; x++)
      {
         float bestError = 1e+30f;
         int bestX = 0;
         int bestY = 0;
         
         BArray2D<float> tileLuma(tileXCells*4, tileYCells*4);
         
         bool allSame = true;
         uint prev;
         for (uint sy = 0; sy < tileYCells * 4; sy++)
            for (uint sx = 0; sx < tileXCells * 4; sx++)
            {
               tileLuma(sx, sy) = lumaImage(x*tileXCells*4+sx, y*tileYCells*4+sy) - tileMeans(x, y)[(sx&3)+(sy&3)*4];
               
               if ((sx|sy)==0)
                  prev = tileLuma(sx, sy);
               else if (tileLuma(sx, sy) != prev)
                  allSame = false;
            }
         
         for (uint iy = 0; iy < cTilesY; iy++)
         {
            for (uint ix = 0; ix < cTilesX; ix++)
            {
               if ((ix == x) && (iy == y))
               {
                  projError(x, y)(ix, iy) = 1e+30f;
                  continue;
               }
                              
               const BVec16Array& basis = tileBasisVecs(ix, iy);
               const BVec16& mean = tileMeans(ix, iy);

               BArray2D<float> residLuma(tileLuma);               
               
               const uint numBasisVecs = 9;
               for (uint i = 0; i < numBasisVecs; i++)
               {  
                  for (uint cy = 0; cy < tileYCells; cy++)
                  {
                     for (uint cx = 0; cx < tileXCells; cx++)
                     {
                        float sum = 0.0f;
                        for (uint sy = 0; sy < 4; sy++)
                           for (uint sx = 0; sx < 4; sx++)
                              sum += basis[i][sx+sy*4] * tileLuma(cx*4+sx, cy*4+sy);
                              
                        for (uint sy = 0; sy < 4; sy++)
                           for (uint sx = 0; sx < 4; sx++)
                              residLuma(cx*4+sx, cy*4+sy) -= basis[i][sx+sy*4] * sum;
                     }
                  }
               }
               ...
               for (uint sy = 0; sy < tileYCells * 4; sy++)
                  for (uint sx = 0; sx < tileXCells * 4; sx++)
                     totalError += residLuma(sx, sy)*residLuma(sx, sy);

               if (totalError < 1.0f)
                  totalError = 0.0f;
                  
               projError(x, y)(ix, iy) = totalError;
         ...
         closestTiles(x, y).first = bestX;
         closestTiles(x, y).second = bestY;
         closestError(x, y) = bestError;
      }
   }
   
   BBitArray2D<1> merged(cTilesX, cTilesY);
   BArray2D<float> extraError(cTilesX, cTilesY);
   
   uint n = 0;
   for ( ; ; )
   {
      float lowestError = 1e+30f;

      uint bestX = 0, bestY = 0;
      uint bestIX = 0, bestIY = 0;
      for (uint y = 0; y < cTilesY; y++)
      {
         for (uint x = 0; x < cTilesX; x++)
         {
            ...
            for (uint iy = 0; iy < cTilesY; iy++)
            {
               for (uint ix = 0; ix < cTilesX; ix++)
               {
                  ...
                  float error = projError(x, y)(ix, iy) + projError(ix, iy)(x, y) + extraError(x, y) + extraError(ix, iy);
      ...
      if (lowestError == 1e+30f)
         break;
         
      //closestError(bestX, bestY) = 1e+30f;
      
      extraError(bestX, bestY) += (projError(bestX, bestY)(bestIX, bestIY) + projError(bestIX, bestIY)(bestX, bestY)) * .15f + extraError(bestIX, bestIY);
      projError(bestX,bestY)(bestIX,bestIY)=1e+30f;
      projError(bestIX,bestIY)(bestX,bestY)=1e+30f;
      //merged(bestX,bestY)=1;
      merged(bestIX,bestIY)=1;
      
      //if (lowestError < 1.0f)
      //   continue;
      n++;
      
      tileImage.drawLine(bestX*tileWidth+tileWidth/2, bestY*tileHeight+tileHeight/2, bestIX*tileWidth+tileWidth/2+2, bestIY*tileHeight+tileHeight/2+2, ((x+y)&1) ? BRGBAColor(128,0,128,0) : BRGBAColor(0,128,0,0));         
      
      if (n >= ((cTilesX*cTilesY-64)))
         break;
         
      if ((n % 64) == 63)
      {
         char buf[256];
         sprintf(buf, "tileimage%i.tga", n);
         BImageUtils::writeTGA24(buf, tileImage);    
         tileImage = image;
         for (uint x = 0; x < cTilesX; x++)
            tileImage.drawLine(x*tileWidth, 0, x*tileWidth, image.getHeight(), BRGBAColor(64,64,64,0));
         for (uint y = 0; y < cTilesY; y++)
   ...
   BImageUtils::writeTGA24("tilebasis.tga", basisImage);    
   BImageUtils::writeTGA24("tileimage.tga", tileImage);    
...
#undef B
   ...
   BArray2D<BVec4> fdctImage[cNumLumaTextures];
