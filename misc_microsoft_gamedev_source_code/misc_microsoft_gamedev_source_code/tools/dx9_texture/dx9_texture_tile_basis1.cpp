   ...
   
{
   ...
   uint tileXCells = cellsX / 4;
   uint tileYCells = cellsY / 4;
         ...
         for (uint cy = 0; cy < 4; cy++)
         {
            for (uint cx = 0; cx < 4; cx++)
            {
               ...
               for (uint sy = 0; sy < 4; sy++)
               {
                  ...
                  {
                     BRGBAColor c;
                     c.set(v[sx+sy*4]*127.0f+128.0f);
                     basisImage.fillRect((sx*2)+(cx*10)+(tx*90), sy*2+(cy*10)+(ty*48), 2,2, c);
                  }
               }
            }
         }  
