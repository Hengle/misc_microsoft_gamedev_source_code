...
bool loadTexture2(const char* pSrcFilename, const char* pDstFilename, int quality, int codebookSize, bool virtualCodebook, float bitRate, int method, bool subsampling, bool useAveDelta, bool useVisualQuant)
{
   ...
   for (uint i = 0; i < 4; i++)
      dctImage[i].setSize(cellsX, cellsY);
   ...
   for (uint sy = 0; sy < cellsY; sy++)
   {
      for (uint sx = 0; sx < cellsX; sx++)
      {
         ...
         int a0 = round(i[0][0]*.25f);
         int r0 = round(i[2][0]*.25f)+123;
         int g0 = round(i[0][2]*.25f)+125;
         
         int a1 = round(i[1][0]*.25f)+128;
         int g1 = round(i[3][0]*.25f)+125;
         int r1 = round(i[0][3]*.25f)+123;
         
         int a2 = round(i[0][1]*.25f)+128;
         int r2 = round(i[2][1]*.25f)+123;
         int g2 = round(i[1][2]*.25f)+125;
         
         int a3 = round(i[1][1]*.25f)+128;
         int g3 = round(i[3][1]*.25f)+125;
         int r3 = round(i[1][3]*.25f)+123;
         
         //if ((r0|g0|a1|g1|r1|a2|r2|g2|a3|g3|r3)&0xFFFFFF00)
         //   DebugBreak();
                  
         dctImage[0](sx, sy).set(r0, g0, 0, a0);
         dctImage[1](sx, sy).set(r1, g1, 0, a1);
         dctImage[2](sx, sy).set(r2, g2, 0, a2);
         dctImage[3](sx, sy).set(r3, g3, 0, a3);
   ...
   for (uint i = 0; i < 4; i++)
   {
      ...
      dctImage[i].setSize(0,0);
      
      BDXTUnpacker unpacker;   
      unpacker.unpack(dctImage[i], &dxtData[0], cDXT5, cellsX, cellsY);
   }
   ...
   for (uint sy = 0; sy < cellsY; sy++)
   {
      for (uint sx = 0; sx < cellsX; sx++)
      {
         ...
         float a0 = c0.a*4.0f;
         float r0 = (c0.r-123)*4.0f;
         float g0 = (c0.g-125)*4.0f;

         float a1 = (c1.a-128)*4.0f;
         float g1 = (c1.g-125)*4.0f;
         float r1 = (c1.r-123)*4.0f;

         float a2 = (c2.a-128)*4.0f;
         float r2 = (c2.r-123)*4.0f;
         float g2 = (c2.g-125)*4.0f;

         float a3 = (c3.a-128)*4.0f;
         float g3 = (c3.g-125)*4.0f;
         float r3 = (c3.r-123)*4.0f;
