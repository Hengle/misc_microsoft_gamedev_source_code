...
bool loadTexture2(const char* pSrcFilename, const char* pDstFilename, int quality, int codebookSize, bool virtualCodebook, float bitRate, int method, bool subsampling, bool useAveDelta, bool useVisualQuant)
{
   ...
   BRGBAImage lumaImage(image.getWidth(), image.getHeight());
   ...
   for (uint y = 0; y < image.getHeight(); y++)
   {
      for (uint x = 0; x < image.getWidth(); x++)
      {
         ...
         lumaImage(x, y).g = yCoCg.r;
      }
   }
   
   BRGBAImage subsampledLumaImage;
   BImageResampler resampler;
   resampler.resample(lumaImage, subsampledLumaImage, image.getWidth()/2, image.getHeight(), 4, false, false, "kaiser", .85f);
                        
   BDXTPacker dxtPacker;
      
   dxtImage.clear();
   dxtPacker.pack(subsampledLumaImage, dxtFormat, false, false, false, dxtImage);
   
   BRGBAImage unpackedLumaImage;
   BDXTUnpacker::unpack(unpackedLumaImage, &dxtImage[0], dxtFormat, subsampledLumaImage.getWidth(), subsampledLumaImage.getHeight());
   
   resampler.resample(unpackedLumaImage, lumaImage, image.getWidth(), image.getHeight(), 4, false, false, "tent", 1.0f);
