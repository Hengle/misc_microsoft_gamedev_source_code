...
// structs
//-----------------------------------------------------------------------------
typedef IndexType (*TRIANGLELIST)[3];
...
struct CStrip
{
    ...
    CStrip( DWORD num_tris, DWORD num_indices ) 
    {
        ...
        m_pIndices = new IndexType[ num_indices ];
    }
...
private:
    ...
    IndexType m_rgCache[CACHE_SIZE];      // vertex cache
...
public:
    ...
    int CreateManyStrips( CStripList *pstriplist, IndexType **ppstripindices );
    int CreateLongStrip( CStripList *pstriplist, IndexType **ppstripindices );
    int CreateCachedStrip( CStripList *pstriplist, IndexType **ppstripindices );
...
//-----------------------------------------------------------------------------
int CStripper::CreateIndices( BOOL bStartCW, int iNumTriangles, int* pTriangles, IndexType* pStripVerts )
{
    int stripcount = 0;
    BOOL bCW = bStartCW;
    int in_edge = -1;
                              ...
                              const CVertCache &VertexCache )
{
    ...
    if( bBestStripFlipped )
    {
        ...
        {
            // Swap vertex indices
            IndexType temp                 = pStrip->m_pIndices[first];
...
//-----------------------------------------------------------------------------
int CStripper::CreateManyStrips( CStripList* pStripList, IndexType** ppStripIndices )
{
    ...
    IndexType*      pStripIndices     = new IndexType[dwIndexCount];
    ...
    while( pStripList->m_dwNumStrips > 0 )
    {
        ...
        if( !pStrip->m_bIsStripCW )
        {
            ...
            pStripIndices[dwNumStripIndices++] = (IndexType)pStrip->m_dwNumIndices + 1;
            pStripIndices[dwNumStripIndices++] = pStrip->m_pIndices[0];
        }
        else
        {
            // add the strip length
            pStripIndices[dwNumStripIndices++] = (IndexType)pStrip->m_dwNumIndices;
        }
...
//-----------------------------------------------------------------------------
int CStripper::CreateLongStrip( CStripList* pStripList, IndexType** ppwStripIndices )
{
    ...
    while( pStripList->m_dwNumStrips )
    {
        ppStrip = FindBestStrip( pStripList, VertexCache );
        pStrip = *ppStrip;
        IndexType wLastVertex  = pStripIndices[dwNumStripIndices - 1];
        IndexType wFirstVertex = pStrip->m_pIndices[0];
...
// Desc: Merge striplist into one big uberlist with (hopefully) optimal caching
//-----------------------------------------------------------------------------
int CStripper::CreateCachedStrip( CStripList* pStripList, IndexType** ppwStripIndices )
{
    DWORD i;
    IndexType pTempVerts[CACHE_SIZE*4];
    ...
    while( pStripList->m_dwNumStrips )
    {
        ...
        do
        {
            ...
            memcpy( pNewList->m_pStrips[j]->m_pIndices, 
                    pTempVerts, num_indices * sizeof(IndexType) );
                             ...
                             BOOL bNonSequential, BOOL bSwapOrientation )
{
    ...
    IndexType pStripVerts[cNumTmpVerts + 1];
...
{
    IndexType* ptriverts = &m_pTriangles[tri + 1][0];
...
int CVertCache::Add( int strip, int vertindex )
{
    ...
    m_rgCache[m_iCachePtr]      = (IndexType)vertindex;
...
//-----------------------------------------------------------------------------
DWORD CountCacheMisses( DWORD dwIndexCount, IndexType *pStripIndices )
{
    CVertCache VertexCache;
...
//-----------------------------------------------------------------------------
DWORD TriStripToTriList( DWORD dwNumStripIndices, const IndexType *pStripIndices, 
                         IndexType *pTriangleIndices )
{
    DWORD dwNumTriangleIndices = 0;
        
    // Unstrip the indices.
    IndexType ind0 = 0;
    IndexType ind1 = pStripIndices[0];
    IndexType ind2 = pStripIndices[1];
...
//-----------------------------------------------------------------------------
DWORD Stripify( DWORD dwNumTriangles, IndexType* pTriangles, 
                DWORD* pdwNumIndices, IndexType** ppStripIndices, DWORD dwFlags )
{
    ...
    *ppStripIndices = 0;
    ...
    if( dwFlags & OUTPUT_TRILIST )
    {
        ...
        IndexType* pTempIndices = new IndexType[dwNumTriangles*3];
...
//-----------------------------------------------------------------------------
VOID ComputeVertexPermutation( DWORD dwNumStripIndices, IndexType* pStripIndices,
                               DWORD dwNumVertices, IndexType** ppVertexPermutation )
{
    ...
    (*ppVertexPermutation) = new IndexType[dwNumVertices];

    for( i = 0; i < dwNumVertices; i++ )
    {
        (*ppVertexPermutation)[i] = (IndexType)pSortTable[i].iOrigIndex;
    }

    // Build original to re-mapped permutation.
    IndexType* pInversePermutation = new IndexType[dwNumVertices];

    for( i = 0; i < dwNumVertices; i++ )
    {
        pInversePermutation[pSortTable[i].iOrigIndex] = (IndexType)i;
    }
