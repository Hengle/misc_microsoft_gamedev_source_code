...
FLOAT CD3DFont::GetRowHeight(void) const
{
...
void CD3DFont::DrawTextBegin(void)
{
	m_InsideTextDraw++;
	
	if (m_InsideTextDraw == 1)
	{
		// Setup renderstate
...
void CD3DFont::DrawTextEnd(void)
{
	m_InsideTextDraw--;

	if (m_InsideTextDraw == 0)
	{
		// Restore the modified renderstates
														...
														float xScale, float yScale)
{
    ...
    if( dwFlags & D3DFONT_CENTERED_X )
    {
        ...
        sx = (vp.Width - xFinal * xScale)/2.0f;
    }
    if( dwFlags & D3DFONT_CENTERED_Y )
    {
        D3DVIEWPORT9 vp;
        m_pd3dDevice->GetViewport( &vp );
        float fLineHeight = ((m_fTexCoords[0][3]-m_fTexCoords[0][1])*m_dwTexHeight);
        sy = (vp.Height - fLineHeight * yScale)/2;
    }

    // Adjust for character spacing
    sx -= m_dwSpacing * xScale;
    FLOAT fStartX = sx;
    ...
    while( *strText )
    {
        ...
        {
            sx = fStartX;
            sy += (m_fTexCoords[0][3]-m_fTexCoords[0][1])*m_dwTexHeight * yScale;
        }
        ...
        FLOAT w = xScale * ((tx2-tx1) *  m_dwTexWidth / m_fTextScale);
        FLOAT h = yScale * ((ty2-ty1) * m_dwTexHeight / m_fTextScale);
...
{
    if(( m_pd3dDevice == NULL ) || (0 != m_InsideTextDraw))
        return E_FAIL;
