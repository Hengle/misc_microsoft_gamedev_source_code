...
/*
		{
			for (float i = 0; i <= 1.0f; i += 1.0f/32.0f)
			{
				...
				Trace("%f %f %f\n", i, saturate(pow(saturate((i * a - b) * 4), 2.0f) * 8.0f), pow(i, 19.0f) );
			}
		}
*/
}
...
HRESULT CMyD3DApplication::OneTimeSceneInit()
{
    ...
    m_bLoadingApp = FALSE;
		...
		//gPolyhedron.createFromSphere(Vec3(0), 75.0f, 5, 5);
...
{
//  Trace("%f %f, %f %f %f\n", (float)m_UserInput.mouseX, (float)m_UserInput.mouseY, vb[0], vb[1], vb[2]);
  
  // because of the orbit cam flip rotation
  Vec3 b(vb);
  Vec3 a(va);
...
*/
  ...
  */
	...
	{
		static float oe = 1.035f;
		static float re = 1.035f;
		...
		cone.createCappedCone(
			...
			Math::fDegToRad(mSpotOuter * oe), mOmniRadius * re, 16);

		cone.clipAgainstPlane(gPolyhedron, Plane(Vec3(0,1,0), -.5f));

		Matrix44 worldToView = gShaderManager.getMatrix(eWorldToView);
...

	{
		...
		if (WasVKeyPressed('P'))
		{
			...
			srand(count);
			count++;
			p[0] = Vec3(Math::fRand(0, width - 1), Math::fRand(0, height - 1), Math::fRand(0, 255));
			p[1] = Vec3(Math::fRand(0, width - 1), Math::fRand(0, height - 1), Math::fRand(0, 255));
			p[2] = Vec3(Math::fRand(0, width - 1), Math::fRand(0, height - 1), Math::fRand(0, 255));
		}
		tilerizer.rasterizeTri(p);

		//Vec3 q[3];
		//q[0] = Vec3(0,0,255);
		//q[1] = Vec3(width-1,0,0);
		//q[2] = Vec3(width-1,height-1,0);
		//tilerizer.rasterizeTri(q);
		
		//q[0] = Vec3(0,0,0);
		//q[1] = Vec3(width-1,height-1,0);
		//q[2] = Vec3(0,height-1,0);
		//tilerizer.rasterizeTri(q);
		...
		const int c = -10;
		const int d = -9;
		Vec4 pos(150 + tilerizer.cellsX() * .5f * c, 200 + tilerizer.cellsY() * .5f * c, -300);
		...
		for (int y = 0; y < tilerizer.cellsY(); y++)
		{
			for (int x = 0; x < tilerizer.cellsX(); x++)
			{
				...
				float z = tilerizer.getCellLimits(tx, ty).mLowestZ;
				int iz = Math::Clamp(Math::FloatToIntTrunc(z + .5f), 0, 255);
		...
		for (int i = 0; i < 3; i++)
		{
			float cx = p[i][0] / tilerizer.cellWidth();
			float cy = p[i][1] / tilerizer.cellHeight();
...
*/
			...
			/*
			gLightingEngine.drawLight(
				LightParams(mOmniPos + Vec4(60,0,60), mOmniRadius, mOmniInner, mOmniOuter, mOmniPower, Vec4(.2, .2f, 1.0f, 1), Vec4(1, 1, 1, 1), false));

			gLightingEngine.drawLight(
				LightParams(mOmniPos + Vec4(-60,0,60), mOmniRadius, mOmniInner, mOmniOuter, mOmniPower, Vec4(.2, 1.0f, .2f, 1), Vec4(1, 1, 1, 1), false));
			*/
      ...
      m_pd3dDevice->EndScene();
			...
			/*
			float minTime = .033f;
			if (m_fElapsedTime < minTime)
			{
				Sleep(
					Math::FloatToIntTrunc(.5f + Math::Clamp<float>(1000.0f * (minTime - m_fElapsedTime), 1.0f, 16.0f))
					);
			}
			*/
    }
                                    ...
                                    LPARAM lParam )
{
    switch( msg )
    {
        case WM_KEYDOWN:
        {
          ...
          {
            case VK_LEFT: 
              m_UserInput.bLeft = true;
              break; 
            case VK_RIGHT: 
              m_UserInput.bRight = true;
              break; 
            case VK_UP: 
              m_UserInput.bUp = true;
              break;                      
            case VK_DOWN: 
              m_UserInput.bDown = true;
              break;                      
          }
