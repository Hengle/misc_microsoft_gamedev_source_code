...
void OBJExp::ExportGeomObject(INode* node, int indentLevel)
{
...
****************************************************************************/
/*
void OBJExp::ExportShapeObject(INode* node, int indentLevel)
{
...
/*
void OBJExp::DumpPoly(PolyLine* line, Matrix3 tm, int indentLevel)
{
...
****************************************************************************/
/*
void OBJExp::ExportLightObject(INode* node, int indentLevel)
{
	TimeValue t = GetStaticFrame();
	TSTR indent = GetIndent(indentLevel);
...
/*
void OBJExp::ExportLightSettings(LightState* ls, GenLight* light, TimeValue t, int indentLevel)
{
	TSTR indent = GetIndent(indentLevel);
...
****************************************************************************/
/*
void OBJExp::ExportCameraObject(INode* node, int indentLevel)
{
	TSTR indent = GetIndent(indentLevel);
...
void OBJExp::ExportCameraSettings(CameraState* cs, CameraObject* cam, TimeValue t, int indentLevel)
{
	TSTR indent = GetIndent(indentLevel);
...
****************************************************************************/
/*
void OBJExp::ExportHelperObject(INode* node, int indentLevel)
{
	TSTR indent = GetIndent(indentLevel);
...
void OBJExp::ExportNodeHeader(INode* node, TCHAR* type, int indentLevel)
{
	TSTR indent = GetIndent(indentLevel);
...
****************************************************************************/
/*
void OBJExp::ExportNodeTM(INode* node, int indentLevel)
{
...
void OBJExp::ExportAnimMesh(INode* node, int indentLevel)
{
...
void OBJExp::ExportMesh(INode* node, TimeValue t, int indentLevel)
{
	int i;
	...
	m_pObjFile->addObject(node->GetName());
   ...
   NmRawPoint pos1, pos2, pos3;
   NmRawPoint norm1, norm2, norm3;
   DWORD vIndex1, vIndex2, vIndex3;
   Point3 v1, v2, v3;
   Point3 vn1, vn2, vn3;
   ...
   int numFaces = mesh->getNumFaces();
	for (i=0; i<numFaces; i++) 
	{
		...
		v1 = tm * mesh->verts[vIndex1];
		v2 = tm * mesh->verts[vIndex2];
		v3 = tm * mesh->verts[vIndex3];

		vn1 = tm.VectorTransform(GetVertexNormal(mesh, i, mesh->getRVertPtr(vIndex1)));
		vn2 = tm.VectorTransform(GetVertexNormal(mesh, i, mesh->getRVertPtr(vIndex2)));
		vn3 = tm.VectorTransform(GetVertexNormal(mesh, i, mesh->getRVertPtr(vIndex3)));
      ...
      vn3 = vn3.Normalize();
		...
		pos1.x = v1.x; pos1.y = v1.y; pos1.z = v1.z;
		pos2.x = v2.x; pos2.y = v2.y; pos2.z = v2.z;
		pos3.x = v3.x; pos3.y = v3.y; pos3.z = v3.z;

		norm1.x = vn1.x; norm1.y = vn1.y; norm1.z = vn1.z;
		norm2.x = vn2.x; norm2.y = vn2.y; norm2.z = vn2.z;
		norm3.x = vn3.x; norm3.y = vn3.y; norm3.z = vn3.z;

		m_pObjFile->addTriangle(m_curObjectId, pos1, pos2, pos3, norm1, norm2, norm3);
      //m_pObjFile->addTriangle(m_curObjectId, pos1, pos2, pos3);
...
Point3 OBJExp::GetVertexNormal(Mesh* mesh, int faceNo, RVertex* rv)
{
...
****************************************************************************/
/*
void OBJExp::ExportIKJoints(INode* node, int indentLevel)
{
	Control* cont;
	TSTR indent = GetIndent(indentLevel);
...
void OBJExp::DumpJointParams(JointParams* joint, int indentLevel)
{
	TSTR indent = GetIndent(indentLevel);
...
****************************************************************************/
/*
void OBJExp::ExportMaterialList()
{
	if (!GetIncludeMtl()) {
		return;
	}
...
void OBJExp::ExportMaterial(INode* node, int indentLevel)
{
	Mtl* mtl = node->GetMtl();
...
void OBJExp::DumpMaterial(Mtl* mtl, int mtlID, int subNo, int indentLevel)
{
	int i;
	TimeValue t = GetStaticFrame();
...
TCHAR* OBJExp::GetMapID(Class_ID cid, int subNo)
{
	static TCHAR buf[50];
...
void OBJExp::DumpTexture(Texmap* tex, Class_ID cid, int subNo, float amt, int indentLevel)
{
	if (!tex) return;
...
void OBJExp::DumpUVGen(StdUVGen* uvGen, int indentLevel)
{
...
****************************************************************************/
/*
BOOL OBJExp::CheckForAndExportFaceMap(Mtl* mtl, Mesh* mesh, int indentLevel)
{
	if (!mtl || !mesh) {
		return FALSE;
	}
...
TSTR OBJExp::GetIndent(int indentLevel)
{
	TSTR indentString = "";
...
BOOL OBJExp::TMNegParity(Matrix3 &m)
{
...
TriObject* OBJExp::GetTriObjectFromNode(INode *node, TimeValue t, int &deleteIt)
{
	deleteIt = FALSE;
...
void OBJExp::DumpMatrix3(Matrix3* m, int indentLevel)
{
	Point3 row;
	TSTR indent = GetIndent(indentLevel);
...
void OBJExp::make_face_uv(Face *f, Point3 *tv)
{
	int na,nhid,i;
	Point3 *basetv;
...
TCHAR* OBJExp::FixupName(TCHAR* name)
{
...
// format consistent.
void OBJExp::CommaScan(TCHAR* buf)
{
    for(; *buf; buf++) if (*buf == ',') *buf = '.';
}

TSTR OBJExp::Format(int value)
{
	TCHAR buf[50];
...
TSTR OBJExp::Format(float value)
{
	TCHAR buf[40];
...
TSTR OBJExp::Format(Point3 value)
{
	TCHAR buf[120];
	TCHAR fmt[120];
...
TSTR OBJExp::Format(Color value)
{
	TCHAR buf[120];
	TCHAR fmt[120];
...
TSTR OBJExp::Format(AngAxis value)
{
	TCHAR buf[160];
	TCHAR fmt[160];
...
TSTR OBJExp::Format(ScaleValue value)
{
	TCHAR buf[280];
