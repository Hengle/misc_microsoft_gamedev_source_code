...
BEGIN_MESSAGE_MAP(CDxtexView, CScrollView)
    ...
    ON_COMMAND(ID_VIEW_ALPHABLENDING, OnViewAlphaBlending)
    ...
    ON_UPDATE_COMMAND_UI(ID_VIEW_ALPHABLENDING, OnUpdateViewAlphaBlending)
...
HRESULT CDxtexView::RenderScene(VOID)
{
    ...
    hr = pd3ddev->SetRenderState(D3DRS_ALPHABLENDENABLE, m_bViewAlphaBlending ? TRUE : FALSE);
...
void CDxtexView::OnViewAlphaBlending(VOID)
{
   m_bViewAlphaBlending = !m_bViewAlphaBlending;
   RenderScene();
   Invalidate(); // force redraw of this view
   m_bTitleModsChanged = TRUE; // force title bar update
}
...
void CDxtexView::OnUpdateViewAlphaBlending(CCmdUI* pCmdUI)
{
   pCmdUI->SetCheck(m_bViewAlphaBlending);
}
