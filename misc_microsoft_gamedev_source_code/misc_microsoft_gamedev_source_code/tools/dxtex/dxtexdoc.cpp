...
bool BDDXDLLHelper::init(void)
{
   ...
   char newPath[256];
   char dir[256];
   _splitpath(path, newPath, dir, NULL, NULL);

   strcat(newPath, dir);      
   strcat(newPath, DDX_DLL_FILENAME);

   mDLLHandle=LoadLibrary(newPath);
...
BOOL CDxtexDoc::LoadDDXFile(LPCTSTR lpszPathName, bool loadDDT)
{
   std::vector<BYTE> ddxData;
   ...
   ddxData.resize(ftell(pFile));
   fseek(pFile, 0, SEEK_SET);   
   
   if (fread(&ddxData[0], ddxData.size(), 1, pFile) != 1)
   {
      fclose(pFile);
   ...
   IDDXBuffer* pNativeBuf = NULL;
   BDDXTextureInfo nativeTextureInfo;
   
   bool status;
   if (loadDDT)
      status = DDXHelper.getInterface()->DDTtoRGBA8(&pNativeBuf, nativeTextureInfo, false, &ddxData[0], ddxData.size());
   else
      status = DDXHelper.getInterface()->unpackDDX(&pNativeBuf, nativeTextureInfo, &ddxData[0], ddxData.size(), true, false);
   ...
   IDDXBuffer* pBuf = NULL;
   BDDXTextureInfo textureInfo;
   if (!DDXHelper.getInterface()->unpackTexture(&pBuf, pNativeBuf->getPtr(), pNativeBuf->getSize(), nativeTextureInfo, textureInfo, true, false))
   {
      if (pNativeBuf)
         pNativeBuf->release();
   ...
   BDDXSerializedTexture texture(pBuf->getPtr(), pBuf->getSize(), textureInfo);
   
   const bool cubemap = (textureInfo.mResourceType == cDDXResourceTypeCubeMap);

   const UINT mipLevels = textureInfo.mNumMipChainLevels + 1;

   IDirect3DCubeTexture9* pCubeTex = NULL;
   IDirect3DTexture9* pTex = NULL;
   
   D3DFORMAT fmt = textureInfo.mHasAlpha ? D3DFMT_A8B8G8R8 : D3DFMT_X8B8G8R8;
   
   UINT bytesPerPixel = sizeof(DWORD);
   
   if (textureInfo.mDataFormat == cDDXDataFormatA16B16G16R16F)
   {
      fmt = D3DFMT_A16B16G16R16F;
      bytesPerPixel = sizeof(__int64);
   }
         
   HRESULT hres;
   if (cubemap)
   {
      hres = pd3ddev->CreateCubeTexture(textureInfo.mWidth, mipLevels, 0, fmt, D3DPOOL_SYSTEMMEM, &pCubeTex, 0);
      m_ptexOrig = pCubeTex;
   }
   else   
   {
      hres = pd3ddev->CreateTexture(textureInfo.mWidth, textureInfo.mHeight, mipLevels, 0, fmt, D3DPOOL_SYSTEMMEM, &pTex, 0);
      m_ptexOrig = pTex;
   }
   ...
   for (UINT faceIndex = 0; faceIndex < numFaces; faceIndex++)
   {
      for (UINT levelIndex = 0; levelIndex < mipLevels; levelIndex++)
      {
         ...
         const BYTE* pLevelData = texture.getSurfaceData(faceIndex, levelIndex);
         ...
         for (UINT y = 0; y < levelHeight; y++)
            memcpy(reinterpret_cast<BYTE*>(lockedRect.pBits) + y * lockedRect.Pitch, pLevelData + y * levelWidth * bytesPerPixel, levelWidth * bytesPerPixel);   
   ...
   m_dwWidth = textureInfo.mWidth;
   m_dwHeight = textureInfo.mHeight;
   m_dwDepth = 0;
   m_numMips = mipLevels;
   m_dwCubeMapFlags = (textureInfo.mResourceType == cDDXResourceTypeCubeMap) ? DDS_CUBEMAP_ALLFACES : 0;
      
   return TRUE;
}

BOOL CDxtexDoc::SaveDDXFile(LPCTSTR lpszPathName, LPDIRECT3DBASETEXTURE9 pTex)
{
   HRESULT hr;
   LPDIRECT3DDEVICE9 pd3ddev = PDxtexApp()->Pd3ddev();
   LPDIRECT3DTEXTURE9 pmiptex;
   LPDIRECT3DCUBETEXTURE9 pcubetex;
   D3DFORMAT fmtFrom;
   const D3DFORMAT fmtTo = D3DFMT_A8B8G8R8;
   LPDIRECT3DBASETEXTURE9 ptexNew;
   LPDIRECT3DTEXTURE9 pmiptexNew;
   LPDIRECT3DCUBETEXTURE9 pcubetexNew;
   
   LPDIRECT3DBASETEXTURE9 ptexCur = m_ptexOrig;
   ...
   if (IsCubeMap())
   {
      ...
      if (FAILED(hr))
         return hr;
      ptexNew = pcubetexNew;
      if (FAILED(hr = BltAllLevels(D3DCUBEMAP_FACE_NEGATIVE_X, ptexCur, ptexNew)))
         return hr;
      if (FAILED(hr = BltAllLevels(D3DCUBEMAP_FACE_POSITIVE_X, ptexCur, ptexNew)))
         return hr;
      if (FAILED(hr = BltAllLevels(D3DCUBEMAP_FACE_NEGATIVE_Y, ptexCur, ptexNew)))
         return hr;
      if (FAILED(hr = BltAllLevels(D3DCUBEMAP_FACE_POSITIVE_Y, ptexCur, ptexNew)))
         return hr;
      if (FAILED(hr = BltAllLevels(D3DCUBEMAP_FACE_NEGATIVE_Z, ptexCur, ptexNew)))
         return hr;
      if (FAILED(hr = BltAllLevels(D3DCUBEMAP_FACE_POSITIVE_Z, ptexCur, ptexNew)))
         return hr;
   }
   else
   {
      ...
      if (FAILED(hr))
         return hr;
      ptexNew = pmiptexNew;
      if (FAILED(BltAllLevels(D3DCUBEMAP_FACE_FORCE_DWORD, ptexCur, ptexNew)))
         return hr;
   }
   
   BOOL hasAlpha = FormatContainsAlpha(fmtFrom);
   
   D3DSURFACE_DESC desc;

   if (IsCubeMap())
      hr = pcubetexNew->GetLevelDesc(0, &desc);
   else
      hr = pmiptexNew->GetLevelDesc(0, &desc);
   ...
   textureInfo.mResourceType = IsCubeMap() ? cDDXResourceTypeCubeMap : cDDXResourceTypeRegularMap;

   if (ptexNew->GetLevelCount() > 1)
   {
      textureInfo.mNumMipChainLevels = ptexNew->GetLevelCount() - 1;
   }

   bool found1BitAlpha = false;

   const UINT numFaces = IsCubeMap() ? 6 : 1;
   
   std::vector<BYTE> textureData;

   for (UINT faceIndex = 0; faceIndex < numFaces; faceIndex++)
   {
      for (UINT levelIndex = 0; levelIndex < ptexNew->GetLevelCount(); levelIndex++)
      {      
         D3DLOCKED_RECT lockedRect;

         if (IsCubeMap())
            hr = pcubetexNew->LockRect((D3DCUBEMAP_FACES)faceIndex, levelIndex, &lockedRect, NULL, 0);
         else
            hr = pmiptexNew->LockRect(levelIndex, &lockedRect, NULL, 0);
         ...
         const UINT mipWidth = max(1U, textureInfo.mWidth >> levelIndex);
         const UINT mipHeight = max(1U, textureInfo.mHeight >> levelIndex);

         const UINT dataSize = mipWidth * mipHeight * sizeof(DWORD);
         ...
         {
            const BYTE* pSrc = reinterpret_cast<BYTE*>(lockedRect.pBits) + lockedRect.Pitch * y;
            BYTE* pDst = &textureData[textureDataOfs] + mipWidth * y * sizeof(DWORD);
         ...
         if (IsCubeMap())
            pcubetexNew->UnlockRect((D3DCUBEMAP_FACES)faceIndex, levelIndex);
         else
            pmiptexNew->UnlockRect(levelIndex);
      }      
   }      
   
   if ((found1BitAlpha) && (fmtFrom == D3DFMT_DXT1))
      hasAlpha = true;
   ...
   packParams.mMipChainLevels = ptexNew->GetLevelCount() - 1;
   packParams.mDataFormat = cDDXDataFormatA8B8G8R8;
   packParams.mResourceType = IsCubeMap() ? cDDXResourceTypeCubeMap : cDDXResourceTypeRegularMap;
   
   textureInfo.mHasAlpha = (0 != hasAlpha);

   IDDXBuffer* pBuf = NULL;
   BDDXTextureInfo outTextureInfo;
   if (!DDXHelper.getInterface()->packTexture(&pBuf, &textureData[0], textureData.size(), textureInfo, packParams, outTextureInfo))
   {
      ReleasePpo(&ptexNew);
   ...
   IDDXBuffer* pDDXBuf = NULL;
   if (!DDXHelper.getInterface()->packDDX(&pDDXBuf, pBuf->getPtr(), pBuf->getSize(), outTextureInfo, cDDXPlatformXbox))
   {
      ReleasePpo(&ptexNew);
   ...
   FILE* pFile = fopen(lpszPathName, "wb");
   if (!pFile)
   {
      if (pDDXBuf)
      {
   ...
   const int numWritten = fwrite(pDDXBuf->getPtr(), pDDXBuf->getSize(), 1, pFile);
   ...
   if ((EOF == fclose(pFile)) || (numWritten != 1))
   {     
      AfxMessageBox(ID_ERROR_COULDNTSAVEFILE);
      return FALSE;
   }
...
BOOL CDxtexDoc::OnOpenDocument(LPCTSTR lpszPathName)
{
    ...
    if (stricmp(extension, ".ddx") == 0)
        return LoadDDXFile(lpszPathName, false);
    else if (stricmp(extension, ".ddt") == 0)
      return LoadDDXFile(lpszPathName, true);
...
BOOL CDxtexDoc::OnSaveDocument(LPCTSTR lpszPathName)
{
    ...
    _splitpath(lpszPathName, NULL, NULL, NULL, extension);    
    if (stricmp(extension, ".ddx") == 0)
       return SaveDDXFile(lpszPathName, ptex);
...
void CDxtexDoc::SetPathName(LPCTSTR lpszPathName, BOOL bAddToMRU) 
{
    ...
    const TCHAR* pszLeaf = strrchr(lpszPathName, '\\');
    if( pszLeaf )
    {
        pszLeaf++;
        TCHAR* pszExtension = (TCHAR*)strrchr(lpszPathName, '.');
        if( pszExtension && 
            CompareString( lcid, NORM_IGNORECASE, pszExtension, -1, TEXT(".dds"), -1 ) != CSTR_EQUAL )
        {
            StringCchCopy(pszExtension, 1, "");
            // rg - Let's not do this.
