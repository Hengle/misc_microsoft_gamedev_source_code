...
int cast_to_int(Resample_Real i)
{
  return (int)i;
}
...
Resample_Real box_filter(Resample_Real t)    /* pulse/Fourier window */
{
...
Resample_Real tent_filter(Resample_Real t)   /* box (*) box, bilinear/triangle */
{
  if (t < 0.0f)
    t = -t;
...
Resample_Real bell_filter(Resample_Real t)    /* box (*) box (*) box */
{
  if (t < 0.0f)
    t = -t;
...
Resample_Real B_spline_filter(Resample_Real t)  /* box (*) box (*) box (*) box */
{
  Resample_Real tt;
...
// See Dodgson, N., "Quadratic Interpolation for Image Resampling" 
#define QUADRATIC_SUPPORT 1.5f
Resample_Real quadratic(Resample_Real t, const Resample_Real R)
{
	if (t < 0.0f)
		t = -t;	
...
Resample_Real quadratic_interp_filter(Resample_Real t)
{
	return quadratic(t, 1.0f);
}

Resample_Real quadratic_approx_filter(Resample_Real t)
{
	return quadratic(t, .5f);
}

Resample_Real quadratic_mix_filter(Resample_Real t)
{
	return quadratic(t, .8f);
}

// See Mitchell, D. and A. Netravali, "Reconstruction Filters in Computer Graphics." 
...
Resample_Real mitchell(Resample_Real t, const Resample_Real B, const Resample_Real C)
{
  Resample_Real tt;
...
Resample_Real mitchell_filter(Resample_Real t)
{
	return mitchell(t, 1.0f / 3.0f, 1.0f / 3.0f);
}

#define CATMULL_ROM_SUPPORT (2.0f)
Resample_Real catmull_rom_filter(Resample_Real t)
{
...
Resample_Real blackman_filter(Resample_Real t)
{
  if (t < 0.0f)
    t = -t;
...
Resample_Real gaussian_filter(Resample_Real t) // with blackman window
{
	if (t < 0)
		t = -t;
...
Resample_Real lanczos3_filter(Resample_Real t)
{
  if (t < 0.0f)
    t = -t;
...
Resample_Real lanczos4_filter(Resample_Real t)
{
  if (t < 0.0f)
    t = -t;
...
Resample_Real lanczos6_filter(Resample_Real t)
{
  if (t < 0.0f)
    t = -t;
...
Resample_Real lanczos12_filter(Resample_Real t)
{
  if (t < 0.0f)
    t = -t;
...
Resample_Real kaiser_filter(Resample_Real t)
{
	if (t < 0.0f)
		t = -t;
...
const int NUM_FILTERS = sizeof(Afilters) / sizeof(Afilters[0]);
#define DEFAULT_FILTER "blackman"

} // end namespace Resampler

using namespace Resampler_Namespace;
...
//--
  ...
  for (i = 0; i < Pclist->n; i++)
  {
    ...
    if (--m_Psrc_y_count[debugRangeCheck(Pclist->p[i].pixel, m_resample_src_y)] == 0)
    {
      m_Psrc_y_flag[debugRangeCheck(Pclist->p[i].pixel, m_resample_src_y)] = FALSE;
...
bool Resampler::put_line(const Sample* Psrc)
{
  ...
  if (!m_Psrc_y_count[debugRangeCheck(m_cur_src_y, m_resample_src_y)])
  {
    m_cur_src_y++;
    return false;
  }
  ...
  m_Psrc_y_flag[debugRangeCheck(m_cur_src_y, m_resample_src_y)] = TRUE;
...
const Resampler::Sample* Resampler::get_line(void)
{
  int i;
  ...
  for (i = 0; i < m_Pclist_y[m_cur_dst_y].n; i++)
    if (!m_Psrc_y_flag[debugRangeCheck(m_Pclist_y[m_cur_dst_y].p[i].pixel, m_resample_src_y)])
      return NULL;
...
Resampler::~Resampler()
{
  ...
  free(m_Pdst_buf); m_Pdst_buf = NULL;
...
void Resampler::restart(void)
{
   ...
   for (i = 0; i < m_resample_dst_y; i++)
   {
      for (j = 0; j < m_Pclist_y[i].n; j++)
         m_Psrc_y_count[debugRangeCheck(m_Pclist_y[i].p[j].pixel, m_resample_src_y)]++;
   }
	...
	Resample_Real src_y_ofs)
{
  ...
  if (Pfilter_name == NULL)
    Pfilter_name = DEFAULT_FILTER;

  for (i = 0; i < NUM_FILTERS; i++)
    if (strcmp(Pfilter_name, Afilters[i].name) == 0)
      break;
  ...
  func = Afilters[i].func;
  support = Afilters[i].support;
...
int Resampler::get_filter_num(void)
{
  return NUM_FILTERS;
}
...
char* Resampler::get_filter_name(int filter_num)
{
  if ((filter_num < 0) || (filter_num >= NUM_FILTERS))
    ...
    return Afilters[filter_num].name;
}
