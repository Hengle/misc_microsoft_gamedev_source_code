...
{
        /// Infinte as far as the memory systm is concerned.
...
class hkMemory
{
	public:
		...
		virtual void reservePagesFromSystemMemory( int numBytes ) {}
            ...
            /// Concretely in many implementions, lists of small sized blocks are kept, so they can be made available
        ...
        virtual void calculateStatistics(hkMemoryStatistics& stats ) =0;
            ...
            /// may or may not be honoured. Look at the docs on specific implementations to see if memory limiting
            /// is supported. If limiting is not supported the method will have no effect. If the memory requsted was
        ...
        virtual hkLimitedMemoryListener* getLimitedMemoryListener() { return HK_NULL; }
            ...
            /// memory, and because the implemention can decide broadly what 'enough memory' means - as it has internal
