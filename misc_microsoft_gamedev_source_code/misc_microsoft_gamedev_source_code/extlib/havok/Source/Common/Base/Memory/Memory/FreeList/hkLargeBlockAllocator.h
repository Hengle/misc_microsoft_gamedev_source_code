...
class hkLargeBlockAllocator
{
    ...
    hkBool isValidAlloc(void* alloc);

        /// Returns the total amount of memory _potentially_ available in an allocated block
        /// NOTE in general this size will be different from the requested size - it will be bigger
        /// You should not assume you can use the extra memory, as in a debug build markers may be added to
        /// the end of your allocation to check for overruns
        /// To access the extra space, you can use resize which is guarenteed to work if size is in range of size specified
    hk_size_t getAllocSize(void* p);

        /// Returns the total amount of bytes taken up by an allocation
    hk_size_t getAllocTotalSize(void* p);
    ...
    void* criticalAlloc(hk_size_t size);
        /// To free a critical memory allocation
    void criticalFree(void* ptr,hk_size_t size);
