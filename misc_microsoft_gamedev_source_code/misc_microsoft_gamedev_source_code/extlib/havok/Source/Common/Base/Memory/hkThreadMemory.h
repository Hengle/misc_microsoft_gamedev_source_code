...
class hkThreadMemory
{
	public:
		...
		HK_DECLARE_SYSTEM_ALLOCATOR();
			...
			///								  memory manager.
			///	If the underlying memory manager returns true in the funtion isDebugMemory() maxNumElemsOnFreeList will be forced to 0
	...
	public:
		...
		{
			enum { MAX_FREELIST=8 };
			FreeList() : m_numElem(0) {}
			void put( void* p )
			{
				if( m_numElem == MAX_FREELIST ) HK_BREAKPOINT(0);
				m_elems[ m_numElem ] = p;
				m_numElem = m_numElem + 1;
			}
			void* get()
			{
				if( m_numElem )
				{
					m_numElem = m_numElem - 1;
					return m_elems[m_numElem];
				}
				return HK_NULL;
			}

			void* m_elems[MAX_FREELIST];
			hkPadSpu<int> m_numElem;
		};
#endif
