...
HK_FORCE_INLINE hkString hkString::substr(int index, int numChars) const
{
	if(numChars > getLength() - index)
	{
		numChars = getLength() - index;
	}
	return hkString(m_string.begin()+index, numChars);
}
...
void hkString::memCpy128(void* dst, const void* src)
{
#if defined(HK_PLATFORM_PS3SPU) || defined(HK_PLATFORM_PS3)
	const vector signed int* srcQuad = reinterpret_cast<const vector signed int*>(src);
	vector signed int*       dstQuad = reinterpret_cast<vector signed int*>(dst);
	...
	dstQuad[7] = d;
#else
	hkString::memCpy16NonEmpty(dst, src, 8);
#endif
}
...
{
#if defined(HK_PLATFORM_PS3SPU) || defined(HK_PLATFORM_PS3)
	const vector signed int* srcQuad = (const vector signed int*)src;
	vector signed int*       dstQuad = (vector signed int*)      dst;
