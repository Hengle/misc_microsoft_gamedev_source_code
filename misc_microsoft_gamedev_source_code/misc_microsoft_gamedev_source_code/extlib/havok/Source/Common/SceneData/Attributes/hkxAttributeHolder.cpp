...
const hkVariant* hkxAttributeHolder::findAttributeVariantByName( const char* name ) const
{
	if( !name )
	{
		return HK_NULL;
...
void* hkxAttributeHolder::findAttributeObjectByName( const char* name, const hkClass* type ) const
{
	if( !name )
	{
		return HK_NULL;
