...
struct hkpContactPointAddedEvent
{
	...
	HK_PAD_ON_SPU(hkpContactPointProperties*)  m_contactPointProperties; 
		...
		/// Gets contact point properties. This method can be only used for non-Toi contact points.
	inline hkpContactPointProperties* getContactPointProperties();

#if ! defined (HK_PLATFORM_SPU)
		/// gets the body index to be used by getNumExtendedUserDatas and getExtendedUserData
	inline int getExtendedUserDataBodyIndex( const hkpEntity* entity );


		/// Number of extended user datas in contact point properties. 
		/// This function can only be used for non-Toi contact points.
	inline int getNumExtendedUserDatas(int bodyIdx) const;

		/// Gets extended user data from contact point properties. 
		/// This function can only be used for non-Toi contact points.
	inline hkUint32& getExtendedUserData(int bodyIdx, int i);
#endif
...
#if !defined(HK_PLATFORM_SPU)	// only available on ppu, use getContactPointProperty for spus
