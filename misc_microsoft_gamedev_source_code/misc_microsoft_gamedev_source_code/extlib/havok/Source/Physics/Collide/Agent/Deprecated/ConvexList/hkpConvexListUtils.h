...
struct hkpProcessCollisionOutputBackup
{
	...
	inline void rollbackOutput( const hkpCdBody& bodyA,	const hkpCdBody& bodyB, hkpProcessCollisionOutput& output, hkpContactMgr* mgr )
	{
		...
		{
			mgr->removeToi( *bodyA.getRootCollidable(), *bodyB.getRootCollidable(), *output.m_constraintOwner.val(), output.m_toi.m_properties );
			output.m_toi = m_toi;
		}
...
public:
	...
	{
		m_contactMgr->removeToi( a, b, constraintOwner, properties );
	}
