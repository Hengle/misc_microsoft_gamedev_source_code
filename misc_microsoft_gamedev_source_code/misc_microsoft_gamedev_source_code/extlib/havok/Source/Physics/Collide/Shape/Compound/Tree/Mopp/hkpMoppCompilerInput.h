...
class hkpMoppCompilerInput
{
	...
	public:
		...
		hkBool  m_enablePrimitiveSplitting;
			...
			///	This tells the mopp compiler to organize the mopp code into smaller chunks so it can be processed on the ps3 spu
		...
		hkBool m_cachePrimitiveExtents;
			...
			/// This instructs the mopp assembled to compress and embed terminal (triangles) directly into the Mopp code
			/// Note: this can only be used in conjunction with chunk subdivision
		hkBool m_embedTerminals;

			/// This specifies the material indices to embed in the MOPP when building in chunks with embedded shapes.
			/// Set to HK_NULL if no materials are required.
		hkUint8* m_embeddedMaterialIndices;
};
