...
/// PS3 note: this header will be dma'd back and forth between PPU and SPU and therefore must not be allocated on PPU stack.
...
// Important: the 16bit m_jobType HAS to be the first member of this class and it HAS to be 16byte aligned! See hkJobHelperBaseClass for more details.
//
struct hkpCollisionQueryJob
{
	public:
		...
		enum JobType
		{
			COLLISION_QUERY_SHAPE_RAYCAST = hkJobHelperBaseClass::HK_COLLISION_QUERY_JOB_BASE,
		...
		typedef hkEnum<JobType,hkObjectIndex> hkpCollisionQueryJobType;

	protected:

		HK_FORCE_INLINE hkpCollisionQueryJob( JobType type );

	public:

			// m_jobType HAS to be the first member of this class and it HAS to be 16byte aligned!
		HK_ALIGN16( hkpCollisionQueryJobType	m_jobType );
...
struct hkpShapeRayCastCommand
{
	...
	public:
		...
		hkpShapeRayCastInput		m_rayInput;
			...
			/// PS3 note: this array will be dma'd to SPU and therefore must not be allocated on PPU stack.
		...
		// ===================================================================
			...
			/// PS3 note: this array will be dma'd from SPU and therefore must not be allocated on PPU stack.
...
struct hkpShapeRayCastJob : public hkpCollisionQueryJob
{
	...
	public:
			...
			/// PS3 note: the jobHeader and the commandArray will be dma'd to SPU and therefore must not be allocated on PPU stack.
		HK_FORCE_INLINE hkpShapeRayCastJob(hkpCollisionQueryJobHeader* jobHeader, const hkpShapeRayCastCommand* commandArray, int numCommands, hkSemaphoreBusyWait* semaphore, int numCommandsPerTask = MAXIMUM_NUMBER_OF_COMMANDS_PER_TASK);
		...
		/// Note that you usually only have to call this function on PS3 if you intend the job to go onto the SPU. It's safe to call it on
...
struct hkpWorldRayCastJob : public hkpCollisionQueryJob
{
	...
	public:
			...
			/// The number of commands that are grouped into one task is customizable.
			/// PS3 note: the jobHeader and the commandArray will be dma'd to SPU and therefore must not be allocated on PPU stack.
		HK_FORCE_INLINE hkpWorldRayCastJob(hkpCollisionQueryJobHeader* jobHeader, const hkpWorldRayCastCommand* commandArray, int numCommands, const hkpBroadPhase* broadphase, hkSemaphoreBusyWait* semaphore, int numCommandsPerTask = MAXIMUM_NUMBER_OF_COMMANDS_PER_TASK);
...
struct hkpPairLinearCastJob : public hkpCollisionQueryJob
{
	public:
		...
		enum { MAXIMUM_NUMBER_OF_COMMANDS_PER_TASK = 240 };
	...
	public:
			...
			/// The number of commands that are grouped into one task is customizable.
			/// PS3 note: the jobHeader and the commandArray will be dma'd to SPU and therefore must not be allocated on PPU stack.
		HK_FORCE_INLINE hkpPairLinearCastJob(hkpCollisionQueryJobHeader* jobHeader, const hkpPairLinearCastCommand* commandArray, int numCommands, const hkpShapeCollectionFilter* filter, hkReal tolerance, hkSemaphoreBusyWait* semaphore, int numCommandsPerTask = MAXIMUM_NUMBER_OF_COMMANDS_PER_TASK);
...
struct hkpWorldLinearCastJob : public hkpCollisionQueryJob
{
	...
	public:
			...
			/// The number of commands that are grouped into one task is customizable.
			/// PS3 note: the jobHeader and the commandArray will be dma'd to SPU and therefore must not be allocated on PPU stack.
		HK_FORCE_INLINE hkpWorldLinearCastJob(hkpCollisionQueryJobHeader* jobHeader, const hkpWorldLinearCastCommand* commandArray, int numCommands, const hkpBroadPhase* broadphase, hkSemaphoreBusyWait* semaphore, int numCommandsPerTask = MAXIMUM_NUMBER_OF_COMMANDS_PER_TASK);
...
struct hkpMoppAabbJob : public hkpCollisionQueryJob
{
	...
	public:
			...
			/// The number of commands that are grouped into one task is customizable.
			/// PS3 note: the jobHeader and the commandArray will be dma'd to SPU and therefore must not be allocated on PPU stack.
		HK_FORCE_INLINE hkpMoppAabbJob(hkpCollisionQueryJobHeader* jobHeader, const hkpMoppAabbCommand* commandArray, int numCommands, const hkUint8* moppCodeData, const hkpMoppCode::CodeInfo& moppCodeInfo, hkSemaphoreBusyWait* semaphore, int numCommandsPerTask = MAXIMUM_NUMBER_OF_COMMANDS_PER_TASK);
...
struct hkpPairGetClosestPointsJob : public hkpCollisionQueryJob
{
	...
	public:
			...
			/// The number of commands that are grouped into one task is customizable.
			/// PS3 note: the jobHeader and the commandArray will be dma'd to SPU and therefore must not be allocated on PPU stack.
		HK_FORCE_INLINE hkpPairGetClosestPointsJob(hkpCollisionQueryJobHeader* jobHeader, const hkpPairGetClosestPointsCommand* commandArray, int numCommands, hkReal tolerance, hkSemaphoreBusyWait* semaphore, int numCommandsPerTask = MAXIMUM_NUMBER_OF_COMMANDS_PER_TASK);
...
/// This job is unable to split itself but is able to spawn hkPairGetClosestPointsJobs which can be processed multithreaded.
struct hkpWorldGetClosestPointsJob : public hkpCollisionQueryJob
{
	...
	public:
			...
			/// For performance reasons you should pass in a large enough pairGetClosestPointsCommandBuffer: for each command's querying collidable and each other object
			/// found by the broadphase an hkpPairGetClosestPointsCommand is generated. Note that once the pairGetClosestPointsCommandBuffer overflows
			/// we need to revert to serialized 'linear' searches (as opposed to parallel searches) which will yield a huge performance decrease.
			/// The supplied broadphase is used to limit the number of possible object pairs and thus helps increasing performance.
			/// Only closest points for object pairs with a distance of the supplied tolerance (or smaller) are reported.
			/// The supplied semaphore is released once all commands of this job have been completed and the job has been removed from the job queue.
			/// PS3 note: the jobHeader and the commandArray will be dma'd to SPU and therefore must not be allocated on PPU stack.
			/// PS3 note: the pairGetClosestPointsCommandBuffer will be dma'd from SPU and therefore must not be allocated on PPU stack.
		HK_FORCE_INLINE hkpWorldGetClosestPointsJob(hkpCollisionQueryJobHeader* jobHeader, const hkpWorldGetClosestPointsCommand* commandArray, int numCommands, hkpPairGetClosestPointsCommand* pairGetClosestPointsCommandBuffer, int pairGetClosestPointsCommandBufferCapacity, const hkpBroadPhase* broadphase, hkReal tolerance, hkSemaphoreBusyWait* semaphore);
