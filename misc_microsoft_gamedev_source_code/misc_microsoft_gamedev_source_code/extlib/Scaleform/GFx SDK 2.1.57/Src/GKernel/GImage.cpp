...
// ***** GImageBase
GImageBase::GImageBase(const GImageBase &src)
{
    // Copy data
...
GImage* GImage::ReadDDSFromMemory(const UByte* ddsData, UPInt dataSize)
{
    GImage *     pimage = 0;
    const UByte* pdata;
...
{
    size_t mlen = gfc_strlen(msg);

    GFxPngContext *pcontext = (GFxPngContext*)png_get_error_ptr(png_ptr);
    if (mlen < sizeof(pcontext->errorMessage))
        gfc_strcpy(pcontext->errorMessage, sizeof(pcontext->errorMessage), msg);
    else
    {
        gfc_strncpy(pcontext->errorMessage, sizeof(pcontext->errorMessage), msg, sizeof(pcontext->errorMessage) - 1);
