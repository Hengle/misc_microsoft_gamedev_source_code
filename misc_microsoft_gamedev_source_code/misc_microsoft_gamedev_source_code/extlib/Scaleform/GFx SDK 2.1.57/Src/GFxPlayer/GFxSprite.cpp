...
GFxASCharacter* GFxSprite::GetTopMostMouseEntity(const GPointF &pt, bool testAll, const GFxASCharacter* ignoreMC)
{
    ...
    GPointF             localPt = m.TransformByInverse(pt);   
    ...
    if (pmask)
    {
        if (pmask->IsUsedAsMask() && !pmask->IsUnloaded())
        {
            ...
            GPointF pp = matrix.Transform(localPt);
    ...
    for (i = n - 1; i >= 0; i--)
    {
        ...
        if (ch != NULL)
        {           
            GFxASCharacter* te = ch->GetTopMostMouseEntity(localPt, testAll, ignoreMC);
...
GFxPlaceObject2* GFxSprite::FindPreviousPlaceObjectInfo(GFxCharPosInfo *ppos, UInt *pframe, int depth, GFxResourceId id, UInt findPrevFlags)
{
    ...
    if (!(findPrevFlags & FindPrev_AllowMove))
    {
        ...
        while(ptag->PlaceType == GFxPlaceObject2::Place_Move)
        {
            ...
            if (!ppos->TextFilter && ptag->Pos.TextFilter)
            {
                ppos->TextFilter = ptag->Pos.TextFilter;
            }
    ...
    *pframe = currentFrame;
    ...
    if (ptag->PlaceType == GFxPlaceObject2::Place_Move)
    {
        if ( (ppos->HasCxform  || !(findPrevFlags & FindPrev_NeedCXForm)) && 
             ...
             (ppos->TextFilter || !(findPrevFlags & FindPrev_NeedFilters)) )
        {           
            return ptag;
        }   
    }
    ...
    while ((pprevTag->PlaceType != GFxPlaceObject2::Place_Add) &&
           (!ppos->HasMatrix || !ppos->HasCxform || !ppos->BlendMode || !ppos->TextFilter))
    {
        ...
        if (!ppos->TextFilter && pprevTag->Pos.TextFilter)
        {
            ppos->TextFilter = pprevTag->Pos.TextFilter;
        }
    }
                                ...
                                GFxASCharacter* origChar)
{
    ...
    if (pscriptCh)
    {
        ...
        if (psymbolName)
        {
            ...
            else // not a placeObject (means - attachMovie) and no registered class
            {
                // Queue up copying init properties if they exist (for SWF 6+).
                if (GetVersion() >= 6 && pinitSource && pscriptCh)
                {
...
void    GFxSprite::CallFrameActions(UInt frameNumber)
{
    ...
    {
        // No dice.
        LogError("Error: CallFrame('%d') - unknown frame\n", frameNumber);
        return;
    }
