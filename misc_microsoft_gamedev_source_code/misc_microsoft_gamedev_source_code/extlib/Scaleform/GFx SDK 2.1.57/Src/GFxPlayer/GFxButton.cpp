...
public:
    ...
    void OnGettingKeyboardFocus()
    {
        ...
        if (proot && !proot->DisableFocusRolloverEvent.IsTrue())
    ...
    {
        GFxMovieRoot* proot = GetMovieRoot();
        if (proot->IsFocusRectShown() && !proot->DisableFocusRolloverEvent.IsTrue())
    ...
    virtual bool    OnButtonEvent(const GFxEventId& event)
    {
        ...
        GFxMouseState prevMouseState = MouseState;
        ...
        RestartCharacters(prevMouseState);
    ...
    void RestartCharacters(GFxMouseState prevMouseState)
    {
        ...
        for (UInt i = 0; i < pDef->ButtonRecords.size(); i++)
        {
            ...
            case MOUSE_OVER:
                {
                    if ((rec->Over) && !rec->MatchMouseState(prevMouseState))
                    {
                        restart = true;
                    }
                    break;
            ...
            case Event_MouseDown:
                {
                    if ((rec->Down) && !rec->MatchMouseState(prevMouseState))
                    {
                        restart = true;
                    }
                    break;
            ...
            case Event_MouseUp:
                {
                    if ((rec->Up) && !rec->MatchMouseState(prevMouseState))
                    {
                        restart = true;
                    }
                    break;
