...
GFxStream::GFxStream(const UByte* pbuffer, UInt bufSize, GFxLog *plog, GFxParseControl *pparseControl)
{
...
void   GFxStream::ReadToBuffer(UByte* pdestBuf, UInt sz)
{
    if (DataSize == 0)
    {
        // In case Underlying file position was changed.
    ...
    // move exisiting data to front of destination buffer
    if (Pos < DataSize)
    {
        UInt szFromBuf = GTL::gmin(sz, DataSize - Pos);
