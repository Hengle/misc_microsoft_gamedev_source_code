...
public:
        ...
        VariableName(parent->GetASEnvironment()->CreateString(def->VariableName))
    {
        ...
        pDocument = *new GFxTextDocView(parent->GetFontManager(), GetLog());
    ...
    void    SetTextValue(const char* pnewText, bool html)
    {
        ...
        if (!IsNoTranslate())
        {
            ...
            if (ptrans)
            {
                ...
                else
                {
                    ...
                    SetInitialFormatsAsDefault();
                    pDocument->ParseHtml(pnewText, GFC_MAX_UPINT, IsCondenseWhite(), NULL, NULL);
                    GFxWStringBuffer wstrippedHtmlBuf;
                ...
                if (translated)
                {
                    if (ptrans->CanReturnHtml())
                    {
                        ...
                        pDocument->ParseHtml(wbuf.ToWStr(), GFC_MAX_UPINT, IsCondenseWhite(), &imageInfoArray, NULL);
        ...
        if (!translated)
        {
            if (html)
            {
                SetInitialFormatsAsDefault(); //!
                GFxStyledText::HTMLImageTagInfoArray imageInfoArray;
                pDocument->ParseHtml(pnewText, GFC_MAX_UPINT, IsCondenseWhite(), &imageInfoArray, NULL);
                if (imageInfoArray.size() > 0)
                {
                    ProcessImageTags(imageInfoArray);
                }
                    ...
                    if (!GTL::IsMax(pos))
                    {
                        ...
                        if (pDocument->GetTextAndParagraphFormat(&ptextFmt, NULL, pos))
                        {
                            if (ptextFmt->IsUrlSet())
                            {
                                ...
                                GFxMovieRoot* proot = GetMovieRoot();
                                if (proot && GFxString::CompareNoCase(url, "asfunction:", sizeof("asfunction:") - 1) == 0)
                                {
...
void GFxEditTextCharacter::ProceedImageSubstitution(const GASFnCall& fn, int idx, const GASValue* pve)
{
    if (pve && pve->IsObject())
    {
        ...
        if (peobj->GetConstMemberRaw (fn.Env->GetSC(), "subString", &val))
        {
            ...
            wstrLen = GTL::gpmin(wstrLen, (UPInt)(sizeof(isElem.SubString)/sizeof(isElem.SubString[0]) - 1));
