...
static inline GFxString GetUrlStrGfx(const GFxString& url)
{
    GFxString urlStrGfx;
    if ((url.GetSize() > 4) &&
...
// 
GFxMoviePreloadTask::GFxMoviePreloadTask(GFxMovieRoot* pmovieRoot, const GFxString& url, bool stripped, bool quietOpen)
    : GFxTask(GFxTask::Id_MovieDataLoad), Url(url), Done(false)
{
    pLoadStates = *new GFxLoadStates(pmovieRoot->pLevel0Def->pLoaderImpl, pmovieRoot->GetSharedImpl());
...
void GFxMoviePreloadTask::Execute()
{
    if (UrlStrGfx.GetLength() > 0)
    {
        ...
        GLock::Locker guard(&DefImplLock);
      ...
      BytesLoaded(0), FirstFrameLoaded(false)
{
    ...
    if (pQueueEntry->pCharacter)
    {
        ...
        pOldChar = pQueueEntry->pCharacter->ResolveCharacter(pMovieRoot);
    ...
    pPreloadTask = *new GFxMoviePreloadTask(pMovieRoot, pQueueEntry->URL, stripped, pqueueEntry->QueitOpen);
    ...
    if (pPreloadTask->IsDone())
    {
        ...
        if (!pdefImpl)
        {
            ...
            if (pQueueEntry->pCharacter)
            {
                ...
                // If we have a move loader listener then set a message
                pOldChar = emptyChar;
            } else if (pQueueEntry->Level != -1)
                pOldChar = pMovieRoot->GetLevelMovie(pQueueEntry->Level);

            if (pOldChar)
                pOldChar->SetAcceptAnimMoves(false);

            if (pQueueEntry->MovieClipLoaderHolder.ToObject())
            {
                GASSERT(pMovieRoot->GetLevelMovie(0));
                GASEnvironment* penv = pMovieRoot->GetLevelMovie(0)->GetASEnvironment();
                GASSERT(penv);
                GASSERT(pQueueEntry->MovieClipLoaderHolder.ToObject()->GetObjectType() 
                    == GASObjectInterface::Object_MovieClipLoader);
                GASMovieClipLoader* pmovieClipLoader = 
                    static_cast<GASMovieClipLoader*>(pQueueEntry->MovieClipLoaderHolder.ToObject());
        ...
        else 
        {
            ...
            if (!CharSwitched)
            {
                ...
                GPtr<GFxSprite> pnewChar = *new GFxSprite(pdefImpl->GetDataDef(), pdefImpl,
            ...
            if (BytesLoaded != pdefImpl->GetBytesLoaded() && pQueueEntry->MovieClipLoaderHolder.ToObject())
            {
                GASSERT(pMovieRoot->GetLevelMovie(0));
                GASEnvironment* penv = pMovieRoot->GetLevelMovie(0)->GetASEnvironment();
                GASSERT(penv);
            ...
            if (!FirstFrameLoaded && pdefImpl->pBindData->GetBindState() & GFxMovieDefImpl::BSF_Frame1Loaded)
            {
                ...
                if (pQueueEntry->Level == -1)
                    psprite = static_cast<GFxSprite*>(pOldChar.GetPtr());
                else
            ...
            if (pdefImpl->pBindData->GetBindState() & GFxMovieDefImpl::BS_Finished)
            {
                ...
                if (pQueueEntry->MovieClipLoaderHolder.ToObject())
                {
                    ...
                    GASMovieClipLoader* pmovieClipLoader = 
                        static_cast<GASMovieClipLoader*>(pQueueEntry->MovieClipLoaderHolder.ToObject());
                    pmovieClipLoader->NotifyOnLoadComplete(penv, pOldChar, 0);
                    pmovieClipLoader->NotifyOnLoadInit(penv, pOldChar);
                }
...
GFxLoadVarsTask::GFxLoadVarsTask(GFxLoadStates* pls, const GFxString& level0Path, const GFxString& url, GFxLoadQueueEntry* pqueueEntry)
    : pLoadStates(pls), Level0Path(level0Path), Url(url), pQueueEntry(pqueueEntry), FileLen(0), Done(false)
{
}

bool GFx_ReadLoadVariables(GFile* pfile, GFxString* pdata, SInt* pfileLen);
//bool GFx_ReadLoadVariables(GFile* pfile, GFxString* pdata, SInt* pfileLen);
...
void GFxLoadVarsTask::Execute()
{
   ...
   // File loading protocol
    ...
    GLock::Locker guard(&DataLock);
    if (pfile)
    {
        if (pQueueEntry->LoadVarsHolder.ToObject())
        {
...
bool GFxLoadVarsTask::GetData(GFxString* data, SInt* fileLen) const
{
    GASSERT(data);
    GASSERT(fileLen);
    if (!Done)
    ...
    : GFxLoadQueueEntryMT(pqueueEntry,pmovieRoot)
{
    ...
    pMovieRoot->GetLevel0Path(&level0Path);
    pTask = *new GFxLoadVarsTask(pLoadStates, level0Path,pqueueEntry->URL, pqueueEntry);
...
:       
    ...
    BackgroundSetByTag(false), MovieIsFocused(true)
{
    TimeElapsed     = 0.0f;
    TimeRemainder   = 0.0f;
...
void    GFxMovieRoot::AddLoadQueueEntryMT(GFxLoadQueueEntry* pentry)
{
    ...
    {
        pentryMT = new GFxLoadQueueEntryMT_LoadVars(pentry,this);
    }
    else
    {
        pentryMT = new GFxLoadQueueEntryMT_LoadMovie(pentry,this);
        if (pentryMT)
        {
...
void GFxMovieRoot::AddMovieLoadQueueEntry(GFxLoadQueueEntry* pentry)
{
    ...
    {
        GFxString urlLowerCase = pentry->URL.ToLower();
...
{   
    ...
    if (pimageResource)
    {
        ...
        pdefImpl = new GFxMovieDefImpl(pimageMovieDataDef,
...
void    GFxMovieRoot::ProcessLoadMovieClip(GFxLoadQueueEntry *pentry, GFxLoadStates* pls, const GFxString& level0Path)
{
    GASSERT(pentry);
    ...
    if (pentry->Level == -1)
    {
        ...
        if (!pnewChar)
        {
            pnewChar = (GFxSprite*)poldChar.GetPtr();
            GFxCharacterCreateInfo ccinfo =
                pparent->GetResourceMovieDef()->GetCharacterCreateInfo(newCharId);
    ...
    if (pentry->MovieClipLoaderHolder.ToObject())
    {
        ...
        GASMovieClipLoader* pmovieClipLoader = 
            static_cast<GASMovieClipLoader*>(pentry->MovieClipLoaderHolder.ToObject());
...
void    GFxMovieRoot::ProcessLoadVars(GFxLoadQueueEntry *pentry, GFxLoadStates* pls, const GFxString& level0Path)
{
    GASSERT(pentry);
    GASSERT((pentry->Type & GFxLoadQueueEntry::LTF_VarsFlag) != 0);
    ...
    if (pentry->URL.GetLength())
    {
        ...
        else
        {
            ...
            if (pfile)
            {
                if (pentry->LoadVarsHolder.ToObject())
                {
                    if (!GFx_ReadLoadVariables(pfile, &data, &fileLen))
...
void GFxMovieRoot::LoadVars(GFxLoadQueueEntry *pentry, GFxLoadStates* pls, const GFxString& data, SInt fileLen)
{
    if (pentry->LoadVarsHolder.ToObject())
    {
        GASSERT(pentry->LoadVarsHolder.ToObject()->GetObjectType() == GASObjectInterface::Object_LoadVars);
        GASLoadVarsObject* ploadVars = 
            static_cast<GASLoadVarsObject*>(pentry->LoadVarsHolder.ToObject());
        GASEnvironment* penv = pLevel0Movie->GetASEnvironment();
...
GFxSprite* GFxMovieRoot::CreateEmptySprite(GFxLoadStates* pls, SInt level)
{
    ...
    GPtr<GFxMovieDataDef> pemptyDataDef = *new GFxMovieDataDef(createKey,
        GFxMovieDataDef::MT_Empty, "");
    GPtr<GFxMovieDefImpl> pemptyMovieImpl;
    if (pemptyDataDef)
    {
        pemptyDataDef->InitEmptyMovieDef();
        pls->SetRelativePathForDataDef(pemptyDataDef);
        pemptyMovieImpl = *new GFxMovieDefImpl(pemptyDataDef, pls->GetBindStates(), 
                                               pls->pLoaderImpl, GFxLoader::LoadAll, 
                                               pSharedState->GetDelegate(), true);
    }
    if (!pemptyMovieImpl)
        return NULL;

    GFxSprite* pnewChar = new GFxSprite(pemptyDataDef, pemptyMovieImpl, this,
...
GFxASCharacter* GFxMovieRoot::GetTopMostEntity(const GPointF& mousePos, bool testAll, const GFxASCharacter* ignoreMC)
{
    GFxASCharacter* ptopMouseCharacter = 0;
    // look for chars marked as topmostLevel first.
    for (int i = (int)TopmostLevelCharacters.size() - 1; i >= 0; --i)
    {
        GFxCharacter* pch = TopmostLevelCharacters[i];
        GASSERT(pch);
        ptopMouseCharacter = pch->GetTopMostMouseEntity(mousePos, testAll, ignoreMC);
    ...
    if (!ptopMouseCharacter)
    {
        ...
        {   
            ...
            ptopMouseCharacter = pmovie->GetTopMostMouseEntity(mousePos, testAll, ignoreMC);
            if (ptopMouseCharacter)
                break;
        }
    }
...
{    
    ...
    if (pac)
    {
        ...
        LogRootFilenames       = (pac->GetActionFlags() & GFxActionControl::Action_LogRootFilenames) != 0;
        LogLongFilenames       = (pac->GetActionFlags() & GFxActionControl::Action_LongFilenames) != 0;
        LogChildFilenames      =  (pac->GetActionFlags() & GFxActionControl::Action_LogChildFilenames) != 0;
    }
    else
    {
        VerboseAction          = 0;
    ...
    if (LevelClipsChanged && (pLevel0Movie->GetLoadingFrame() > 0))
    {
        ...
        for (movieIndex = (UInt)MovieLevels.size(); movieIndex > 0; movieIndex--)
        {
            ...
            // if (MovieLevels[movieIndex-1].pSprite->GetLoadingFrame() > 0) ??? Do we need this check here?
        ...
        (NeedMouseUpdate || MouseMoved || (MouseButtonState.MouseButtonStateLast != (UInt)MouseButtons) || MouseWheelScroll) )
    {
        ...
        if (lastState != MouseButtons)
        {
            ...
            {
                if (MouseButtons & MouseButton_Left)
                    pIMEManager->OnMouseDown(this, MouseButtons, ptopMouseCharacter);
                else
                    pIMEManager->OnMouseUp(this, MouseButtons, ptopMouseCharacter);
            }
        }
#endif //#ifndef GFC_NO_IME_SUPPORT
        ...
        if ( (MouseButtonState.MouseButtonStateCurrent & MouseButton_Left) &&
            !(MouseButtonState.MouseButtonStateLast & MouseButtonState.MouseButtonStateCurrent)) // left mouse button was down
        {
            ...
            if (ptopMouseCharacter != curFocused)
            {
                ...
                { 
                    ...
                    if (!ptopMouseCharacter || !ptopMouseCharacter->DoesAcceptMouseFocus())
                        QueueSetFocusTo(NULL, ptopMouseCharacter, GFx_FocusMovedByMouse);
                    else
                        QueueSetFocusTo(ptopMouseCharacter, ptopMouseCharacter, GFx_FocusMovedByMouse);
                }
    ...
    if (TimeRemainder >= FrameTime)
    {
        ...
        bool frameCatchUp = (frameCatchUpCount > 0);
        do
        {
            if (frameCatchUp)
        ...
        } while(frameCatchUpCount-- > 0 && TimeRemainder >= FrameTime);
...
UInt  GFxMovieRoot::HandleEvent(const GFxEvent &event)
{
    ...
    if (pKeyboardState && event.SpecialKeysState.IsInitialized())
    {
        pKeyboardState->SetKeyToggled(GFxKey::NumLock, event.SpecialKeysState.IsNumToggled());
        pKeyboardState->SetKeyToggled(GFxKey::CapsLock, event.SpecialKeysState.IsCapsToggled());
        pKeyboardState->SetKeyToggled(GFxKey::ScrollLock, event.SpecialKeysState.IsScrollToggled());
    }
...
void    GFxMovieRoot::Display()
{
    ...
    DisplayTopmostLevelCharacters(context, rootStackData);
...
public:
    inline bool operator()(const GFxASCharacter* a, const GFxASCharacter* b) const
    {
        ...
        if (GTL::gabs(aRect.Top - bRect.Top) <= Epsilon ||
            GTL::gabs(aRect.Bottom - bRect.Bottom) <= Epsilon ||
            GTL::gabs(centerA.y - centerB.y) <= Epsilon)
        {
            // same row
            //@DBG
...
void GFxMovieRoot::FinalizeProcessFocusKey(ProcessFocusKeyInfo* pfocusInfo)
{
    ...
    if (pfocusInfo->Initialized)
    {
        ...
        {
            GFxASCharacter* psetFocusToCh = pfocusInfo->TabableArray[pfocusInfo->CurFocusIdx];
            QueueSetFocusTo(psetFocusToCh, NULL);
...
void GFxMovieRoot::HideFocusRect()
{
    if (IsShowingRect)
    {
        ...
        if (curFocused && curFocused->GetParent())
            if (!curFocused->OnLosingKeyboardFocus(NULL))
...
void GFxMovieRoot::SetFocusTo(GFxASCharacter* ch)
{
    ...
    if (curFocused && curFocused->GetParent())
    {
        ...
        if (!curFocused->OnLosingKeyboardFocus(ch))
            return; // focus loss was prevented
    }
...
void GFxMovieRoot::QueueSetFocusTo(GFxASCharacter* ch, GFxASCharacter* ptopMostCh, GFxFocusMovedType fmt)
{
    ...
    if (curFocused && curFocused->GetParent())
    {
        ...
        if (!curFocused->OnLosingKeyboardFocus(ch, fmt))
    ...
    GASSelection::QueueSetFocus(GetLevelMovie(0)->GetASEnvironment(), ch, fmt); 
}

// Instantly transfers focus w/o any queuing
void GFxMovieRoot::TransferFocus(GFxASCharacter* pNewFocus, GFxFocusMovedType fmt)
{
    GPtr<GFxASCharacter> curFocused = LastFocused;
    ...
    if (curFocused && curFocused->GetParent())
    {
        ...
        curFocused->OnFocus(GFxASCharacter::KillFocus, pNewFocus, fmt);
    }
    ...
    if (pNewFocus)
    {
        pNewFocus->OnFocus(GFxASCharacter::SetFocus, curFocused, fmt);
    }
...
void GFxMovieRoot::AddTopmostLevelCharacter(GFxCharacter* pch)
{
    // make sure this character is not in the list yet
    for (UPInt i = 0, n = TopmostLevelCharacters.size(); i < n; ++i)
    {
        if (TopmostLevelCharacters[i] == pch)
            return;
    }
    TopmostLevelCharacters.push_back(pch);
}
...
void GFxMovieRoot::RemoveTopmostLevelCharacter(GFxCharacter* pch)
{
    for (UPInt i = 0, n = TopmostLevelCharacters.size(); i < n; ++i)
    {
        if (TopmostLevelCharacters[i] == pch)
        {
            TopmostLevelCharacters.remove(i);
            return;
        }
    }
}

void GFxMovieRoot::DisplayTopmostLevelCharacters(GFxDisplayContext &context, StackData stackdata) const
{
    for (UPInt i = 0, n = TopmostLevelCharacters.size(); i < n; ++i)
    {
        GFxCharacter* pch = TopmostLevelCharacters[i];
        GASSERT(pch);
        pch->Display(context, stackdata);
    }
}
...
void GFxMovieRoot::SetIMECandidateListStyle(const GFxIMECandidateListStyle& st)
{
    ...
    if (!pIMECandidateListStyle)
        pIMECandidateListStyle = new GFxIMECandidateListStyle(st);
    else
...
void    GFxSwfEvent::AttachTo(GFxASCharacter* ch) const
{
    ...
    {            
        ...
        ch->SetEventHandlers(Event, method);
    }
}
...
void    GFxPlaceObject2::ExecuteStateReverse(GFxSprite* m, int frame)
{
    ...
    case Place_Move:        
        {       
            ...
            if (Pos.TextFilter) findPrevFlags |= GFxSprite::FindPrev_NeedFilters;
...
bool GFx_ReadLoadVariables(GFile* pfile, GFxString* pdata, SInt* pfileLen)
{
    ...
    UByte* td = (UByte*) GALLOC(*pfileLen);
    ...
    if (prefix16[0] == GByteUtil::BEToSystem((UInt16)0xFFFE)) // little endian
    {
        ...
        else
        {
            ...
            wcsptr = (wchar_t*) GALLOC(textLength * sizeof(wchar_t));
            for (SInt i=0; i < textLength; i++)
