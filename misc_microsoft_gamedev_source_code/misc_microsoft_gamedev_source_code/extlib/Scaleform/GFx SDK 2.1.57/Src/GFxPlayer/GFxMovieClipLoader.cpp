...
void GASMovieClipLoaderProto::LoadClip(const GASFnCall& fn)
{
    ...
    {
        const char* ptail = "";
        int         level = GFxMovieRoot::ParseLevelName(fn.Arg(1).ToString(fn.Env).ToCStr(), &ptail,
                                                         fn.Env->GetTarget()->IsCaseSensitive());
        if (level == -1)
            return;
        fn.Env->GetMovieRoot()->AddLoadQueueEntry(fn.Arg(1).ToString(fn.Env).ToCStr(), urlStr.ToCStr(), fn.Env,
                                                  GFxLoadQueueEntry::LM_None, pmovieClipLoader);
    }
