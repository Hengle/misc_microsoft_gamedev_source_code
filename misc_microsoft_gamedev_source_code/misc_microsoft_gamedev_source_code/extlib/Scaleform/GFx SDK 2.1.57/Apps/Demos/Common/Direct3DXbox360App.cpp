...
Direct3DXboxApp::Direct3DXboxApp(): FxApp()
{
    ...
    MouseX = MouseY = 100;
    pFrameBuffer[0] = pFrameBuffer[1] = 0;
    pTilingDepth = pTilingRT = 0;
}
...
void Direct3DXboxApp::CheckTilingSetup(bool forceTiling)
{
    if (pFrameBuffer[0])
        pFrameBuffer[0]->Release();
    if (pFrameBuffer[1])
        pFrameBuffer[1]->Release();

    if (pTilingDepth)
    {
        pDevice->SetDepthStencilSurface(0);
        pTilingDepth->Release();
    }
    if (pTilingRT)
    {
    ...
    pFrameBuffer[0] = pFrameBuffer[1] = 0;
    pTilingRT = pTilingDepth = 0;

    if (forceTiling)
        PredicatedTiling = 1;
    else if (Width > 1024 || Height > 1024)
        PredicatedTiling = 1;
    else if (PresentParams.MultiSampleType != D3DMULTISAMPLE_NONE && (Width > 640 || Height > 480))
        PredicatedTiling = 1;
    else
        PredicatedTiling = 0;
    ...
    if (PredicatedTiling)
    {
        ...
        PresentParams.RingBufferParameters.SecondarySize = 16 * 1024 * 1024;
        PresentParams.DisableAutoFrontBuffer    = 1;
        PresentParams.DisableAutoBackBuffer     = 1;
        ...
        NumTiles = 0;
        for (SInt x = 0; x < Width; x += TileWidth)
            for (SInt y = 0; y < Height; y += TileHeight)
            {
                Tiles[NumTiles].x1 = x;
                Tiles[NumTiles].x2 = GTL::gmin<SInt>(Width, x + TileWidth);
                Tiles[NumTiles].y1 = y;
                Tiles[NumTiles].y2 = GTL::gmin<SInt>(Height, y + TileHeight);
                NumTiles++;
            }
    }
    else
    {
        PresentParams.DisableAutoFrontBuffer    = 0;
        PresentParams.DisableAutoBackBuffer     = 0;
...
void Direct3DXboxApp::SetupTilingBuffers()
{
    if (PredicatedTiling)
    {
        // Front buffers
        pDevice->CreateTexture(PresentParams.BackBufferWidth, PresentParams.BackBufferHeight, 1, 0, D3DFMT_LE_X8R8G8B8, D3DPOOL_DEFAULT,
            &pFrameBuffer[0], 0);
        ...
        pDevice->CreateRenderTarget(TileWidth, TileHeight, D3DFMT_A8R8G8B8, PresentParams.MultiSampleType, 0, FALSE,
            &pTilingRT, &SurfaceParams);

        SurfaceParams.Base = pTilingRT->Size / GPU_EDRAM_TILE_SIZE;
        SurfaceParams.HierarchicalZBase = 0;
        pDevice->CreateDepthStencilSurface(TileWidth, TileHeight, D3DFMT_D24S8, PresentParams.MultiSampleType, 0, FALSE,
            &pTilingDepth, &SurfaceParams);

        pDevice->SetRenderTarget(0, pTilingRT);
        pDevice->SetDepthStencilSurface(pTilingDepth);
    }
}
                                  ...
                                  const SetupWindowParams& extraParams )
{
    ...
    // Configure for predicated tiling if needed
    CheckTilingSetup(extraParams.ForceTiling);
...
void    Direct3DXboxApp::PrepareRendererForFrame()
{
    if (PredicatedTiling)
    {
        ...
        pDevice->BeginTiling(0, NumTiles, Tiles, &color, 0, 0);
    }
}
