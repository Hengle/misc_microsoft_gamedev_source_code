...
public:
    ...
    GEXPORT bool            AddWaitHandler(WaitHandler handler, void*  pdata);
    GEXPORT bool            RemoveWaitHandler(WaitHandler handler, void*  pdata);
...
public:
    ...
    typedef SInt (*ThreadFn)(GThread *pthread, void*  h);
    ...
    explicit GEXPORT GThread(UPInt stackSize = 128 * 1024, int processor = -1);
    ...
    // "processor" parameter specifies which hardware processor this thread will be run on. 
    // -1 means OS decides this. Implemented only from Win32 and XBox360
    GEXPORT          GThread(ThreadFn threadFunction, void*  userHandle = 0, UPInt stackSize = 128 * 1024,
    ...
    GEXPORT static int GetCPUCount();
...
#if defined(GFC_OS_WIN32) || defined(GFC_OS_XBOX) || defined(GFC_OS_XBOX360)
    GINLINE void*           GetOSHandle() const { return ThreadHandle; }
