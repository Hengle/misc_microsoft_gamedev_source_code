...
bool GFx_ReadLoadData(GFile* pfile, GFxString* pdata, SInt* pfileLen);
...
void GFxLoadVarsTask::Execute()
{
   ...
   // File loading protocol
    ...
    if (pfile)
    {
        ...
        {
            if (!GFx_ReadLoadData(pfile, &Data, &FileLen))
...
bool GFx_ReadLoadVariables(GFile* pfile, GFxString* pdata, SInt* pfileLen)
{
    ...
    GASGlobalContext::Unescape((const char*)td, *pfileLen, pdata);
    GFREE(td);
    return true;
}
/*
...
bool GFx_ReadLoadData(GFile* pfile, GFxString* pdata, SInt* pfileLen)
{
    ...
    {
        for (SInt i=0; i < *pfileLen>>1; i++)
            prefix[i] = GByteUtil::LEToSystem(prefix[i]);
        pdata->AppendString( ((const wchar_t*)td)+1, (*pfileLen>>1)-1 );
    }
    ...
    {
        pdata->AppendString((const char*)td, *pfileLen);
    }
