...
protected:
    ...
    enum FlagMasks
    {
        Mask_Scale9GridExists   = 0x1,
        Mask_TopmostLevel       = 0x2
    };
    UInt8                   Flags;
...
public:
    ...
    virtual GFxASCharacter *GetTopMostMouseEntity(const GPointF &pt, bool testAll = false, const GFxASCharacter* ignoreMC = NULL) { GUNUSED3(pt, testAll, ignoreMC); return NULL; }
    ...
    virtual void            Display(GFxDisplayContext&, StackData stackData)  { stackData; }
    ...
    virtual const GFxPointTestCacheProvider* GetPointTestCacheProvider() const { return NULL; }
};
...
protected:
    ...
    { 
        (v) ? (Flags |= Mask_TabEnabled) : Flags = UInt16((Flags & (~Mask_TabEnabled)) | ((Mask_TabEnabled - 1) & Mask_TabEnabled)); 
    }
    ...
    { 
        (v) ? (Flags |= Mask_FocusRect) : Flags = UInt16((Flags & (~Mask_FocusRect)) | ((Mask_FocusRect - 1) & Mask_FocusRect)); 
    }
    ...
    { 
        (v) ? (Flags |= Mask_UseHandCursor) : Flags = UInt16((Flags & (~Mask_UseHandCursor)) | ((Mask_UseHandCursor - 1) & Mask_UseHandCursor)); 
    }
...
public:
    ...
    virtual void            SetBlendMode(BlendType blend)       { BlendMode = blend; SetDirtyFlag(); }
    virtual void            SetName(const GASString& name);
    ...
    virtual void            OnFocus(FocusEventType event, GFxASCharacter* oldOrNewFocusCh, GFxFocusMovedType fmt);
    ...
    virtual bool            OnLosingKeyboardFocus(GFxASCharacter* newFocusCh, GFxFocusMovedType fmt = GFx_FocusMovedByKeyboard) 
    { 
        GUNUSED2(newFocusCh, fmt); 
        return true; 
    }
    ...
    void                    SetSingleEventHandler(const GFxEventId& id, const GASValue& method);
    void                    SetEventHandlers(const GFxEventId& id, const GASValue& method);
...
public:
    ...
    GFxASCharacter*  GetTopMostMouseEntity(const GPointF &pt, bool testAll = false, const GFxASCharacter* ignoreMC = NULL);

    virtual const GFxPointTestCacheProvider* GetPointTestCacheProvider() const { return this; }
};
