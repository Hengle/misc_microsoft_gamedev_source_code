...
#define GFX_EXPORT_MINOR_VERSION    13
...
#define gfc_getcwd  getcwd
#define gfc_chdir   chdir
#define gfc_mkdir(x)   mkdir(x,0777)
#endif
...
inline char* gfc_strlwr(char* str, size_t numberOfElements)
{
    ...
    return strlwr(str);
#else
    for (size_t i = 0; i < numberOfElements && str[i]; i++)
        str[i] = tolower(str[i]);
    return str;
#endif
}
...
void commaprint(unsigned long n)
{
    char retbuf[30];
...
GFxDataExporter::GFxDataExporter(UByte verboseFlags)
{
    ...
    ExportTextureList       = false;
    TextureListFile         = NULL;
    ...
    public:
        ...
        virtual void    LoadTagUpdate(const GFxProgressHandler::TagInfo& info)
        {
            pExporter->LoadTagProgressCallback(info);
        }
...
GFxString GFxDataExporter::CutFilename(const GFxString& filename)
{
    GFxString noext(filename);
    for (int n = (int)filename.GetSize() - 1; n >= 0; --n)
    {
...

{
    ...
    if (ExportTextureList && TextureListFile)
    {       
       
       GFxString tempFilename = buf;
       if (ToLowercase)
          tempFilename = tempFilename.ToLower();
       else if (ToUppercase)
          tempFilename = tempFilename.ToUpper();

       const char* ddxExtension = ".ddx";

       // remove the forward slash to make it consistent
       GFxString subDirPath = pdestPath;
       if (subDirPath.GetLength() > 0)
         subDirPath.Remove(subDirPath.GetLength()-1);
       
       char finalBuffer[1024];
       gfc_sprintf(finalBuffer, 1024, "      <File>%s\\%s%s</File>\n", subDirPath.ToCStr(), tempFilename.ToCStr(), ddxExtension);
       
       fprintf(TextureListFile, "%s", finalBuffer);
    }
    ...
    GFxString destPath = pimageExporter->MakePath(pdestPath, imgDescr.ImageName);
    if (Info)
    {
  ...
  if(!Info)
    ...
    {
        if(!gfc_strcmp(pimageExporter->GetName(),"TGA"))
        {
        ...
        else if (!gfc_strcmp(pimageExporter->GetName(),"DDS"))
        {
            UInt32 w,h;
            ...
            UInt32 size = GDXTHelper::CalculateDDSSize(w,h, /*GImage::Image_ARGB_8888*/ destImageFormat,DTXn, UncompressedDDS,GenMipMapLevels, DTX1Allowed);
            printf("estimated size (bytes) : ");
            commaprint(size);printf("\n");
            TotalMemoryCount+=size;
        }
        else if (!gfc_strcmp(pimageExporter->GetName(),"ORIG"))
        {
...
bool GFxDataExporter::FindIdenticalImage(const GFxString& imagePath, const GFxString& filePath, GFxExportImageDescr* pdescr, GFxImageExporter* pimageExporter)
{
    ...
    if (pimage)
    {
        ...
        if (pfileNamesArr)
        {
            for (UPInt i = 0, n = pfileNamesArr->size(); i < n; ++i)
            {
...
void GFxDataExporter::LoadTagProgressCallback(const GFxProgressHandler::TagInfo& info)
{
    GFxTagInfo tagInfo;
    ...
    else if ((ExportFonts || GlyphsStripped) && (
...
void GFxDataExporter::WriteGlyphTextureInfo(GFile* pout, UInt textureId, const GFxFontTextureDescr& textureDescr)
{
    ...
    const UInt numTextureGlyphs = (UInt)textureDescr.TextureGlyphs.size();
    UInt i, n;
    ...
    for (i = 0, n = (UInt)textureDescr.Fonts.size(); i < n; ++i)
    {
        // save FONTINFO
        ...
        for(UInt i = 0, n = (UInt)fntDescr.TexGlyphsIndicesMap.size(); i < n; ++i)
        {
...
void GFxDataExporter::WriteStrippedSwf(const char* srcFileName, const char* dstFileName, const GFxString& name)
{
    ...
    if (ExportGradients)
    {
        ...
        for (; iter != GradientTextures.end(); ++iter)
        {
            ...
            for (UInt i = 0, n = (UInt)gtDescr.SlaveIds.size(); i < n; ++i)
            {
    ...
    for (UPInt i = 0, n = TagsToBeRemoved.size(); i < n; ++i)
    {
...
int GFxDataExporter::Process()
{
    ...
    for (UPInt i = 0, n = SrcFileNames.size(); i < n; ++i)
    {
        ...
        if (ExportFonts)
        {
            ...
            if (Info)
            {
                printf("Font textures             : %d \n", (int)FontTextures.size());
            }


            TotalImageCount += (int)FontTextures.size();
            ...
            { // collect number of fonts per each texture
                ...
                for(; iter != FontTextures.end(); ++iter)
                {
                    ...
                    GPtr<GFxFontTextureCounter> pcnt = *new GFxFontTextureCounter((UInt)textureDescr.Fonts.size());
                ...
                for (i = 0, n = textureDescr.Fonts.size(); i < n; ++i)
                {
                    ...
                    struct TextureGlyphVisitor : public GFxTextureGlyphData::TextureGlyphVisitor
                    {
                        ...
                        void Visit(UInt index, GFxTextureGlyph* ptextureGlyph)
                        {
                            ...
                            {
                                UInt indexInTexture = (UInt)TextureDescr.TextureGlyphs.size();
        ...
        if (ExportGradients)
        {
            ...
            TotalImageCount += (int)GradientTextures.size();
        }
        if (Info)
        {
            printf("Gradient textures         : %d \n", (int)GradientTextures.size());
        }

        if (!Quiet && !Info) printf("Processing %d images - ", TotalImageCount);

        if (ExportTextureList)
        {
           WriteTextureListBegin(pMovieDef, SrcFileNames[i], path, name);
        }
        ...
        if (TotalImageCount > 0)
        {
            ...
            if (FontTextures.size() > 0)
            {
                ...
                for(; it != FontTextures.end(); ++it)
                {
                    ...
                    if (Info)
                    {
                        int nfonts=(int)ftDescr.Fonts.size();
...
void    GFxDataExporter::WriteFontList(GFxMovieDef* pmovieDef, const char* swfFileName, const GFxString& path, const GFxString& name)
{
    ...
    {        
        ...
        virtual void Visit(GFxMovieDef*, GFxResource* presource, GFxResourceId rid, const char*)
        {
            ...
            gfc_sprintf(buf, sizeof(buf), ", %d glyphs", pfontResource->GetGlyphShapeCount()); 
            font += buf;
...
//============================================================================
// void GFxDataExporter::WriteTextureListBegin()
//============================================================================
void GFxDataExporter::WriteTextureListBegin(GFxMovieDef* pmovieDef, const char* swfFileName, const GFxString& path, const GFxString& name)
{   
   pmovieDef;
   swfFileName;
   // setup output file
    char buf[1024];
    gfc_sprintf(buf, 1024, "%s.dep", name.ToCStr());
    if (ToLowercase)
        gfc_strlwr(buf, sizeof(buf));
    else if (ToUppercase)
        gfc_strupr(buf, sizeof(buf));
    GFxString fullPath = path;
    fullPath += buf;    
    if (!Quiet) printf("Saving list of textures as '%s'\n", fullPath.ToCStr());

    // write out data in UTF8
#if defined(GFC_CC_MSVC) && (GFC_CC_MSVC >= 1400)
    TextureListFile = NULL;
    fopen_s(&TextureListFile, fullPath, "w");
#else
    TextureListFile = fopen(fullPath, "w");
#endif // defined(GFC_CC_MSVC) && (GFC_CC_MSVC >= 1400)
    if (!TextureListFile)
    {
        fprintf(stderr, "\nError: Can't open destination file '%s' to write to\n", fullPath.ToCStr());
    ...
    fprintf(TextureListFile, "<?xml version=\"1.0\"?>\n");
    fprintf(TextureListFile, "<GFxDependencies>\n");
    fprintf(TextureListFile, "   <FileDependencies>\n");
}

//============================================================================
// void GFxDataExporter::WriteTextureListEnd()
//============================================================================
void GFxDataExporter::WriteTextureListEnd()
{   
   if (TextureListFile)
...
{
    if (gfc_stricmp(pfilterName, "box") == 0)
...
int GFxDataExporter::ParseCommandLine(int argc, const char** const argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (argv[i][0] == '-')
        {
            ...
            else if (gfc_strcmp(curArg, "ne") == 0)
            {
                NoExportNames = true;
            }
            ...
            else if (gfc_stricmp(curArg, "x") == 0)
            {
                QuietProgress = true;
            }
            ...
            else if (gfc_stricmp(curArg, "rescale") == 0)
            {
                ...
                {
                    fprintf(stderr, "Error: No rescale mode is specified. \n");
                }
            ...
            {
                curArg += 7;
                int f = ParseRescaleFilterType(curArg);
                if (f >= 0)
                {
            ...
            else if (gfc_stricmp(curArg, "info") == 0)
            {
                Info = true;
            }
            else if (gfc_stricmp(curArg, "tlist") == 0)
            {
                ExportTextureList = true;
            }
            else
            {
...
{
    printf("GFxExport v%d.%2.2d for SDK v%d.%d.%d, (c) 2006-2008 Scaleform Corporation\n",
...
void GFxDataExporter::ShowHelpScreen()
{
    ...
    printf(
           ...
           "\n"
           "Ensemble Studios Functionality\n"
           "  -tlist        Generates a dependecy (.dep) xml file that lists all of the textures exported\n"
           );
}
