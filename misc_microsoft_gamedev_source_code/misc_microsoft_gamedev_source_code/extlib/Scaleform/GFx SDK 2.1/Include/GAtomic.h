...
struct GAtomicOpsRaw_DefImpl : public O
{
    ...
    inline static O_T   Exchange_Sync(volatile O_T* p, O_T val)             { O_FullSync sync; GUNUSED(sync);    return Exchange_NoSync(p, val); }
    inline static O_T   Exchange_Release(volatile O_T* p, O_T val)          { O_ReleaseSync sync; GUNUSED(sync); return Exchange_NoSync(p, val); }
    inline static O_T   Exchange_Acquire(volatile O_T* p, O_T val)          { O_AcquireSync sync; GUNUSED(sync); return Exchange_NoSync(p, val); }  
    inline static O_T   ExchangeAdd_Sync(volatile O_T* p, O_T val)          { O_FullSync sync; GUNUSED(sync);    return ExchangeAdd_NoSync(p, val); }
    inline static O_T   ExchangeAdd_Release(volatile O_T* p, O_T val)       { O_ReleaseSync sync; GUNUSED(sync); return ExchangeAdd_NoSync(p, val); }
    inline static O_T   ExchangeAdd_Acquire(volatile O_T* p, O_T val)       { O_AcquireSync sync; GUNUSED(sync); return ExchangeAdd_NoSync(p, val); }
    inline static bool  CompareAndSet_Sync(volatile O_T* p, O_T c, O_T val)    { O_FullSync sync; GUNUSED(sync);    return CompareAndSet_NoSync(p,c,val); }
    inline static bool  CompareAndSet_Release(volatile O_T* p, O_T c, O_T val) { O_ReleaseSync sync; GUNUSED(sync); return CompareAndSet_NoSync(p,c,val); }
    inline static bool  CompareAndSet_Acquire(volatile O_T* p, O_T c, O_T val) { O_AcquireSync sync; GUNUSED(sync); return CompareAndSet_NoSync(p,c,val); }
...
#else
    inline static void  Store_Release(volatile O_T* p, O_T val)             { O_ReleaseSync sync; GUNUSED(sync); *p = val; }       
#endif
    inline static O_T     Load_Acquire(const volatile O_T* p)               { O_AcquireSync sync; GUNUSED(sync); return *p; }
};
