...
public:
    ...
    bool        Write()
    {
        if (!GDXTHelper::CompressToFile(Params.pImage, Params.pFile, Params.pExporter->Quality,
            Params.Rescale, Params.pExporter->RescaleFilter, Params.pExporter->MipFilter,
            Params.pImageDescr->TargetWidth, Params.pImageDescr->TargetHeight, Params.pExporter->DTXn,
            Params.UncompressedDDS, Params.pExporter->GenMipMapLevels, Params.pExporter->DTX1Allowed))
        {
            fprintf(stderr, "Error: Can't compress image '%s' to DDS, error: '%s'\n", Params.pFile->GetFilePath(), GDXTHelper::GetLastErrorString());
            return false;
        }
        return true;
    }
...
public:
    ...
    GFxString   MakePath(const GFxString& path, const GFxString& nameNoExt)
    {
        GFxString destPath = path;
        destPath += nameNoExt;
        destPath += ".";
        Params.PathNoExt = destPath;
        if (Params.pImageDescr->Format == GFxFileConstants::File_JPEG)
        {
            destPath += "jpg";
        }
        else
    ...
    bool        Write()
    {
        if (Params.pJpegDesc)
        {
            ...
            if (Params.pJpegDesc->TagType == 35)
            {
                // need to save also TGA with alpha
