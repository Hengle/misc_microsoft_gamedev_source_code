...
static int SearchForCmds(const UByte* pbuf, UInt bufSize, GFxFsCommand& cmd, GTL::garray<GFxString>& stringHolderArray, bool exportParams)
{
    if (!pbuf)
        return 0;
    const UByte* fsCmd = pbuf;
    UInt remainingSize = bufSize;
    ...
    do 
    {
        ...
        if (newfsCmd)
        {
            ...
            cmd.CommandIndices.push_back(stringHolderArray.size()-1);
            ...
            if (exportParams)
            {
                const char* pend = (const char*)fsCmd;
                const char* pparam = pend + 1;
                ...
                if (pparam && *pend == 0)
                {
                    ...
                    cmd.ParameterIndices.push_back(stringHolderArray.size()-1);
                }
                else
    ...
    return cmd.CommandIndices.size();
}
...
{
    for (UInt j = 0, k = cmd.CommandIndices.size(); j < k; j++)
    {
        ...
        case 34: // DefineButton2
            {
                ...
                //GFxCharacterCreateInfo chInfo = pmovieImpl->GetCharacterCreateInfo(GFxResourceId(id));
                //GFxCharacterDef* chDef = chInfo.pCharDef;
                GFxString exportName;
                //!if (chDef)
                {
                    const GFxString* pname = pmovieImpl->GetNameOfExportedResource(GFxResourceId(id));
                    if (pname)
                        exportName.CompareNoCase(*pname);
                }
                GFxFsCommandOrigin cmdOrig;
        ...
        case 39: // Define Sprite
            {
                ...
                GFxMovieDefImpl* pmovieImpl = static_cast<GFxMovieDefImpl*>(pmovieDef);
                //!GFxCharacterDef* chDef = pmovieImpl->GetCharacterDef(id);
                GFxString exportName;
                //!if (chDef)
                {
                    const GFxString* pname = pmovieImpl->GetNameOfExportedResource(GFxResourceId(id));
        ...
        case 70: // PlaceObj3
            {
                ...
                if (placeObj2.EventHandlers.size() > 0)
                {
                    ...
                    for(UInt i = 0, n = placeObj2.EventHandlers.size(); i < n; ++i)
                    {
...
{
    for (UInt i = 0, n = fscommands.size(); i < n; ++i)
    {
        ...
        case GFxFsCommandOrigin::Button:
            {
                ...
                fprintf(fout, ":\n");
                for(UInt i = 0, n = cmdOrig.Commands.size(); i < n; i++)
                {
    ...
    for (UInt j = 0, k = cmd.CommandIndices.size(); j < k; j++)
    {
        ...
        if (i < n)
        {
            if (str.CompareNoCase(cmdList[cmdSortedIdx[i]]) != 0)
            {
                ...
                cmdSortedIdx.insert(i, cmdList.size()-1);
            }
        }
        else
        {
            cmdList.push_back(str);
            cmdSortedIdx.push_back(cmdList.size()-1);
        }
    }
...
void GFxDataExporter::WriteFsCommands(GFxMovieDef* pmovieDef, const char* swfFileName, const GFxString& path, const GFxString& name, UInt mask)
{
    ...
    for (UInt i = 0, n = TagsWithActions.size(); i < n; ++i)
    {
    ...
    for (UInt curMask = 1; curMask <= mask; curMask <<= 1)
    {
        if (curMask & mask)
        {
            ...
            if (curMask == FSList)
            {
                ...
                for (UInt i = 0, n = cmdSortedIdx.size(); i < n; ++i)
                {
