...
Filename    :   RenderTextureXBox360.cpp
...
bool    FxPlayerApp::SetupRTTexture()
{
    ...
    if (pMovie)
    {
        ...
        if (pimageRes)    
        {
            ...
            if (pimageInfo)
            {
                GPtr<GTextureD3D9> ptexture = *((GRendererXbox360*)pRenderer)->CreateTexture();
...
{   
    ...
    // Compile simple 3d vertex shader.
...
bool    FxPlayerApp::LoadMovie(char *pfilename)
{
    ...
    // Substitute image resource in SWF/GFX file with render texture.
    GFxResource*	  pres = pMovie->GetMovieDef()->GetResource("texture1");
    GFxImageResource* pimageRes = 0;
    if (pres && (pres->GetResourceType() == GFxResource::RT_Image))
        pimageRes = (GFxImageResource*)pres;
        ...
        case 'L':
            {
                ...
                case FxPlayerSettings::AAMode_EdgeAA:
                    ...
                    break;
                /*
                // Supporting FSAA together with render textures wuld require
                // complicated tilig implementation due to EDRAM limits. For
