...
public:
    ...
    GPtr<GASMovieClipLoader>    pMovieClipLoader;
    GPtr<GASLoadVarsObject>     pLoadVars;

    // Constructor helper.      
    void    PConstruct(LoadType type, GFxCharacterHandle* pchar, SInt level, const GFxString &url, LoadMethod method)
    {
        Type        = type;
        pCharacter  = pchar;
        Method      = method;
    ...
    // *** Constructors
    GFxLoadQueueEntry(const GFxString &url, LoadMethod method, bool loadingVars = false)
    {
        ...
        PConstruct((LoadType) (typeFlag), NULL, -1, url, method);
    }

    GFxLoadQueueEntry(GFxCharacterHandle* pchar, const GFxString &url, LoadMethod method, bool loadingVars = false)
    {
        LoadTypeFlags typeFlag;
        if (loadingVars)
        {
        ...
        PConstruct((LoadType) (typeFlag), pchar, -1, url, method);
    }   
    
    GFxLoadQueueEntry(SInt level, const GFxString &url, LoadMethod method, bool loadingVars = false)
    {
        LoadTypeFlags typeFlag;
        if (loadingVars)
        {
        ...
        PConstruct((LoadType) (LTF_LevelFlag | typeFlag), 0, level, url, method);
    }
};
...
// A node created in root for every GFxSprite which its own GFxMovieDefImpl,
// serving as a node. We cache temporary state of these in beginning of Advance
// so that it is always consistent for all instances.
struct GFxSpriteRootNode : public GPodDListNode<GFxSpriteRootNode>, public GNewOverrideBase
{
    // Sprite this is for
    GFxSprite * pSprite;
    ...
    GFxSpriteRootNode(GFxSprite *psprite, bool importFlag = 0)
        : pSprite(psprite),  ImportFlag(importFlag)
    {  }
};
...
public:
    ...
    // A list of all sprites that have a root MovieDefImpl, and could thus
    // be loaded progressively in background.
    GPodDList<GFxSpriteRootNode> RootSpriteNodes;
    ...
    virtual Float       Advance(Float deltaT, bool frameCatchUp);

    // Events.
    virtual void        HandleEvent(const GFxEvent &event);
    ...
    */
    ...
    virtual Handle      GetUserData() const                                 { return UserData; }
    virtual void        SetUserData(Handle ud)                              { UserData = ud;  }
...
/*
    ...
    GFxASCharacter*     GetTopMostEntity(const GPointF& mousePos, bool testAll);
...
public:
    ...
    // Queue up setting of focus
    void                QueueSetFocusTo(GFxASCharacter* ch);
    ...
    void                TransferFocus(GFxASCharacter* pNewFocus);
    ...
    // Invoked when mouse pressed; checks, for newCharWithInput == LastFocused, if
    // not - invokes LastFocused->OnLosingMouseInput()
    void                FocusMouseInputMoved(GFxASCharacter* newCharWithInput);
