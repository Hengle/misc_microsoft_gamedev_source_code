...
void    GFxFontData::Read(GFxLoadProcess* p, GFxTagType tagType)
{
    GASSERT(tagType == GFxTag_DefineFont ||
            tagType == GFxTag_DefineFont2 ||
      ...
      //  TextureGlyphs.resize(Glyphs.size());
        ...
        if (isLoadingFontShapes)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                GPtr<GFxShapeCharacterDef> s = *new GFxShapeCharacterDef;
                s->Read(p, GFxTag_DefineShape, false);
    ...
    else if ((tagType == GFxTag_DefineFont2) || (tagType == GFxTag_DefineFont3))
    {
        if (tagType == GFxTag_DefineFont2)
    ...
    //    TextureGlyphs.resize(Glyphs.size());
        ...
        if (isLoadingFontShapes)
        {
            GFxTagType fontGlyphShapeTag = (tagType == GFxTag_DefineFont2) ?
                                           GFxTag_DefineShape2 : tagType;
            ...
            for (int i = 0; i < glyphCount; i++)
            {
                ...
                // Create & read the shape.
                GPtr<GFxShapeCharacterDef> s = *new GFxShapeCharacterDef;
                s->Read(p, fontGlyphShapeTag, false);
        ...
        if (hasLayout)
        {
            ...
            Float sfactor = (tagType == GFxTag_DefineFont3)  ? 1.0f / 20.0f : 1.0f;
...
GFxShapeCharacterDef*   GFxFontData::GetGlyphShape(UInt index)
{
