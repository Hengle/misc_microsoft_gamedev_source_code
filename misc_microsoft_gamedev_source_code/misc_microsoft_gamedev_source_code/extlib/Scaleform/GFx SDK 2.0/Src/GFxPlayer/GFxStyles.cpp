...
{
    static GFxGradientImageResourceCreator inst;
    return GFxResourceData(&inst, this);
}

GFxResourceKey  GFxGradientData::CreateGradientKey()
{
    static GFxGradientImageResourceKey inst;
    return GFxResourceKey(&inst, this);
}
...
{   
    ...
    else
    {
        ...
        GetFillTexture(&ft, dc, scaleMultiplier);
        if (ft.pTexture)        
                                     ...
                                     Float scaleMultiplier) const
{
    ...
    if (IsGradientFill())
    {
        ...
        if (pimageRes)
        {
            ...
            if (pimageInfo)
            {	
                //-- if the texture is the same don't bother recomputing
                GTexture* pCurrentTexture = pimageInfo->GetTexture(prenderer);
                if (pftexture->pTexture == pCurrentTexture)
                   return 1;
                ...
                pftexture->TextureMatrix.Prepend(ImageMatrix.GetInverse());
    ...
    else if (IsImageFill())
    {
        ...
        if (pimageInfo)
        {
            ...
            if (ptexture)
            {
                ...
                pftexture->TextureMatrix.SetInverse(ImageMatrix);
                pftexture->TextureMatrix.PrependScaling(scaleMultiplier);
