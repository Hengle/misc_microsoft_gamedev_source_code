...
void    GASMouseCtorFunction::AddListener(const GASFnCall& fn)
{
    if (fn.NArgs < 1)
    {
        fn.LogScriptError("Error: Mouse.addListener needs one argument (the listener object)\n");
        return;
    }
    ...
    if (listener == NULL)
    {
        fn.LogScriptError("Error: Mouse.addListener passed a NULL object; ignored\n");
        return;
    }
...
void    GASMouseCtorFunction::RemoveListener(const GASFnCall& fn)
{
    if (fn.NArgs < 1)
    {
        fn.LogScriptError("Error: Mouse.removeListener needs one Argument (the listener object)\n");
        return;
    }
    ...
    if (listener == NULL)
    {
        fn.LogScriptError("Error: Mouse.removeListener passed a NULL object; ignored\n");
        return;
    }
    ...
    for (UPInt i = mo->Listeners.size() - 1; i >= 0; i--)
    {
        if (mo->Listeners[i] == listener)
        {
...
void    GASMouseCtorFunction::Show(const GASFnCall& fn)
{
    ...
    if (proot->pUserEventHandler)
        ...
        fn.LogScriptWarning("Warning: no user event handler interface is installed; Mouse.show failed.\n");
}
...
void    GASMouseCtorFunction::Hide(const GASFnCall& fn)
{
    ...
    if (proot->pUserEventHandler)
        ...
        fn.LogScriptWarning("Warning: no user event handler interface is installed; Mouse.hide failed.\n");
}
...
void    GASMouseCtorFunction::SetCursorType(const GASFnCall& fn)
{
    ...
    if (!SetCursorType(proot, cursorShapeId))
        fn.LogScriptWarning("Warning: no user event handler interface is installed; Mouse.setCursorType failed.\n");
}
