                                    ...
                                    const GASValue& val, const GASPropFlags& flags)
{
    ...
    {
        allowToSet = true;
        //TODO: should be array here!
        //GASNumber v = val.ToNumber(penv);
        //if (v >= 0)
        //    ParagraphFormat.SetTabStops((UInt)v);
    }
...
{ 
    ...
    SetConstMemberRaw(psc, "color", (textFmt.IsColorSet())?GASValue(GASNumber(textFmt.GetColor() & 0xFFFFFFu)):nullVal);
...
{ 
    ...
    //?SetConstMemberRaw(psc, "tabStops", (paraFmt.IsTabStopsSet())?GASValue(GASNumber(paraFmt.GetTabStops())):nullVal);
}
