...
GFxEdgeAAGenerator::GFxEdgeAAGenerator(GTessellator &tess, const GTL::garray<GFxFillStyle> *pfills)
{
    ...
    int fillSize = pFills ? (int)pFills->size() : 0;
...
bool    GFxEdgeAAGenerator::GenerateSolidMesh(GFxVertexArray *pvertices, GFxMesh *pmesh, Float scaleMultiplier)
{
    ...
    int  fillsSize = pFills ? (int)pFills->size() : 0;
    ...
    for (i=0; i< EdgeAA.GetNumTriangles(); i++)
    {
            ...
            (i == 0) )
        {
                ...
                (gft == GRenderer::GFill_2Texture) )
            {
                ...
                // If s3 did not change and s1 did not change (or at least change texture use-status)
                if ( (s3 == v3.id) &&
                     ((s1 == v1.id) || (!v1Texture && !s1IsTexture))  )
                {
