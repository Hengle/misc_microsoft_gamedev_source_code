...
bool    GFxFontMap::MapFont(const char* pfontName, const char* pnewFontName, MapFontFlags mff)
{
    if (!pImpl || !pfontName)
        return 0;
    ...
    MapEntry  me(pnewFontName, mff);
    pImpl->FontMap.set(fontName, me);
    return 1;
}

bool    GFxFontMap::MapFont(const wchar_t* pfontName, const wchar_t* pnewFontName, MapFontFlags mff)
{
    if (!pImpl || !pfontName)
        return 0;
    ...
    MapEntry  me(GFxString(pnewFontName), mff);
    pImpl->FontMap.set(fontName, me);
    return 1;
}
