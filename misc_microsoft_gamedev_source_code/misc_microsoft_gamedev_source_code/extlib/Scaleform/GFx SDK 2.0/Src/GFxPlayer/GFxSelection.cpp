...
void GASSelection::DoTransferFocus(const GASFnCall& fn)
{
    ...
    GASSERT(proot && fn.NArgs == 1);
    GASSERT(fn.Arg(0).IsCharacter());

    proot->TransferFocus(fn.Arg(0).ToASCharacter(fn.Env));
}

void GASSelection::QueueSetFocus(GASEnvironment* penv, GFxASCharacter* pNewFocus)
{
    GASSERT(penv);
