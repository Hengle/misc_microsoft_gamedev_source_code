    ...
    if (ms->MouseButtonStateLast & 1)
    {
            ...
            || ActiveEntity->GetTrackAsMenu())
        {
            if (TopmostEntity
                ...
                && TopmostEntity->GetTrackAsMenu() == true)
            {
...
public:
    ...
    virtual bool    PointTestLocal(const GPointF &pt, bool testShape = 0) const
    {
        ...
        for (UInt i = 0; i < pDef->ButtonRecords.size(); i++)
        {
            ...
            if (ch->PointTestLocal (p, testShape))
                return true;
        }   
        return false;
    }
    ...
    virtual GFxASCharacter* GetTopMostMouseEntity(const GPointF &pt, bool)
    {
        if (!GetVisible())
            return 0;
        ...
        for (UInt i = 0; i < pDef->ButtonRecords.size(); i++)
        {
            ...
            if (i < RecordCharacter.size())
            {
                ...
                if (pcharacter && pcharacter->GetCharacterDef()->PointTestLocal(SubP, 1))
                {
    ...
    virtual bool    SetMember(GASEnvironment* penv, const GASString& name, const GASValue& val, const GASPropFlags& flags = GASPropFlags())
    {
        ...
        if (ASButtonObj)
        {
            return ASButtonObj->SetMember(penv, name, val, flags);
        }
        return false;
    }
