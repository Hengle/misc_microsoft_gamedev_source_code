...
{   
    ...
    case M_enabled:
        {
            Enabled = val.ToBool(pEnv);
            return 1;
        }       

    case M_trackAsMenu:
        {
            TrackAsMenu = val.ToBool(pEnv);
            return 1;
        }
    ...
    case M_framesloaded:        
        ...
        LogScriptWarning("Attempt to write read-only property %s.%s, ignored\n",
                         GetName().ToCStr(), GFxASCharacter_MemberTable[member].pName);
        return 1;

    case M_tabEnabled:
        TabEnabled = val.ToBool(GetASEnvironment());
        return 1;
    ...
    case M_focusrect:
        FocusRect = val.ToBool(GetASEnvironment());
        return 1;
    ...
    case M_useHandCursor:
        UseHandCursor = val.ToBool(GetASEnvironment());
        return 1;
...
bool    GFxASCharacter::GetStandardMember(StandardMember member, GASValue* val, bool opcodeFlag) const
{
    if (opcodeFlag && ((member < M_BuiltInProperty_Begin) || (member > M_BuiltInProperty_End)))
    {
        LogScriptError("Invalid GetProperty query, property number %d\n", member);
        return 0;
    }
    ...
    case M_focusrect:   
        {
            ...
            if (FocusRect.IsDefined())
                val->SetBool(FocusRect.IsTrue());
            else
                val->SetNull();
            return 1;
        }
    ...
    case M_useHandCursor:
        if (!UseHandCursor.IsDefined())
            return 0;
        val->SetBool(UseHandCursor.IsTrue());
        return 1;
...
void    GFxASCharacter::SetEventHandler(const GFxEventId& id, const GASValue& method)
{ 
...
void GFxASCharacter::OnFocus(FocusEventType event, GFxASCharacter* oldOrNewFocusCh)
{
...
GFxASCharacter*  GFxGenericCharacter::GetTopMostMouseEntity(const GPointF &pt, bool testAll)
{   
    if (!GetVisible())
        return 0;
    ...
    if ((ClipDepth == 0) && pDef->PointTestLocal(p, 1, this))
    {
...
bool GFxGenericCharacter::PointTestLocal(const GPointF &pt, bool testShape) const
{     
    return pDef->PointTestLocal(pt, testShape, this);
}
