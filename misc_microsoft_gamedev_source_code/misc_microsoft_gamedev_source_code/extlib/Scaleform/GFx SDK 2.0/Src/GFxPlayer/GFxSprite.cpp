...
bool GFxSpriteDef::PointTestLocal(const GPointF &pt, bool testShape, const GFxPointTestCacheProvider *pcache) const  
{ 
    GUNUSED3(pt, testShape, pcache);
    return false;
}
    ...
    MouseStatePriv(UP)
{
    ...
    {
        pRootNode = new GFxSpriteRootNode(this, importFlag);
        ...
        pRoot->RootSpriteNodes.PushFront(pRootNode);
    }
...
GFxSprite::~GFxSprite()
{
    if (pRootNode)    
    {
        pRoot->RootSpriteNodes.Remove(pRootNode);    
        delete pRootNode;
    }
...
void    GFxSprite::CalcDisplayListHitTestMaskArray(GTL::garray<UByte> *phitTest, const GPointF &pt, bool testShape) const
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (pmaskch->GetClipDepth() > 0)
        {
            ...
            (*phitTest)[i] = pmaskch->PointTestLocal(p, 1);
...
GFxASCharacter* GFxSprite::GetTopMostMouseEntity(const GPointF &pt, bool testAll)
{
    // Invisible sprites/buttons don't receive mouse events (i.e. are disabled).
    // Masks also shouldn't receive mouse events (!AB)
    if (HitTestDisable || !GetVisible() || IsUsedAsMask())
        return 0;
    ...
    for (i = n - 1; i >= 0; i--)
    {
        ...
        if (ch != NULL)
        {           
            GFxASCharacter* te = ch->GetTopMostMouseEntity(p, testAll);
...
{       
    ...
    if (InitActionsExecuted[frame] == false)
    {
        ...
        */
        ...
        const Frame* pinitActions = pDef->GetInitActions(frame);
       
        if (pinitActions && pinitActions->GetTagCount() > 0)
        {
            // Need to execute these actions.
            for (UInt i= 0; i < pinitActions->GetTagCount(); i++)
            {
                GASExecuteTag*  e = pinitActions->GetTag(i);
                e->Execute(this);
            }
            markFrameAsVisited = true;
        }
...
void GFxSprite::ExecuteImportedInitActions(GFxMovieDef* psourceMovie)
{
    ...
    for(UPInt f = 0, fc = pdataDef->InitActionList.size(); f < fc; ++f)
    {
        const Frame& actionsList = pdataDef->InitActionList[f];
        for(UInt i = 0, n = actionsList.GetTagCount(); i < n; ++i)
        {
...
void    GFxSprite::Display(GFxDisplayContext &context, StackData stackData)
{
    ...
    if (!GetVisible() && !IsUsedAsMask())
        return;

    // If Alpha is Zero, don't draw the object.
    if (!IsUsedAsMask())
    {
        const Cxform &cx = GetCxform();
    ...
    context.pResourceBinding = &pDefImpl->ResourceBinding;
                                ...
                                GFxASCharacter* origChar)
{
    ...
    if (pscriptCh)
    {
        ...
        // MA: Original lookup of GetNameOfExportedResource was done based on 'cdef' pointer.
        // TBD
        // We can try to look up a name based on an id; however, does that id, which is local
        // in an import correspond with a correct GetResourceMovieDef?
        //
        // OLD: const GFxStringI* psymbolName = ch->GetResourceMovieDef()->GetNameOfExportedResource (cdef);

        GASFunctionRef ctorFunc;    
        const GFxString* psymbolName = ch->GetResourceMovieDef()->GetNameOfExportedResource(ch->GetId());
...
void    GFxSprite::ReplaceDisplayObject(const GFxCharPosInfo &pos, const GASString& name)
{
    ...
    DisplayList.ReplaceDisplayObject(pos, ch.GetPtr());     
...
bool    GFxSprite::SetMember(GASEnvironment* penv, const GASString& name, const GASValue& val, const GASPropFlags& flags)
{    
    ...
    if (ASMovieClipObj)
    {
        ...
        return ASMovieClipObj->SetMember(penv, name, val, flags);
    }
    return false;
}
...
// INSENSITIVE.
GFxASCharacter* GFxSprite::GetRelativeTarget(const GASString& name)
{
...
{ 
    ...
    if (id.Id == GFxEventId::Event_KeyDown)
    {
        ...
        if (!((*pkeyMask) & GFxCharacter::KeyMask_KeyPress))
        {
            ...
            if ((rv = OnEvent(GFxEventId(GFxEventId::Event_KeyPress, short(kc), 0)))!=0)
            {
...
bool GFxSprite::PointTestLocal(const GPointF &pt, bool testShape) const 
{ 
    if (HitTestDisable || !GetBounds(GRenderer::Matrix()).Contains(pt))
        return false;
    ...
    if (pmask)
    {
        if (pmask->IsUsedAsMask() && !pmask->IsUnloaded())
        {
            ...
            if (!pmask->PointTestLocal(p, testShape))
                return false;
        }
    }

    GTL::garray<UByte> hitTest;
    CalcDisplayListHitTestMaskArray(&hitTest, pt, testShape);
    ...
    for (i = n - 1; i >= 0; i--)
    {
        ...
        if (pch->PointTestLocal(p, testShape))
            return true;
    }   

    if (pDrawingAPI)
    {
        if (pDrawingAPI->PointTestLocal(pt, testShape))
            return true;
    }
...
bool    GFxSprite::IsTabable() const
{
    ...
    if (!TabEnabled.IsDefined())
        return (ActsAsButton() || GetTabIndex() > 0);
    else
        return TabEnabled.IsTrue();
}
...
void GFxSprite::OnLosingKeyboardFocus() 
{
...
{
    if (level == 0)
        FocusRect = true; // _focusrect in _root is true by default
    else if (level > 0)
    {
...
static void GFx_SpriteLocalToGlobal(const GASFnCall& fn)
{
    ...
    if ((xval.GetType() != GASValue::NUMBER) || (yval.GetType() != GASValue::NUMBER))
        return;
...
static void GFx_SpriteHitTest(const GASFnCall& fn)
{
    ...
    if (fn.NArgs >= 2)
    {
        ...
        if (fn.NArgs >= 3) // optional parameter shapeFlag
            shapeFlag = fn.Arg(2).ToBool(fn.Env);
        else
            shapeFlag = false;
        ...
        if (spriteLocalBounds.Contains(ptSpr))
        {
            if (!shapeFlag)
                ...
                fn.Result->SetBool(psprite->PointTestLocal (ptSpr, true));
        }
        else
...
void    GFxSprite::SpriteSwapDepths(const GASFnCall& fn)
{
    ...
    if (arg.GetType() == GASValue::NUMBER)
    {
