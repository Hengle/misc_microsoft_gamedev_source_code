                         ...
                         bool useEdgeAA)
{
    ...
    if (TriangleIndices.size() > 0)
    {
            ...
            if (fillsSize)  
            {
                ...
                for (styleIndex = 0; styleIndex < StyleCount; styleIndex++)
                {
                    if (Styles[styleIndex] != GFxPathConsts::Style_None)
                        if (pfills[Styles[styleIndex]].GetFillTexture(&ft[styleIndex], dc, scaleMultiplier))
        ...
        // else (!useEdgeAA)
        {
            ...
            pfills[GetStyle()].Apply(dc, scaleMultiplier);
        }
...
# endif // GFC_NO_FXPLAYER_EDGEAA
    ...
    sx = (Float)mat.GetXScale();
    sy = (Float)mat.GetYScale();
...
//useStrokerAA = 0; // To enforce the regular stroker
    ...
    if (pCachedMesh)
    {
        ...
        // EdgeAA start must also match (if disabling/enabling EdgeAA, tesselation is recomputed).
...
# endif
        ...
        // Use StrokerAA instead of Stroker+Tesselator if possible.
        if (useStrokerAA)   
        {
...
# ifndef GFC_NO_FXPLAYER_EDGEAA
                ...
                if (needEdgeAA)
                {
                    ...
                    GFxEdgeAAGenerator aaGen(strokeTess, &strokeFillStyles);
...
# endif // #ifndef GFC_NO_FXPLAYER_EDGEAA
    ...
    if (Vertices.GetSize())
    {
        ...
        {
            // Note that at his point null pfill is ok, because it would indicate
        ...
        pCachedMesh->Display(context, pfill, fillsSize,
                             0, Vertices.GetSize(), multiplier, IsUsingEdgeAA());
    }
...
// Tesselate the shape's paths into a different GFxMesh for each fill style.
...
void    GFxMeshSet::Display(
            GFxDisplayContext &context,
            const GRenderer::Matrix& mat, const GRenderer::Cxform& cx, GRenderer::BlendType blend,
...

{
	...
	m.r[0] = XMVectorMultiply( scale , mat.r[0] );
	m.r[1] = XMVectorMultiply( scale , mat.r[1] );
	...
	*/
    ...
    prenderer->SetMatrix(*((GRenderer::Matrix*)&m));
	...
	Float strokeOffset = 5.0f * context.InvPixelScale;
    for (ishape = 0, imesh=0, istrip=0 ; ishape < Shapes.size(); ishape++)
    {
        ...
        {
            Meshes[imesh].Display(context, fills.size() ? &fills[0] : 0, (UInt)fills.size(),
                                  Shapes[ishape].VertexStartIndex, Shapes[ishape].VertexCount,
                                  ScaleMultiplierInv, IsUsingEdgeAA());
        }
        ...
        {                        
            ...
            //GRenderer::Matrix strokeMat(*((GRenderer::Matrix*)&m));  
            ...
            for (i = 0; (i < Shapes[ishape].StrokeCount) && (istrip < Strokes.size()); i++, istrip++)
            {
                ...
                Strokes[istrip].Display(context, lineStyles[style], mat,
...
void  GFxMeshSet::AddTesselatedShape(const GCompoundShape &shape, const GTL::garray<GFxFillStyle> &fills,
                                     GFxDisplayContext &context)
{
    UInt            i, j;
    GTessellator    tess;
    ...
    tess.SetFillRule(GTessellator::FillEvenOdd);
    ...
    // Add 1 to styles to comform to the existing policy. 
    ...
    {
        GFC_DEBUG_WARNING(1, "GFxMeshSet::AddTesselatedShape failes - more then 65K vertices generated");
        return;
    }
    ...
    if (rconfig.IsUsingEdgeAA() && !EdgeAADisabled)
    {
        ...
        if (AllowCxformAddAlpha && rconfig.HasCxformAddAlpha() && !hasVF_CF32)
            ...
            for (i = 0; i < fills.size(); i++)
                if (fills[i].GetType() != GFxFill_Solid)
                {
                    useEdgeAA = 0;
                    break;
                }
    ...
    if (useEdgeAA)
    {
        ...
        GFxEdgeAAGenerator aaGen(tess, &fills);
