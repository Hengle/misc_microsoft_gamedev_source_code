...
//
GFxStaticTextCharacterDef::GFxStaticTextCharacterDef(GFxMovieDataDef* prootDef) : 
    RootDef(prootDef), Flags(0)
{
}
...
bool    GFxStaticTextCharacterDef::PointTestLocal(const GPointF &pt, bool testShape, const GFxPointTestCacheProvider *pcache) const
{
    GUNUSED2(testShape, pcache);
...
public:
        ...
        pDef(def)
    {
        ...
        for (UInt i = 0, n = pDef->TextRecords.GetSize(); i < n; ++i)
        {
            ...
            GFxResourceBindData fontData = pbindingDefImpl->ResourceBinding.GetResourceData(prec->pFont);
...
void    GSTDCALL GFx_DefineTextLoader(GFxLoadProcess* p, const GFxTagInfo& tagInfo)  // Read a DefineText tag.
{
    ...
    GPtr<GFxStaticTextCharacterDef>   pch = *new GFxStaticTextCharacterDef(p->GetDataDef());
