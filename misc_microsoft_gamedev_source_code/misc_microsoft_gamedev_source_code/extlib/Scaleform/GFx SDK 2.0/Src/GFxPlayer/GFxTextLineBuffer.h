...
struct GFxTextImageDesc : public GRefCountBase<GFxTextImageDesc>
{
    ...
    GPtr<GFxShapeCharacterDef>  pImageShape;
...
class GFxTextLineHighlighterIterator
{
public:
...
public:
    ...
    public:
        ...
        UInt GetFontSize() const { return (LenAndFontSize >> Shift_FontSize) & Mask_FontSize; }
    ...
    class GlyphIterator
    {
        ...
        GFxTextLineHighlighterIterator* pHighlighter;
        UInt32                          Color;
        ...
        void UpdateDesc();
    public:
        GlyphIterator(void* pglyphs, UInt glyphsCount, GFxFormatDataEntry* pfmtData, GFxTextLineHighlighterIterator* phighlighter = NULL) : 
            pHighlighter(phighlighter), Color(0)
        {
    ...
    struct LineData8
    {
        ...
        void  SetLeading(SInt leading) { GASSERT(leading >= -256 && leading < 256); Leading = (SInt8)leading; }
        SInt  GetLeading() const       { return SInt(Leading); }
        SInt  GetNonNegLeading() const { return Leading > 0 ? SInt(Leading) : 0; }
...
public:
    ...
    public:
        Iterator():pLineBuffer(NULL), CurrentPos(0), YOffset(0), StaticText(false) {}
        Iterator(const Iterator& it):
            pLineBuffer(it.pLineBuffer), CurrentPos(it.CurrentPos), YOffset(it.YOffset), 
            StaticText(it.StaticText) {}

        bool IsFinished() const 
        { 
            return !pLineBuffer || CurrentPos >= pLineBuffer->Lines.size() || CurrentPos < 0; 
        }
        ...
        UInt GetIndex() const { return (CurrentPos >= 0) ? UInt(CurrentPos) : ~0u; }
    };
    ...
    Iterator End()                          { return Iterator(*this, (UInt)Lines.size() - 1); }
    ...
    ConstIterator End() const               { return ConstIterator(*this, (UInt)Lines.size() - 1); }
    ...
    void Display(GFxDisplayContext &context, const Matrix& mat, const Cxform& cx, bool nextFrame, const GFxTextFieldParam& param);
...
public:
    ...
    GTL::garray<GFxFillStyle>   DummyStyle; // used to pass a color on to ShapeCharacter::Display()
    GTL::garray<GFxLineStyle>   DummyLineStyle;
    ...
    GPtr<GFxDrawingContext>     pBackgroundDrawing, pUnderlineDrawing;
