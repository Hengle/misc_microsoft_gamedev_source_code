...
struct GAtomicOpsRaw_4ByteImpl : public GAtomicOpsRawBase
{
    ...
    static inline UInt32   Exchange_NoSync(UInt32 *i, UInt32 j)
    {
        GASSERT(0);
        int dummy, ret;
        ...
        asm("1:\n\t"
            ...
            "stwcx. %[o],0,%[i]\n\t"
            "bne-   1b\n"
            : "+m" (*i), [r] "=b" (ret), [o] "=r" (dummy) : [i] "b" (i), [j] "b" (j));

        return ret;
    }

    static inline UInt32   ExchangeAdd_NoSync(UInt32 *i, UInt32 j)
    {
        int dummy, ret;
    ...
    static inline bool     CompareAndSet_NoSync(UInt32 *i, UInt32 c, UInt32 value)
    {
        int ret, dummy;
        ...
        asm("1:\n\t"
            ...
            "bne-   1b\n\t"
            "2:\n"
            : "+m" (*i), [r] "=b" (ret), [o] "=r" (dummy) : [i] "b" (i), [cmp] "b" (c), [val] "b" (value));
...
public:
    ...
    inline void Lock()
    {
        ...
        asm("1:\n\t"
            ...
            "bne-   1b\n\t"
            "isync\n"
            : "+m" (lock), [r] "=b" (dummy) : [n] "r" (1), [l] "b" (&lock));
    }
