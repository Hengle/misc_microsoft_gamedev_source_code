...
void GMutexImpl::Unlock(GMutex* pmutex)
{
    ...
    if ((Recursive ? ReleaseMutex(hMutexOrSemaphore) : ReleaseSemaphore(hMutexOrSemaphore, 1, NULL))  != 0)
    {
        if (lockCount == 0)
...
{
    GRefCountBase<GWaitable>::SetRefCountMode(GFC_REFCOUNT_THREADSAFE);
...
bool    GWaitConditionImpl::Wait(UInt delay)
{
    ...
    // Create a new event and add it to the queue pool
    WaitQueueLoc.Lock();
    QueuePush(pentry = GetNewEvent());
    WaitQueueLoc.Unlock();

    // Block and wait on the event
    DWORD waitResult = ::WaitForSingleObject(pentry->hEvent, (delay == GFC_WAIT_INFINITE) ? INFINITE : delay);
...
bool    GWaitConditionImpl::Wait(GMutex *pmutex, UInt delay)
{
    ...
    // However, if notify() comes in at this point in the other thread it will set our corresponding event
    // so wait will just fall through, as expected
...
DWORD WINAPI GThread_Win32StartFn(void * phandle)
{
...
bool    GThread::Start(ThreadState initialState)
{
    ...
    ThreadHandle    = ::CreateThread(0, 0, GThread_Win32StartFn, this, 0, (DWORD*) &threadId);
