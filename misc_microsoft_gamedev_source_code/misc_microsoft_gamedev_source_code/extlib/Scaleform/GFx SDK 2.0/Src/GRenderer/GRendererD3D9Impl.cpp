...
Filename    :   GRendererD3D9Impl.cpp
Content     :   Direct3D9 Sample renderer implementation
Created     :   
...
// ***** Classes implemented
class GRendererD3D9Impl;
class GTextureD3D9Impl;
...
// ***** GTextureD3D9Impl implementation


// GTextureD3D9Impl declaration
class GTextureD3D9Impl : public GTextureD3D9
{
...
public:
    ...
    // Renderer 
    GRendererD3D9Impl*  pRenderer;
    SInt                Width, Height;

    // D3D9 Texture pointer
    IDirect3DTexture9*  pD3DTexture;

    GTextureD3D9Impl(GRendererD3D9Impl *prenderer);
    ~GTextureD3D9Impl();
...
private:
    ...
    GPtr<IDirect3DVertexBuffer9>    pVertexBuffer;
    GPtr<IDirect3DIndexBuffer9>     pIndexBuffer;
    // Allocated sizes
    ...
    if (!pVertexBuffer)
    {
        ...
        do
        {
            createResult = pDevice->CreateVertexBuffer(
                                    ...
                                    0, D3DPOOL_DEFAULT, &pVertexBuffer.GetRawRef(), NULL );
            ...
            {                
                ...
                {
                    GFC_DEBUG_WARNING(1, "GRendererD3D9 - Failed to create dynamic vertex buffer");
                    VertexBufferSize = 0;
                    return 0;
                }
            }
    ...
    if (!pIndexBuffer)
    {
        ...
        do
        {
            ...
            if (FAILED(createResult))
            {
                ...
                {
                    GFC_DEBUG_WARNING(1, "GRendererD3D9 - Failed to create dynamic index buffer");
...
void    GDynamicVertexStream::DrawTriangles()
{
    ...
    else if (RenderMethod == RM_NotIndexed)
    {
        ...
        while(triangles < Primitive.TriangleCount)
        {
            ...
            pDevice->DrawPrimitive(D3DPT_TRIANGLELIST, 0, batch);
            triangles += batch;
        }
    }
...
void*   GDynamicVertexStream::LockVertexBuffer(UInt vertexCount, UInt vertexSize)
{
    ...
    if ((VertexBufferSize - NextVBOffset) > size)
    {
        offset          = NextVBOffset;
        NextVBOffset    = NextVBOffset + size;
    ...
    if (::IsBadWritePtr(pbuffer, size))
    {
        ...
        GFC_DEBUG_WARNING(!warningReported,
                          "GD3D9Renderer::LockVertexData - Direct3D Lock returned bad pointer, function failed");
        warningReported = 1;
...
// ***** GRendererD3D9 Implementation

class GRendererD3D9Impl : public GRendererD3D9
{
public:
    ...
    SInt                    TempW, TempH;
    GPtr<IDirect3DTexture9> pTempTextureA, pTempTextureLA;
    GPtr<IDirect3DSurface9> pTempSurfaceA, pTempSurfaceLA;
    ...
    ~GRendererD3D9Impl()
    {
        ...
        while (Textures.pFirst != &Textures)
            ((GTextureD3D9Impl*)Textures.pFirst)->RemoveFromRenderer();
    ...
    bool    CreateVertexDeclaration(GPtr<IDirect3DVertexDeclaration9> *pdeclPtr, const D3DVERTEXELEMENT9 *pvertexElements)
    {
        if (!*pdeclPtr)
        {
            ...
            {
                GFC_DEBUG_WARNING(1, "GRendererD3D9 - failed to create vertex declaration");
                return 0;
            }
        }
        return 1;
    ...
    bool    CreateVertexShader(GPtr<IDirect3DVertexShader9> *pshaderPtr, const char* pshaderText)
    {
        if (!*pshaderPtr)
        {
            ...
            {
                GFC_DEBUG_WARNING1(1, "GRendererD3D9 - Can't create D3D9 vshader; error code = %d", hr);
                return 0;
            }
        }
        return 1;
    ...
    bool    InitShaders(D3DCAPS9 *caps9)
    {
        bool success = 1;
        UInt i;
        UInt vstart = 0;

        if ((caps9->DeclTypes & D3DDTCAPS_UBYTE4N) == 0)
            vstart = 1;
    ...
    void    ApplyFillTexture(const FillTexture &fill, UInt stageIndex, UInt matrixVertexConstIndex)
    {
        ...
        GTextureD3D9Impl* ptexture = ((GTextureD3D9Impl*)fill.pTexture);
    ...
    public:
        ...
        void    Apply(GRendererD3D9Impl *prenderer) const
        {
        ...
        bool    NeedsSecondPass(GRendererD3D9Impl *prenderer) const
        {
        ...
        void    ApplySecondPass(GRendererD3D9Impl* prenderer) const
        {
    ...
    {
        return new GTextureD3D9Impl(this);
    }
    ...
    bool        GetRenderCaps(RenderCaps *pcaps)
    {
        if (!ModeSet)
        {
            GFC_DEBUG_WARNING(1, "GRendererD3D9::GetRenderCaps fails - video mode not set");
            return 0;
        }
        ...
        //if (caps9.MaxVertexIndex > 0xFFFF)
        //    pcaps->CapBits |= Cap_Index32;
        
        if (UsePixelShaders)
        {
            GASSERT((caps9.PixelShaderVersion & 0xFFFF) >= 0x0101);
            pcaps->CapBits |= Cap_CxformAdd;
            pcaps->CapBits |= Cap_FillGouraudTex;
        }
      
        if (caps9.StencilCaps & (D3DSTENCILCAPS_INCR|D3DSTENCILCAPS_INCRSAT))
        {
...

    {
        ...
        surfaceDesc.Height= vpin.Height;
        {
            ...
            pDevice->GetBackBuffer(0,0, D3DBACKBUFFER_TYPE_MONO, &psurface.GetRawRef());
            if (psurface)
    ...
    void    ApplyBlendMode(BlendType mode, bool premultiplyFill = 0)
    {
        ...
        static BlendModeDescAlpha acmodes[15] =
        {
            ...
            { D3DBLENDOP_ADD, D3DBLEND_ONE, D3DBLEND_ONE, D3DBLEND_DESTALPHA, D3DBLEND_ONE },             // Add
            { D3DBLENDOP_REVSUBTRACT, D3DBLEND_ONE, D3DBLEND_ONE, D3DBLEND_DESTALPHA, D3DBLEND_ONE },     // Subtract
    ...
    void    ApplyMatrix(const Matrix& matIn)
    {
        ...
        pDevice->SetVertexShaderConstantF(0, &mrows[0][0], 4);
    }
    ...
    {       
        ...
        {
            GFC_DEBUG_WARNING(!VertexStream.HasVertexData(), "GRendererD3D9::DrawIndexedTriList failed, vertex data not specified");
            GFC_DEBUG_WARNING(!VertexStream.HasIndexData(), "GRendererD3D9::DrawIndexedTriList failed, index data not specified");
            GFC_DEBUG_WARNING((VertexStream.GetD3DIndexFormat() == D3DFMT_UNKNOWN),
                              "GRendererD3D9::DrawIndexedTriList failed, index buffer format not specified");
            return;
        }
    ...
    void    DrawLineStrip(int baseVertexIndex, int lineCount)
    {
        ...
        {
            GFC_DEBUG_WARNING(1, "GRendererD3D9::DrawLineStrip failed, vertex data not specified");
            return;
        }
        ...
        if (!VertexStream.InitVertexBufferData(baseVertexIndex, lineCount + 1))
            return;
        pDevice->DrawPrimitive(D3DPT_LINESTRIP, 0, lineCount);
                        ...
                        CacheProvider *pcache)
    {
        ...
        GTextureD3D9Impl* ptexture = (GTextureD3D9Impl*)pti;
        // Texture must be valid for rendering
        if (!ptexture->pD3DTexture && !ptexture->CallRecreate())
        {
            GFC_DEBUG_WARNING(1, "GRendererD3D9::DrawBitmaps failed, empty texture specified/could not recreate texture");
            return;     
        }
        ...
        while (ibitmap < count)
        {
            ...
            if (ivertex)
            {                                     
                pDevice->DrawPrimitive(D3DPT_TRIANGLELIST, 0, ivertex / 3);
                RenderStats.Primitives++;
            }
        }
    ...
    // Defines all D3D9 color channels for D3DRS_COLORWRITEENABLE
                                ...
                                HWND hwnd)
    {
        ...
        if ((caps9.PixelShaderVersion & 0xFFFF) >= GRENDERER_SHADER_VERSION)        
            UsePixelShaders = 1;

        // Detect stencil op.
        if (caps9.StencilCaps & D3DSTENCILCAPS_INCR)
        {
            StencilOpInc = D3DSTENCILOP_INCR;
            StencilOpDec = D3DSTENCILOP_DECR;
        }
        else if (caps9.StencilCaps & D3DSTENCILCAPS_INCRSAT)
        {
        ...
        if (!InitShaders(&caps9) || !VertexStream.Initialize(pd3dDevice))
        {
            ReleaseShaders();
            pDevice->Release();
            pDevice = 0;
    ...
    virtual bool                ResetVideoMode()
    {
        ...
        GTextureD3D9Impl* ptexture = (GTextureD3D9Impl*)Textures.pFirst;
        while (ptexture != &Textures)
        {
			// Save texture in case CallHandlers clear them out.
			GPtr<GTextureD3D9Impl> ptextSave = ptexture;			
            ...
            ptexture = (GTextureD3D9Impl*)ptexture->pNext;
        }
    ...
    virtual DisplayStatus       CheckDisplayStatus() const
    {
        ...
        while (ptexture != &Textures)
        {
            ...
            GTextureD3D9Impl* pnext = (GTextureD3D9Impl*)ptexture->pNext;
            ptexture->AddRef();
...
GTextureD3D9Impl::GTextureD3D9Impl(GRendererD3D9Impl *prenderer)
...
GTextureD3D9Impl::~GTextureD3D9Impl()
{
    if (pD3DTexture)
        pD3DTexture->Release(); 
}
...
// Obtains the renderer that create TextureInfo 
GRenderer*  GTextureD3D9Impl::GetRenderer() const
    { return pRenderer; }
bool        GTextureD3D9Impl::IsDataValid() const
    { return (pD3DTexture != 0);    }


// Remove texture from renderer, notifies renderer destruction
void    GTextureD3D9Impl::RemoveFromRenderer()
{
    if (pD3DTexture)
    {
        pD3DTexture->Release();
...
bool GTextureD3D9Impl::InitTexture(IDirect3DTexture9 *ptex, SInt width, SInt height)
{
...
bool GTextureD3D9Impl::InitTexture(GImageBase* pim, int targetWidth, int targetHeight)
{   
    if (!pRenderer || !pRenderer->pDevice)
        return 0;
    ...
    // Set the actual data.
    if (!pim->IsDataCompressed() && (w != pim->Width || h != pim->Height || pim->Format == GImage::Image_RGB_888))
    {
    ...
    else
    {
        ...
        {           
            ...
            result = D3DXLoadSurfaceFromMemory(
                        psurface, NULL, NULL,
                        psourceImage->pData,
                        (bytesPerPixel == 1) ? D3DFMT_A8 : D3DFMT_A8B8G8R8, // NOTE: format order conversion from GL
            ...
            if (level< (levelsNeeded-1))
            {
                ...
                GRendererD3D9Impl::MakeNextMiplevel((int*) &sourceRect.right, (int*) &sourceRect.bottom,
                                                    psourceImage->pData);
            }
        }
    }    
...
bool GTextureD3D9Impl::InitTexture(int width, int height, GImage::ImageFormat format, int mipmaps,
                                  int targetWidth , int targetHeight)
{
    if (!pRenderer || !pRenderer->pDevice)
        return 0;
...
void GTextureD3D9Impl::Update(int level, int n, const UpdateRect *rects, const GImageBase *pim)
{
    UInt     bytesPerPixel;
    ...
    if (TextureFormat == D3DFMT_A8)
    {
        if (!pRenderer->pTempTextureA)
        {
            pRenderer->pDevice->CreateTexture(pRenderer->TempW, pRenderer->TempH,
                1, 0, D3DFMT_A8, D3DPOOL_SYSTEMMEM, &pRenderer->pTempTextureA.GetRawRef(), 0);
    ...
    else if (TextureFormat == D3DFMT_A8L8)
    {
        ...
        {
            pRenderer->pDevice->CreateTexture(pRenderer->TempW, pRenderer->TempH,
                1, 0, D3DFMT_A8L8, D3DPOOL_SYSTEMMEM, &pRenderer->pTempTextureLA.GetRawRef(), 0);
    ...
    {
        GFC_DEBUG_WARNING(1, "GTextureD3D9::Update failed, could not recreate texture");
        return;     
    }
...
bool GTextureD3D9Impl::InitTextureFromFile(const char* pfilename, int targetWidth, int targetHeight)
{   
    if (!pRenderer || !pRenderer->pDevice)
        return 0;
    ...
    if (result != S_OK)
    {
        GFC_DEBUG_ERROR1(1, "GTextureD3D9 - Can't create texture from file %s", pfilename);
        Width = Height = 0;
        return 0;
    }
...
// Factory.
GRendererD3D9*  GRendererD3D9::CreateRenderer()
{
    return new GRendererD3D9Impl;
}
