...
namespace GMath2D
{
    ...
    static GINLINE void CalcParallel(Vertex* v1, Vertex* v2,
                                     GCoordType d, GCoordType w)
    {
        GCoordType dx2 = w * (v1->y - v2->y) / d;
        GCoordType dy2 = w * (v2->x - v1->x) / d;
        v1->x -= dx2;
        v1->y -= dy2;
        v2->x -= dx2;
        v2->y -= dy2;
...
//------------------------------------------------------------------------
void GEdgeAA::calcIntersectionPoint(GCoordType width, GCoordType miterLimit,
                                    ...
                                    GCoordType* x, GCoordType* y) const
{
    ...
    GCoordType dx1 = fabsf(v1.x - v2.x);
    GCoordType dy1 = fabsf(v1.y - v2.y);
    GCoordType dx2 = fabsf(v3.x - v4.x);
    GCoordType dy2 = fabsf(v3.y - v4.y);
    GCoordType epsilon = (dx1 + dy1 + dx2 + dy2) * G_IntersectionEpsilonAA;

    GMath2D::CalcParallel(&v1, &v2, width);
    GMath2D::CalcParallel(&v3, &v4, width);

    if(GMath2D::CalcIntersection(v1, v2, v3, v4, x, y, epsilon))
    {
        GCoordType dist = GMath2D::CalcDistance(*x, *y, vx, vy);
        GCoordType lim  = GCoordType(fabs(width)) * miterLimit;
        if(dist > lim)
        {
        ...
        for(j = 0; j < 3; j++)
        {
            if((tri.edgeStat[j] & EdgeModified) == 0)
            {
                   ...
                   MeshTriangles[FanEdges.last() >> 2].style)
                {
                    ...
                    // It protects us from using improper startng edges,
                ...
                if(!cyclic)
                {
                    ...
                    calcIntersectionPoint(-width,
                                          GetIntersectionMiterLimit(),
                                          0,
                                          FanEdges.size() - 1,
                                          &x, &y);
                ...
                if(aaMethod != AA_OuterEdges)
                {
                    if(aaMethod == AA_AllEdges)
                    {
                        ...
                        for(start = 0; start < FanEdges.size(); )
                        {
                            ...
                            calcIntersectionPoint(width, GetIntersectionMiterLimit(),
                                                  start, end - 1, &x, &y);
                            newVerIdx = Vertices.size();
                            Vertices.add(VertexType(x, y));
                            TmpStarVer.add(newVerIdx);
