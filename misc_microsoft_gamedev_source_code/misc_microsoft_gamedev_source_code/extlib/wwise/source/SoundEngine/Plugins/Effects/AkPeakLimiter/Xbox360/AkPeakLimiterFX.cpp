...
//-----------------------------------------------------------------------------
void CAkPeakLimiterFX::Execute(	AkAudioBuffer* io_pBuffer )		// Input buffer interface.
{
	AkPeakLimiterFXParams Params;
	m_pSharedParams->GetParams( &Params );
	...
	if ( bPreStop )
	{
		...
		// Zero out invalid frames between uValidFrames and uMaxFrames (make it valid before we will use it).
