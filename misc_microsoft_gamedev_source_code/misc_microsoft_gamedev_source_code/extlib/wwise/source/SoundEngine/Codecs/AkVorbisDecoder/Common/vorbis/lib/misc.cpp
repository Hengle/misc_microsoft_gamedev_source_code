...
#error Not used: do not include in project
...
extern void *_VDBG_malloc(void *ptr,long bytes,char *file,long line){
  ...
  if(charptr){
    charptr-=HEAD_ALIGN;
    _ripremove(charptr);
    ptr=realloc(charptr,bytes);
  }else{
    charptr=(char*)malloc(bytes);
    memset(charptr,0,bytes);
  }
  return _insert(charptr,bytes,file,line);
}
