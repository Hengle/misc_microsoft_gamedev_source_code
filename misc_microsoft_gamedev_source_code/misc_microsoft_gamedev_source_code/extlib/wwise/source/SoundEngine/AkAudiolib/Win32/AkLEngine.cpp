...
{
	__m128 vTmp = _mm_load_ps1( &in_fControlValue ); 
	out_Volumes.vector[0] = _mm_mul_ps( in_Volumes.vector[0], vTmp );
	out_Volumes.vector[1] = _mm_mul_ps( in_Volumes.vector[1], vTmp );
}
...
AKRESULT CAkLEngine::Init()
{
	...
	// find out if we have SSE2 available
	m_bSSE2 = _set_SSE2_enable(1) != 0;
...
AkUInt32 CAkLEngine::GetNumBufferNeededAndSubmit()
{
	...
	{
		// Failure in IsDataNeeded means that the output device broke/disappeared. Switch to a dummy device
		// to avoid further problems.

		g_pAkSink->Term();
		AkDelete( g_LEngineDefaultPoolId, g_pAkSink );

		g_pAkSink = CAkSink::Create( g_PDSettings, true );
		AKASSERT( g_pAkSink );
...
/*	//code to be enabled later on if we want to display efective volume.
					//include positionning volume
					AkSpeakerVolumes l_PosVolumes;
					if( pVPLSrc->m_Src.m_Param.ePosType == Ak3DUserDef || pVPLSrc->m_Src.m_Param.ePosType == Ak3DGameDef )
					{
					CAkListener::Get3DVolumes( pCtx, &( pVPLSrc->m_Src.m_Param ), &l_PosVolumes );
					//Using the max speaker output.
					AkVolumeValue PosVolume = AkMath::Max( l_PosVolumes.fCenter, l_PosVolumes.fFrontLeft );
					PosVolume = AkMath::Max( PosVolume, l_PosVolumes.fFrontRight );
					PosVolume = AkMath::Max( PosVolume, l_PosVolumes.fRearLeft );
					PosVolume = AkMath::Max( PosVolume, l_PosVolumes.fRearRight );

					out_pPipelineData->fVolume += PosVolume;
					}
*/
...
// Query hardware.
bool CAkLEngine::IsHardwareDevicePresent()
{
	return g_pAkSink ? g_pAkSink->IsHardwareDevicePresent() : false;
}
