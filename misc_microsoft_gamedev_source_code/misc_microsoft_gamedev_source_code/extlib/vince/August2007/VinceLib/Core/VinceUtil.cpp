...
namespace Vince
{
  ...
  //DmGetXboxName is a debug-only function
	...
	//-----------------------------------------------------------------------------
	// Trace()
	// Utility routine to write formatted strings to debug window
	//
	// Desc: Should only be used for debugging purposes.
	//-----------------------------------------------------------------------------
    void Trace(const char* szMessage, ...)
    {
        const int MAX_DEBUG_OUT_BUFFER = 500;
	    char szBuffer[MAX_DEBUG_OUT_BUFFER];
	    va_list	args;

	    // Use the variable argument list to print to a buffer, then output to the debug monitor
	    va_start(args, szMessage);
	    vsprintf_s(szBuffer, MAX_DEBUG_OUT_BUFFER, szMessage, args);
