...
namespace Vince
{
	...
	const char* GetVinceRoot()
	{
		...
		{
			strncpy( buffer + iLastSlash + 1, "Vince\\", 6);
	...
	const char* GetVinceAppRoot()
	{
		...
		const char* cstrAppDataPath = getenv("APPDATA");
		if ( NULL == cstrAppDataPath)
		{
			buffer[0] = '\0';
		}
		else
		{
			strncpy( buffer, cstrAppDataPath, MAX_PATH - 24);
			buffer[MAX_PATH - 24] = '\0';
		}
		strncat( buffer, "\\Microsoft Games\\Vince\\", 23 );
		return SAFE_COPY(buffer);
#endif
	}
		...
		if( !fFullPath )
		{
			if (fOutput)
			{
				if ( NULL != VincePaths.DestinationFolder)
					strncpy(strFullname, VincePaths.DestinationFolder, MAX_PATH - 1);
			}
			else
			{
				if ( NULL != VincePaths.SourceFolder)
					strncpy(strFullname, VincePaths.SourceFolder, MAX_PATH - 1);
			}
			...
			if (length > 0)
			{
				if ( strFullname[length - 1] != '\\' )
					strncat(strFullname, "\\", 1);
			}
		}
		strncat(strFullname, cstrFilename, strlen(cstrFilename) );
	...
	FILE* VinceFileOpen(const char* cstrFilename, const char* cstrFlags, bool fOutput)
	{
		...
		FILE* fileResult = fopen(cstrFullFileName, cstrFlags);
		...
		if (!fileResult && fOutput)
		{
			...
			if ( 3 == dwError )		// Directory does not exist
			{
				...
				if ( S_OK == _mkdir(VincePaths.DestinationFolder) )
				{
					...
					fileResult = fopen(cstrFullFileName, cstrFlags);
				}
			}
		}
...
#ifdef _XBOX
		bool success = (XBDM_NOERR == DmGetXboxName(buffer, &dwBufLen));
#else
		bool success = (0 != GetComputerName(buffer, &dwBufLen));
#endif
		if (success)
		{
			return SAFE_COPY(buffer);
	...
	const char* GetCurrentDateTimeString()
	{
        ...
        _snprintf( strDateTime, 29, "%d/%d/%d %d:%2.2d:%2.2d", SystemTime.wMonth,
