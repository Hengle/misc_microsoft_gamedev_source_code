...
namespace Vince
{
        ...
        if (fControl) 
        {
            ...
            while ( bValidFile && !feof(fControl) ) 
            {
                ...
                else
                {
                    ...
                    {
                        int intResult = 0;
                        int retValue = sscanf( line, "%i", &intResult );
                        if ( 1 == retValue )
                        {
    ...
    const char* CLogControl::GetLogFileName()
    {
        ...
        _snprintf(buffer, MAX_PATH - 1, "%s_%s_%04d.log", m_cstrLogBaseName, m_cstrMachineName, m_sequenceNumber);
        
        // Compressed files have .cmp added to the name
// Compressed can not be true if NO_VINCE_COMPRESSION is defined, but just to be absolutely safe:
