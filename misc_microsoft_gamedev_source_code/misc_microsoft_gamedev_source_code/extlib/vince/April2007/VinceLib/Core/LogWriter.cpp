...
namespace Vince
{
    ...
    void CLogWriter::Initialize()
    {
        ...
        if (!m_fInitialized)
        {
            ...
            if ( m_fAutoUpload )
            {
                const char* project   = pSettings->Fetch("Project", "Unknown");
                const char* webServer = pSettings->Fetch("UploadWebServer", "");
                const char* aspxPage  = pSettings->Fetch("UploadaspxPage", "");
                int iPort             = pSettings->Fetch("WebPort", 80);
                bool bDeleteLog       = pSettings->Fetch("DeleteLogAfterUpload", false);
                bool bUseLSP          = pSettings->Fetch("UseLSP", false);
                DWORD titleID         = pSettings->Fetch("TitleID", 0x00000000);
    ...
    bool CLogWriter::IsUploading()
    {
       if ( !m_fLoggingActive )
         return false;

       return m_pLogControl->IsUploading();
    }
