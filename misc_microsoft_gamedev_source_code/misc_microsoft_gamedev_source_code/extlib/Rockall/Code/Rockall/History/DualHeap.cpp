...
DUAL_HEAP::DUAL_HEAP
	...
	//   private cache.
	//
	ActiveCaches = ((LIST*) ROCKALL::New( sizeof(LIST) ));
	FreeCaches = ((LIST*) ROCKALL::New( sizeof(LIST) ));
	Tls = ((TLS*) ROCKALL::New( sizeof(TLS) ));
	...
	if
		...
		//
		PLACEMENT_NEW( ActiveCaches,LIST );
		PLACEMENT_NEW( FreeCaches,LIST );
...
void DUAL_HEAP::CreateThreadCache( void )
	{
	...
	if ( ! FreeCaches -> EndOfList() )
		{
		//
		//   We have found a free one.
		//
		ThreadCache = ((THREAD_CACHE*) FreeCaches -> First());

		//
		//   Unlink it from the free list and put
		//   it back in the active list.
		//
		ThreadCache -> Delete( FreeCaches );
		ThreadCache -> Insert( ActiveCaches );
		}
...
void DUAL_HEAP::ActivateCacheStack( CACHE_STACK *CacheStack )
	{
	//
...
bool DUAL_HEAP::Delete( void *Address,int Size )
    {
	//
...
void DUAL_HEAP::DeleteAll( bool Recycle )
    {
	//
	//   Although it is very rare there is a chance
	//   that we failed to build the basic heap structures.
...
CACHE_STACK *DUAL_HEAP::FindCache( int Size,THREAD_CACHE *ThreadCache )
	{
	if ( Size <= MaxSize1 )
...
void DUAL_HEAP::FlushAllThreadCaches( void )
	{
	...
	for 
			( 
			Current = ((THREAD_CACHE*) ActiveCaches -> First());
			(Current != NULL);
...
void DUAL_HEAP::FlushThreadCache( THREAD_CACHE *ThreadCache )
	{
	//
...
void DUAL_HEAP::FlushCacheStack( CACHE_STACK *CacheStack )
    {
	//
...
void *DUAL_HEAP::New( int Size,int *Space,bool Zero )
    {
	//
	//   Although it is very rare there is a chance
	//   that we failed to build the basic heap structures.
...
bool DUAL_HEAP::SearchAllThreadCaches( void *Address,int Size )
	{
...
bool DUAL_HEAP::SearchThreadCache
		( 
		void						  *Address,
		int							  Size,
...
bool DUAL_HEAP::SearchCacheStack( void *Address,CACHE_STACK *CacheStack )
    {
	//
	//   There is a chance that this cache is not active. 
	//   If so we skip the cache flush.
...
bool DUAL_HEAP::Truncate( int MaxFreeSpace )
    {
	//
	//   Although it is very rare there is a chance
	//   that we failed to build the basic heap structures.
...
bool DUAL_HEAP::Verify( void *Address,int *Space )
    {
	AUTO int Size;
...
bool DUAL_HEAP::Walk( bool *Activity,void **Address,int *Space )
    {
	//
	//   Walk the heap.
	//
...
void DUAL_HEAP::DeleteThreadCache( void )
	{
	REGISTER THREAD_CACHE *ThreadCache = 
		((THREAD_CACHE*) Tls -> GetPointer());
	...
	if ( ThreadCache != NULL )
		...
		ThreadCache -> Delete( ActiveCaches );
		ThreadCache -> Insert( FreeCaches );
...
DUAL_HEAP::~DUAL_HEAP( void )
	...
	if ( Active )
		...
		PLACEMENT_DELETE( FreeCaches,LIST );
		PLACEMENT_DELETE( ActiveCaches,LIST );

		//
		//   Delete the space.
		//
		ROCKALL::Delete( Tls );
		ROCKALL::Delete( FreeCaches );
		ROCKALL::Delete( ActiveCaches );
