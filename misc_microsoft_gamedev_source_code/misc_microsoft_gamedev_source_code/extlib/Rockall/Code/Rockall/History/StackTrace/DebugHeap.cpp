                ...
                ThreadSafe)
{
    ...
    //
    // since walking back the stack is a costly operation we will enable
    // stack tracing mechanism based on an env. var CSReportLeaks = 1
    //
    char
        lpCSReportLeaks[] = "CSReportLeaks",
        lpValue[4] = "\0";

    m_bReportLeaks = false;
    //
    // get the env var
    //
    if (GetEnvironmentVariableA(lpCSReportLeaks, lpValue, 4))
    {
        if (atol(lpValue) == 1)
            m_bReportLeaks = true;
    }
    ...
    {
        //
        // load the symbols for the current application
        // to be able to report the stack traces for the leaks
        // TODO: we should allow the user to set the symbols path
        // TODO: this way the user can forget to always set the 
        // TODO: _NT_SYMBOL_PATH environment var
        //
        _ReloadSymbols(NULL);
    }
    //
    ...
    //   is not corrupt.
    //
    if ((MaxFreeSpace < 0) || ROCKALL::Corrupt())
    {
...
{
    AUTO DEBUG_HEADER *Header = (Address == (void*)AllocationFailure)
...
void DEBUG_HEAP::HeapLeaks(void)
{
    ...
    AUTO void *Address = NULL;
    AUTO int Space;
    AUTO DEBUG_HEADER *Header = NULL;
    LPSTR
        bufCallStack[MAXSTACKFRAMES];
    DWORD
        cCount,
        cFrames;
    bool
        bError = false;
    //
    // initialize the bufCallStack[] array
    //
    if (m_bReportLeaks)
    {
        ZeroMemory(bufCallStack, sizeof(LPSTR) * MAXSTACKFRAMES);
        for (cCount = 0; cCount < MAXSTACKFRAMES; cCount++)
        {
            bufCallStack[cCount] = (LPSTR)malloc(SYMNAMELENGTH);
            if (!bufCallStack[cCount])
                bError = true;
        }
    }
    //
    //   Walk the heap and find all the active and
    //   available space.  We would normally expect
    ...
    while (ROCKALL::Walk(&Active, &Address, &Space))
    {
        ...
        if (Active)
        {
            ...
            //
            // now add the call stack but only for the active allocations
            // and not for the free space since there's no call stack in this case
            //
            if (Active && m_bReportLeaks)
            {
                if (!bError)
                {
                    Header = (DEBUG_HEADER*)Address;
                    cFrames = FormatCallStackA(Header->rgStack,
                                               bufCallStack,
                                               SYMNAMELENGTH,
                                               MAXSTACKFRAMES);
                    //
                    //   Write the string to the debug window.
                    //
                    for (cCount = 0; cCount < (int)cFrames; cCount++)
                    {
                        OutputDebugStringA(bufCallStack[cCount]);
                        OutputDebugStringA("\n");
                    }
                }
            }
...
{
    REGISTER void *Address = (void*)AllocationFailure; //NULL
    //
    ...
    {
        int
            size_Debug_Guard = sizeof(DEBUG_GUARD),
            size_Debug_Header = sizeof(DEBUG_HEADER),
            size_Debug_Trailer = sizeof(DEBUG_TRAILER);

        AUTO int TotalSize;
        REGISTER int NewSize = (Size + sizeof(DEBUG_GUARD) + GuardMask) & ~GuardMask;
        ...
        if (Address != (void*)AllocationFailure) 
        {
            ...
            //   of guard words.
            //
            SetGuardWords((DEBUG_HEADER*)Address,
                          (Space == NULL) ? Size : *Space, 
                          TotalSize);
            //
            //   Compute the external address and place
            ...
            if (Zero)
            { 
                ZeroMemory(Address, (Space == NULL) ? Size : *Space); 
            } 
        }
        //
        //   We ensure that the heap has not become corrupt
...
void DEBUG_HEAP::SetGuardWords(DEBUG_HEADER *Header, int Size, int TotalSize)
{
       ...
       )
   {
        ...
        {
            DWORD
                cFilled = GetCallStack(Header->rgStack, MAXSTACKFRAMES - 1, 3);
            Header->rgStack[cFilled] = NULL;
        }
    }
    else
...
void DEBUG_HEAP::TestGuardWords(DEBUG_HEADER *Header, int TotalSize)
{
        ...
        )
    {
        ...
        for (Count = Header->Size; (Count & GuardMask) != 0; Count ++)
        {
            ...
            //
            if (Trailer->MidGuard[ByteIndex] != ((char*)&GuardValue)[ByteIndex])
            {
