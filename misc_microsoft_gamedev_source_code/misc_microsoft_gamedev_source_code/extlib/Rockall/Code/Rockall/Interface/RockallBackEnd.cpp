...
int ROCKALL_BACK_END::NaturalSize( void )
    {
		return ENVIRONMENT::AllocationSize();
	}
...
void *ROCKALL_BACK_END::NewArea( int AlignMask,int Size,bool User )
	...
	if ( AlignMask > NaturalSize() )
		{
		assert((AlignMask + Size)>0 && (AlignMask + Size)>=Size); //PREfast wants to ensure things are right
		...
		if ( NewMemory != NULL )
			...
			//  information.
			//  
			DebugPrint( "New memory allocated at\t 0x%08x of %d bytes (by %s request)\n",
