...
CONST SBIT32 DebugBufferSize		  = 4096;
...
DEBUG_HEAP::DEBUG_HEAP
	...
	if ( FunctionTrace )
		{
			DEBUG_HEAP::EnableFunctionTrace();
		}
	else
		{ CallStack = NULL; }
	...
	//   raise an exception when there is an error.
	//
    ...
    /********************************************************************/
    /*                                                                  */
    /*   Enable CallStack gathering.                                    */
    /*                                                                  */
    /*   Turn on the call stacks collection within the heap.            */
    /*                                                                  */
    /********************************************************************/

void DEBUG_HEAP::EnableFunctionTrace(void)
	{
		//
		//   Multiple calls to this function will not do anything
		//
		if (CallStack) return;
		//
		...
		if ( Active )
			...
			if ( CallStack != NULL )
				...
				if ( Header -> Count > 0 )
					...
					// Modules
					CallStack->ModuleList(Buffer, DebugBufferSize);
					DebugPrint("%s",Buffer);
					}
				else
					{
...
bool DEBUG_HEAP::NewGuard( void **Address,int Size,int *Space )
	...
	AUTO HEADER *Header = 		((HEADER*) ROCKALL_FRONT_END::New( MinimumSize,& ActualSize,false ));
...
bool DEBUG_HEAP::VerifyGuard( void *Address,int *Size,int *Space )
    ...
    /*                                                                  */
    /*   We need to verify the guard words at various times to ensure   */
...
bool DEBUG_HEAP::VerifyGuardWords( void *Address,int Size )
	...
	if ( Size2 > 0 )
		{
#ifdef BIG_ENDIAN
		REGISTER SBIT32 Mask =  ((1 << (Size2 * 8)) - 1);
#else
...
bool DEBUG_HEAP::VerifyHeader
	...
	if ( VerifyAddress( Address,Header,Space,Verify ) )
		...
		if
			...
			WriteGuardWords( *Header,sizeof(HEADER) );
...
bool DEBUG_HEAP::VerifyTrailer
	...
	if ( ! VerifyGuardWords( ((void*) (*Trailer)),Size ) )
		...
		WriteGuardWords( *Trailer,Size ); //aleksger
		}
...
void DEBUG_HEAP::UserError( void *Address,void *Details,const char *Message )
	{
