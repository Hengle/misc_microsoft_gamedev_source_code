...
PAGE_HEAP::PAGE_HEAP
	...
	if ( FunctionTrace )
		...
		//   will call traces.
		//
			CallStack = ((CALL_STACK*) ROCKALL_FRONT_END::SpecialNew( sizeof(CALL_STACK) ));
...
bool PAGE_HEAP::VerifyHeader
	...
	if ( VerifyAddress( Address,Header,Space,Verify ) )
		...
		if
			...
			REGISTER int GuardAreaSize = (NumberOfGuardWords * sizeof(GuardWords));
			REGISTER int Size = (HeaderSize + GuardAreaSize - sizeof(HEADER));
			...
			WriteGuardWords( *Header,sizeof(HEADER) ); //aleksger
...
void PAGE_HEAP::UserError( void *Address,void *Details, const char *Message )
	{
