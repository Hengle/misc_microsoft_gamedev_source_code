...
ROCKALL_FRONT_END::ROCKALL_FRONT_END
	...
	TRY
		...
		this->ThreadSafeFlag = ThreadSafeFlag;
		...
		if 
			...
			if ( NewMemory != NULL )
				...
				//aleksger - Moved the caches to later area to ensure proper alignment
		...
		ROCKALL_FRONT_END::Exception( ((char*) Message) );
		}
	catch ( ... )
		{
		//
...
bool ROCKALL_FRONT_END::Delete( void *Address,int Size )
	...
	TRY
		{
		if (!ThreadSafeFlag) { assert((OwnerThread == 0) || (GetCurrentThreadId() == OwnerThread)); }
		//
...
void ROCKALL_FRONT_END::Exception(CONST char *Message )
	{ /* void */ }
...
ROCKALL_FRONT_END::~ROCKALL_FRONT_END( void )
	...
	TRY
		...
		if ( (Available()) && (NumberOfCaches > 0) && (TotalSize > 0) )
			...
			//
			//aleksger - The area begins where ThreadSafe starts
			RockallBackEnd -> DeleteArea( ((VOID*) ThreadSafe),TotalSize,False );
...
void ROCKALL_FRONT_END::PrintMemStatistics( void )
{
	Heap->PrintHeapUsagePattern();
}
