...
DYNAMIC_DEBUG_HEAP::DYNAMIC_DEBUG_HEAP
	...
	HeapArray = ((DYNAMIC_HEAP*) SMALL_HEAP::New( (sizeof(DYNAMIC_HEAP) * 3) ));
	HeapWalk = NULL;
	...
	//
	if (  (AllHeaps != NULL) && (HeapArray != NULL)) 
		{
		//
		...
		//
		PLACEMENT_NEW( & HeapArray[0],DYNAMIC_HEAP );
		PLACEMENT_NEW( & HeapArray[1],DYNAMIC_HEAP );
		PLACEMENT_NEW( & HeapArray[2],DYNAMIC_HEAP );

		//
		//   Setup the heap for each linked list
		//   element and store the pointer.
		//
		HeapArray[0].Heap = & DebugHeap;
		HeapArray[1].Heap = & FastHeap;
		HeapArray[2].Heap = & PageHeap;

		//
		//   Insert each linked list element into
		//   the list of heaps.
		//
		HeapArray[0].Insert( AllHeaps );
		HeapArray[1].Insert( AllHeaps );
		HeapArray[2].Insert( AllHeaps );
...
bool DYNAMIC_DEBUG_HEAP::MultipleDelete
	...
	//   HeapArray.  Although this is not as fast it
	...
	for ( Count=0;Count < Actual;Count ++ )
		...
		//
		if ( ! Delete( HeapArray[ Count ],Size ) )
			{ Result = false; }
		}
...
DYNAMIC_DEBUG_HEAP::~DYNAMIC_DEBUG_HEAP( void )
	...
	if ( Active )
		...
		//   the list of heaps.
		//
		HeapArray[2].Delete( AllHeaps );
		HeapArray[1].Delete( AllHeaps );
		HeapArray[0].Delete( AllHeaps );

		//
		//   Delete each linked list element.
		//
		PLACEMENT_DELETE( & HeapArray[2],DYNAMIC_HEAP );
		PLACEMENT_DELETE( & HeapArray[1],DYNAMIC_HEAP );
		PLACEMENT_DELETE( & HeapArray[0],DYNAMIC_HEAP );
