...
DEBUG_HEAP_TEST::DEBUG_HEAP_TEST( VOID )
	{
	Active = False;
	MaxThreads = 0;
	ZeroAll = False;
...
BOOLEAN DEBUG_HEAP_TEST::CreateHeaps
		( 
	...
	if ( (! Active) && (NumberOfHeaps > 0) && (NumberOfThreads > 0) )
		...
		//
		Heaps = ((DEBUG_HEAP*) malloc( (NumberOfHeaps * sizeof(DEBUG_HEAP)) ));

		Threads = ((DEBUG_HEAP**) malloc( (NumberOfThreads * sizeof(DEBUG_HEAP*)) ));
		...
		if ( (Heaps != NULL) && (Threads != NULL) )
			...
			for ( Count=0;Count < NumberOfHeaps;Count ++ )
				{
				PLACEMENT_NEW( & Heaps[ Count ],DEBUG_HEAP )
					(
					0,
					False,
					False,
...
CHAR *DEBUG_HEAP_TEST::NameOfHeap( VOID )
	{ return ((CHAR*) "Rockall 'DEBUG_HEAP' (Standard heap)" ); }
...
VOID DEBUG_HEAP_TEST::DeleteHeaps( VOID )
	{
	//
	...
	if ( Active )
		...
		for ( Count=0;Count < MaxHeaps;Count ++ )
			{ PLACEMENT_DELETE( & Heaps[ Count ],DEBUG_HEAP ); }
