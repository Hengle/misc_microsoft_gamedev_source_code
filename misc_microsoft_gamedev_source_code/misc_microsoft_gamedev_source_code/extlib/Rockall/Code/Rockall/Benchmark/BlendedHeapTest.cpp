...
BLENDED_HEAP_TEST::BLENDED_HEAP_TEST( VOID )
	{
	Active = False;
	MaxThreads = 0;
	ZeroAll = False;
...
BOOLEAN BLENDED_HEAP_TEST::CreateHeaps
		( 
	...
	if ( (! Active) && (NumberOfHeaps > 0) && (NumberOfThreads > 0) )
		...
		//
		Heaps = ((BLENDED_HEAP*) malloc( (NumberOfHeaps * sizeof(BLENDED_HEAP)) ));

		Threads = ((BLENDED_HEAP**) malloc( (NumberOfThreads * sizeof(BLENDED_HEAP*)) ));
		...
		if ( (Heaps != NULL) && (Threads != NULL) )
			...
			for ( Count=0;Count < NumberOfHeaps;Count ++ )
				{
				PLACEMENT_NEW( & Heaps[ Count ],BLENDED_HEAP )
					(
					4194304,
					False,
					False,
...
CHAR *BLENDED_HEAP_TEST::NameOfHeap( VOID )
	{ return ((CHAR*) "Rockall 'BLENDED_HEAP' (Standard heap)" ); }
...
VOID BLENDED_HEAP_TEST::DeleteHeaps( VOID )
	{
	//
	...
	if ( Active )
		...
		for ( Count=0;Count < MaxHeaps;Count ++ )
			{ PLACEMENT_DELETE( & Heaps[ Count ],BLENDED_HEAP ); }
