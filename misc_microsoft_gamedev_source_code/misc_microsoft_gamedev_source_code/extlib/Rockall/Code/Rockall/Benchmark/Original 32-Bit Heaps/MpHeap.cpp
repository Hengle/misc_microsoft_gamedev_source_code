    ...
    )
{
    ...
    for (i=0; i < MpHeap->HeapCount; i++) {
        Entry = &MpHeap->Entry[i];
        EnterCriticalSection(&Entry->Lock);
        FreeSize = ((DWORD) HeapCompact(Entry->Heap, 0));
        LeaveCriticalSection(&Entry->Lock);
    ...
    )
{
    ...
    HeapHint = ((DWORD) (HeapEntry - &MpHeap->Entry[0]));
    ...
    do {
        ...
        Header->Next = Next;
    } while ( InterlockedCompareExchangePointer((void**) &HeapEntry->DelayedFreeList,
                                         &Header->Next,
                                         Next) != Next);
    return(TRUE);
}
    ...
    )
{
    ...
    //
    FreeList = (PSINGLE_LIST_ENTRY)InterlockedExchangePointer((LPLONG)&HeapEntry->DelayedFreeList,NULL);
    while (FreeList != NULL) {
        Next = FreeList->Next;
