...
FAST_HEAP_TEST::FAST_HEAP_TEST( VOID )
	{
	Active = False;
	MaxThreads = 0;
	ZeroAll = False;
...
BOOLEAN FAST_HEAP_TEST::CreateHeaps
		( 
	...
	if ( (! Active) && (NumberOfHeaps > 0) && (NumberOfThreads > 0) )
		...
		//
		Heaps = ((FAST_HEAP*) malloc( (NumberOfHeaps * sizeof(FAST_HEAP)) ));
...
CHAR *FAST_HEAP_TEST::NameOfHeap( VOID )
	{ return ((CHAR*) "Rockall 'FAST_HEAP' (Standard heap)" ); }
...
VOID FAST_HEAP_TEST::DeleteHeaps( VOID )
	{
	//
