...
SMALL_HEAP_TEST::SMALL_HEAP_TEST( VOID )
	{
	Active = False;
	MaxThreads = 0;
	ZeroAll = False;
...
BOOLEAN SMALL_HEAP_TEST::CreateHeaps
		( 
	...
	if ( (! Active) && (NumberOfHeaps > 0) && (NumberOfThreads > 0) )
		...
		//
		Heaps = ((SMALL_HEAP*) malloc( (NumberOfHeaps * sizeof(SMALL_HEAP)) ));

		Threads = ((SMALL_HEAP**) malloc( (NumberOfThreads * sizeof(SMALL_HEAP*)) ));
		...
		if ( (Heaps != NULL) && (Threads != NULL) )
			...
			for ( Count=0;Count < NumberOfHeaps;Count ++ )
				{
				PLACEMENT_NEW( & Heaps[ Count ],SMALL_HEAP )
					(
					4194304,
					True,
					False,
...
CHAR *SMALL_HEAP_TEST::NameOfHeap( VOID )
	{ return ((CHAR*) "Rockall 'SMALL_HEAP' (Standard heap)" ); }
...
VOID SMALL_HEAP_TEST::DeleteHeaps( VOID )
	{
	//
	...
	if ( Active )
		...
		for ( Count=0;Count < MaxHeaps;Count ++ )
			{ PLACEMENT_DELETE( & Heaps[ Count ],SMALL_HEAP ); }
