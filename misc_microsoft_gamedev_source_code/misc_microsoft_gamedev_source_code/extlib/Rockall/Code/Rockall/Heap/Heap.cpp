...
GLOBALLOCK*		HEAP::_Globallock	  = NULL;
BYTE			HEAP::_LockMem[sizeof(*_Globallock)];
VOLATILE SBIT32 HEAP::_HeapCount	  = 0;
#endif
    /********************************************************************/
    /*                                                                  */
    /*   Class constructor.                                             */
...
HEAP::HEAP
	...
	if ( (Size1 >= 1) && (Size2 >= 3) )
		...
		//		Placing the global lock into the static memory reserved by lockMem
		if (AtomicIncrement(&_HeapCount) == 1)
		{
			_Globallock = PLACEMENT_NEW(_LockMem,GLOBALLOCK);
		}
...
VOID HEAP::PrintHeapStatistics( VOID )
	...
	//
	if (!_Globallock)
		Failure("Unable to print heap statistics due lack of lock!",true);
	_Globallock->ClaimLock();
...
HEAP::~HEAP( VOID )
	...
	if ( Statistics != NULL ) 
		...
		BYTE temp[sizeof(_LockMem)];
		MEMCPY(temp, _LockMem, sizeof(_LockMem));
		if (!AtomicDecrement(&_HeapCount))
		{
			PLACEMENT_DELETE((GLOBALLOCK*)temp,GLOBALLOCK);
		}
#endif
