...
template <class LOCK=PARTIAL_LOCK> class UNIQUE : public HASH<SNATIVE,POINTER>
    ...
    public:
		...
		DETAIL *CreateString( CONST CHAR *String,SBIT32 Size );

		SBIT32 CompareStrings( CONST DETAIL *Detail1, CONST DETAIL *Detail2 );
		...
		INLINE CONST CHAR *Value( DETAIL *Detail )
			{ return Detail -> Text; }
...
template <class LOCK> DETAIL *UNIQUE<LOCK>::CreateString
	...
	//
	Detail1 -> Size = Size;
	Detail1 -> Text = const_cast<char*>(String); //Temporarily remove const protection
	Detail1 -> Uses = 1;
