...
public:
   ...
   ~SPINLOCK( VOID ) { DeleteCriticalSection(&mCriticalSection); }
         ...
         if (TryEnterCriticalSection(&mCriticalSection) == 0)
         {
            ...
            else
            {
               ...
               for ( ; ; )
               {
                  ...
                  SBIT32 totalTime = GetTickCount() - startTime;
                  if (totalTime >= Sleep)
                     return False;
   ...
   INLINE VOID ReleaseLock( VOID ) { LeaveCriticalSection(&mCriticalSection); }
