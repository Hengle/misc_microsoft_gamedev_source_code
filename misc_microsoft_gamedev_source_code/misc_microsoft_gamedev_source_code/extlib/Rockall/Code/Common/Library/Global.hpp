...
// On Xenon we have to change logic on endianess
#if defined(_XENON)
#define BIG_ENDIAN					1
#define POWERPC						1
#define ZEROMEMORY(dest,size)		XMemSet(dest,0,size)
#define MEMCPY(dest,src, size)		XMemCpy(dest,src,size)
#else
#define ZEROMEMORY(dest,size)		ZeroMemory(dest,size)
...
VOID Failure(CONST char *Message,BOOLEAN Report = True );
#endif
