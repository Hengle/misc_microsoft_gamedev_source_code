...
// rg [8/25/06] - HACK to see if our deadlock problems are caused by a buggy SHARELOCK class.
...
public:
   ...
   INLINE BOOLEAN ChangeSharedLockToExclusiveLock( SBIT32 Sleep = INFINITE ) { return True; }

   INLINE BOOLEAN ClaimExclusiveLock( SBIT32 Sleep = INFINITE ) { return mSpinLock.ClaimLock(Sleep); }

   INLINE BOOLEAN ClaimShareLock( SBIT32 Sleep = INFINITE ) { return mSpinLock.ClaimLock(Sleep); }

   INLINE VOID ReleaseExclusiveLock( VOID ) { mSpinLock.ReleaseLock(); }

   INLINE VOID ReleaseShareLock( VOID ) { mSpinLock.ReleaseLock(); }
};
