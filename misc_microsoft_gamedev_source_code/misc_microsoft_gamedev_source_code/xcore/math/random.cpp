...
uint Random::uRandFast(void) 
{
   return (RND_FIB ^ RND_SHR3) + RND_SWB;
}
...
float Random::fRand(float l, float h)
{
   ...
   {
      const uint r = uRand();
      f = static_cast<float>(l + (h - l) * (r / 4294967296.0));
      // Probably not necessary, but I'm paranoid about roundoff error and I want to guarantee the output will always be in range.
   } while ((f < l) || (f >= h));
   return f;
}
...
int Random::iRand(int l, int h)
{
   ...
   do
   {
      // Must be VERY careful how this function is implemented!
      r = l + static_cast<int>((h - l) * dRand(0.0f, 1.0f));
