...
// (V0, V1, V2).
//-----------------------------------------------------------------------------
static inline BOOL PointOnPlaneInsideTriangle( XMVECTOR P, XMVECTOR V0, XMVECTOR V1, XMVECTOR V2 )
{
    ...
    BOOL Inside0 = XMVector4GreaterOrEqual( XMVector3Dot( C0, N ), g_VectorZero );
    BOOL Inside1 = XMVector4GreaterOrEqual( XMVector3Dot( C1, N ), g_VectorZero );
    BOOL Inside2 = XMVector4GreaterOrEqual( XMVector3Dot( C2, N ), g_VectorZero );
...
VOID ComputeBoundingOrientedBoxFromPoints( OrientedBox* pOut, UINT Count, const XMFLOAT3* pPoints, UINT Stride )
{
    ...
    if ( XMVector4Less( Det, g_VectorZero ) )
    {
...
BOOL IntersectPointFrustum( XMVECTOR Point, const Frustum* pVolume )
{
    ...
    for (INT i = 0; i < 6; i++)
    {
        ...
        bOutside |= XMVector4Greater( Dot, g_VectorZero );
    }
...
BOOL IntersectRaySphere( XMVECTOR Origin, XMVECTOR Direction, const Sphere* pVolume, FLOAT* pDist )
{
    ...
    if ( XMVector4Less( s, g_VectorZero ) && XMVector4Greater( l2, r2 ))
        return FALSE;
...
BOOL IntersectRayAxisAlignedBox( XMVECTOR Origin, XMVECTOR Direction, const AxisAlignedBox* pVolume, FLOAT* pDist )
{
    ...
    if ( XMVector4Greater( XMVectorSplatX( t_min ), XMVectorSplatX( t_max ) ) )
        return FALSE;

    // if ( t_max < 0.0f ) return FALSE;
    if ( XMVector4Less( XMVectorSplatX( t_max ), g_VectorZero ) )
        return FALSE;
    
    // if (IsParallel && (-Extents > AxisDotOrigin || Extents < AxisDotOrigin)) return FALSE;
    XMVECTOR ParallelOverlap = XMVectorInBounds( AxisDotOrigin, Extents );
    if ( XMVector3AnyTrue( XMVectorAndCInt( IsParallel, ParallelOverlap ) ) )
        return FALSE;
...
BOOL IntersectTriangleTriangle( XMVECTOR A0, XMVECTOR A1, XMVECTOR A2, XMVECTOR B0, XMVECTOR B1, XMVECTOR B2 )
{
    ...
    BDist = XMVectorSelect( BDist, g_VectorZero, BDistIsZero );
    
    static UINT BDistIsLessCR;
    XMVECTOR BDistIsLess = XMVectorGreaterR( &BDistIsLessCR, g_VectorZero, BDist );

    static UINT BDistIsGreaterCR;
    XMVECTOR BDistIsGreater = XMVectorGreaterR( &BDistIsGreaterCR, BDist, g_VectorZero );
    ...
    ADist = XMVectorSelect( ADist, g_VectorZero, ADistIsZero );

    static UINT ADistIsLessCR;
    XMVECTOR ADistIsLess = XMVectorGreaterR( &ADistIsLessCR, g_VectorZero, ADist );

    static UINT ADistIsGreaterCR;
    XMVECTOR ADistIsGreater = XMVectorGreaterR( &ADistIsGreaterCR, ADist, g_VectorZero );
    ...
    if ( XMVector4Greater( Dist0, g_VectorZero ) )
        return FALSE;
    
    XMVECTOR Dist1 = XMVector3Dot( Delta1, XMVector3Cross( (BB1 - BB0), (AA1 - AA0) ) );
    if ( XMVector4Greater( Dist1, g_VectorZero ) )
        return FALSE;
...
BOOL IntersectTriangleSphere( XMVECTOR V0, XMVECTOR V1, XMVECTOR V2, const Sphere* pVolume )
{
    ...
    if ( XMVector4Less( Dist, -Radius ) || XMVector4Greater( Dist, Radius ) )
        return FALSE;

    // Project the center of the sphere onto the plane of the triangle.
    XMVECTOR Point = Center - (N * Dist);
    
    // Is it inside all the edges? If so we must intersect because the distance 
    // to the plane is less than the radius.
    if ( PointOnPlaneInsideTriangle( Point, V0, V1, V2 ) )
        return TRUE;
    ...
    if ( XMVector4LessOrEqual( XMVector3LengthSq( Center - Point ), RadiusSq ) )
        return 1;
...
BOOL IntersectTriangleAxisAlignedBox( XMVECTOR V0, XMVECTOR V1, XMVECTOR V2, const AxisAlignedBox* pVolume )
{
    ...
    XMVECTOR NormalSelect = XMVectorGreater( Normal, g_VectorZero );
    ...
    if ( XMVector4Greater( MinDist, Dist ) || XMVector4Less( MaxDist, Dist ) )
        return FALSE;
    ...
    // Make w zero.
    e0 = XMVectorInsert( e0, g_VectorZero, 0, 0, 0, 0, 1);
    e1 = XMVectorInsert( e1, g_VectorZero, 0, 0, 0, 0, 1);
    e2 = XMVectorInsert( e2, g_VectorZero, 0, 0, 0, 0, 1);
    ...
    if ( XMVector4Greater( Min, Radius ) || XMVector4Less( Max, -Radius ) )
        return FALSE;
...
BOOL IntersectOrientedBoxOrientedBox( const OrientedBox* pVolumeA, const OrientedBox* pVolumeB )
{
    ...
    if ( XMVector3Greater( XMVectorAbs(d), XMVectorAdd( d_A, d_B ) ) )
        return FALSE;
...
INT IntersectTriangleFrustum( XMVECTOR V0, XMVECTOR V1, XMVECTOR V2, const Frustum* pVolume )
{
    ...
    for ( INT i = 0; i < 6; i++ )
    {
        ...
        if ( XMVector4Greater( MinDist, PlaneDist ) )
        {
            // Outside the plane.
            return 0;
        }
        else if ( XMVector4GreaterOrEqual( MaxDist, PlaneDist ) )
        {
    ...
    for ( INT i = 0; i < 3; i++ )
    {
        for ( INT j = 0; j < 6; j++ )
        {
            ...
            if ( XMVector4Greater( MinA, MaxB ) || XMVector4Greater( MinB, MaxA ) )
                return 0;
        }
    }
...
INT IntersectSphereFrustum( const Sphere* pVolumeA, const Frustum* pVolumeB )
{
    ...
    for ( INT i = 0; i < 6; i++ )
    {
        ...
        if ( XMVector4Greater( Dist[i], Radius ) )
        {
            // Outside the plane.
            return 0;
        }
        else if ( XMVector4Greater( Dist[i], -Radius ) )
        {
            // Intersecting the plane.
            InsideAll = FALSE;
        
            // Check if the center is outside the plane.
            if ( XMVector4Greater( Dist[i], g_VectorZero ) )
    ...
    for ( INT i = 0; i < 6; i++ )
    {
        if ( XMVector4Greater( Dist[i], g_VectorZero ) )
        {
            ...
            for ( INT j = 0; j < 4; j++ )
            {
                ...
                if ( XMVector4Greater( XMVector4Dot( Point, Planes[plane_index] ), g_VectorZero ) )
                    InsideFace = FALSE;
            }
...
INT IntersectOrientedBoxFrustum( const OrientedBox* pVolumeA, const Frustum* pVolumeB )
{
    ...
    for ( INT i = 0; i < 6; i++ )
    {
        ...
        if ( XMVector4Greater( Dist, Radius ) )
        {
            // Outside the plane.
            return 0;
        }
        else if ( XMVector4GreaterOrEqual( Dist, -Radius ) )
        {
            // Intersecting the plane.
            InsideAll = FALSE;
        
            // Check if the center is outside the plane.
            if ( XMVector4Greater( Dist, g_VectorZero ) )
                CenterInsideAll = FALSE;
        }
    }
    ...
    for ( INT i = 0; i < 3; i++ )
    {
        for ( INT j = 0; j < 6; j++ )
        {
            ...
            if ( XMVector4Greater( Dist, FrustumMax + Radius ) || XMVector4Less( Dist, FrustumMin - Radius ) )
                return 0;
        }
    }
...
INT IntersectFrustumFrustum( const Frustum* pVolumeA, const Frustum* pVolumeB )
{
    ...
    for ( INT i = 0; i < 6; i++ )
    {
        ...
        if ( XMVector4Greater( Min, PlaneDistB[i] ) )
        {
            // Outside the plane.
            return 0;
        }
        else if ( XMVector4GreaterOrEqual( Max, PlaneDistB[i] ) )
        {
            // Intersecting the plane.
            InsideAll = FALSE;
        }
    ...
    for ( INT i = 0; i < 6; i++ )
    {
        ...
        if ( XMVector4Greater( Min, PlaneDistA[i] ) )
        {
            // Outside the plane.
            return 0;
        }
...
//-----------------------------------------------------------------------------
inline INT FastIntersectTrianglePlane( XMVECTOR V0, XMVECTOR V1, XMVECTOR V2, XMVECTOR Plane )
{
    // Plane0
    ...
    if ( XMVector4Greater( MinDist, g_VectorZero ) )
    {
        // Outside the plane.
        return 0;
    }
    else if ( XMVector4GreaterOrEqual( MaxDist, g_VectorZero ) )
    {
        // Intersecting the plane.
        return 1;
    }
...
INT IntersectTriangle6Planes( XMVECTOR V0, XMVECTOR V1, XMVECTOR V2, XMVECTOR Plane0, XMVECTOR Plane1, XMVECTOR Plane2, XMVECTOR Plane3, XMVECTOR Plane4, XMVECTOR Plane5 )
{
    ...
    Code = FastIntersectTrianglePlane( V0, V1, V2, Plane0 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectTrianglePlane( V0, V1, V2, Plane1 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectTrianglePlane( V0, V1, V2, Plane2 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectTrianglePlane( V0, V1, V2, Plane3 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectTrianglePlane( V0, V1, V2, Plane4 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectTrianglePlane( V0, V1, V2, Plane5 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;
    
    // If the triangle is not intersecting any planes it is fully inside.
    if ( OrCode == 2 )
        return 2;
...
//-----------------------------------------------------------------------------
inline INT FastIntersectSpherePlane( XMVECTOR Center, XMVECTOR Radius, XMVECTOR Plane )
{
...
INT IntersectSphere6Planes( const Sphere* pVolume, XMVECTOR Plane0, XMVECTOR Plane1, XMVECTOR Plane2, XMVECTOR Plane3, XMVECTOR Plane4, XMVECTOR Plane5 )
{
    ...
    // Test against each plane.
    Code = FastIntersectSpherePlane( Center, Radius, Plane0 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectSpherePlane( Center, Radius, Plane1 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectSpherePlane( Center, Radius, Plane2 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectSpherePlane( Center, Radius, Plane3 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectSpherePlane( Center, Radius, Plane4 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectSpherePlane( Center, Radius, Plane5 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;
    
    // If the sphere is not intersecting any planes it is fully inside.
    if ( OrCode == 2 )
        return 2;
...
//-----------------------------------------------------------------------------
inline INT FastIntersectAxisAlignedBoxPlane( XMVECTOR Center, XMVECTOR Extents, XMVECTOR Plane )
{
    // Compute the distance to the center of the box.
    XMVECTOR Dist = XMVector4Dot( Center, Plane );
...
INT IntersectAxisAlignedBox6Planes( const AxisAlignedBox* pVolume, XMVECTOR Plane0, XMVECTOR Plane1, XMVECTOR Plane2, XMVECTOR Plane3, XMVECTOR Plane4, XMVECTOR Plane5 )
{
    ...
    // Test against each plane.
    Code = FastIntersectAxisAlignedBoxPlane( Center, Extents, Plane0 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectAxisAlignedBoxPlane( Center, Extents, Plane1 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectAxisAlignedBoxPlane( Center, Extents, Plane2 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectAxisAlignedBoxPlane( Center, Extents, Plane3 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectAxisAlignedBoxPlane( Center, Extents, Plane4 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectAxisAlignedBoxPlane( Center, Extents, Plane5 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;
    
    // If the box is not intersecting any planes it is fully inside.
    if ( OrCode == 2 )
        return 2;
...
//-----------------------------------------------------------------------------
inline INT FastIntersectOrientedBoxPlane( XMVECTOR Center, XMVECTOR Extents, XMVECTOR Axis0, XMVECTOR Axis1, XMVECTOR Axis2, XMVECTOR Plane )
{
    // Compute the distance to the center of the box.
    XMVECTOR Dist = XMVector4Dot( Center, Plane );
...
INT IntersectOrientedBox6Planes( const OrientedBox* pVolume, XMVECTOR Plane0, XMVECTOR Plane1, XMVECTOR Plane2, XMVECTOR Plane3, XMVECTOR Plane4, XMVECTOR Plane5 )
{
    ...
    // Test against each plane.
    Code = FastIntersectOrientedBoxPlane( Center, Extents, R.r[0], R.r[1], R.r[2], Plane0 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectOrientedBoxPlane( Center, Extents, R.r[0], R.r[1], R.r[2], Plane1 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectOrientedBoxPlane( Center, Extents, R.r[0], R.r[1], R.r[2], Plane2 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectOrientedBoxPlane( Center, Extents, R.r[0], R.r[1], R.r[2], Plane3 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectOrientedBoxPlane( Center, Extents, R.r[0], R.r[1], R.r[2], Plane4 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectOrientedBoxPlane( Center, Extents, R.r[0], R.r[1], R.r[2], Plane5 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;
...
//-----------------------------------------------------------------------------
inline INT FastIntersectFrustumPlane( XMVECTOR Point0, XMVECTOR Point1, XMVECTOR Point2, XMVECTOR Point3, XMVECTOR Point4, XMVECTOR Point5, XMVECTOR Point6, XMVECTOR Point7, XMVECTOR Plane )
{
    // Find the min/max projection of the frustum onto the plane normal.
    ...
    if ( XMVector4Greater( Min, PlaneDist ) )
    {
        // Outside the plane.
        return 0;
    }
    else if ( XMVector4GreaterOrEqual( Max, PlaneDist ) )
    {
        // Intersecting the plane.
        return 1;
    }
...
INT IntersectFrustum6Planes( const Frustum* pVolume, XMVECTOR Plane0, XMVECTOR Plane1, XMVECTOR Plane2, XMVECTOR Plane3, XMVECTOR Plane4, XMVECTOR Plane5 )
{
    ...
    // Test against each plane.
    Code = FastIntersectFrustumPlane( Corners0, Corners1, Corners2, Corners3, Corners4, Corners5, Corners6, Corners7, Plane0 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectFrustumPlane( Corners0, Corners1, Corners2, Corners3, Corners4, Corners5, Corners6, Corners7, Plane1 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectFrustumPlane( Corners0, Corners1, Corners2, Corners3, Corners4, Corners5, Corners6, Corners7, Plane2 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectFrustumPlane( Corners0, Corners1, Corners2, Corners3, Corners4, Corners5, Corners6, Corners7, Plane3 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectFrustumPlane( Corners0, Corners1, Corners2, Corners3, Corners4, Corners5, Corners6, Corners7, Plane4 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;

    Code = FastIntersectFrustumPlane( Corners0, Corners1, Corners2, Corners3, Corners4, Corners5, Corners6, Corners7, Plane5 );
    if ( Code == 0 ) return 0;
    OrCode |= Code;
    
    // If the frustum is not intersecting any planes it is fully inside.
    if ( OrCode == 2 )
        return 2;
...
INT IntersectTrianglePlane( XMVECTOR V0, XMVECTOR V1, XMVECTOR V2, XMVECTOR Plane )
{
	...
	return FastIntersectTrianglePlane( V0, V1, V2, Plane );
}
...
INT IntersectSpherePlane( const Sphere* pVolume, const XMVECTOR Plane )
{
	...
	return FastIntersectSpherePlane( Center, Radius, Plane );
}
...
INT IntersectAxisAlignedBoxPlane( const AxisAlignedBox* pVolume, XMVECTOR Plane )
{
	...
	return FastIntersectAxisAlignedBoxPlane( Center, Extents, Plane );
}
...
INT IntersectOrientedBoxPlane( const OrientedBox* pVolume, XMVECTOR Plane )
{
	...
	return FastIntersectOrientedBoxPlane( Center, Extents, R.r[0], R.r[1], R.r[2], Plane );
}
...
INT IntersectFrustumPlane( const Frustum* pVolume, XMVECTOR Plane )
{
    ...
    return FastIntersectFrustumPlane( Corners0, Corners1, Corners2, Corners3, Corners4, Corners5, Corners6, Corners7, Plane );
}
