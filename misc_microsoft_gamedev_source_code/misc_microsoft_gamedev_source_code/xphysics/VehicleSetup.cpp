...
//#include <demos/PhysicsApi/VehiclePhysics/VehicleApi/VehicleSetup.h>
//#include <demos/PhysicsApi/VehiclePhysics/VehicleApi/FrictionMapVehicleRaycastWheelCollide.h>
...
void VehicleSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultSuspension& suspension ) 
{
   ...
   const float springLength = 0.35f;
	suspension.m_wheelParams[0].m_length = springLength;
	suspension.m_wheelParams[1].m_length = springLength;
	suspension.m_wheelParams[2].m_length = springLength;
	suspension.m_wheelParams[3].m_length = springLength;
	...
	//
	{
      ...
      /*
		suspension.m_wheelParams[0].m_hardpointCs.set ( hardPointFrontX, hardPointY, -hardPointZ); 
		suspension.m_wheelParams[1].m_hardpointCs.set ( hardPointFrontX, hardPointY,  hardPointZ); 
		suspension.m_wheelParams[2].m_hardpointCs.set ( hardPointBackX, hardPointY, -hardPointZ); 
		suspension.m_wheelParams[3].m_hardpointCs.set ( hardPointBackX, hardPointY,  hardPointZ); 
      */

  		suspension.m_wheelParams[0].m_hardpointCs.set ( -hardPointZ, hardPointY, hardPointFrontX); 
		suspension.m_wheelParams[1].m_hardpointCs.set ( hardPointZ, hardPointY, hardPointFrontX); 
		suspension.m_wheelParams[2].m_hardpointCs.set ( -hardPointZ, hardPointY,  hardPointBackX); 
		suspension.m_wheelParams[3].m_hardpointCs.set ( hardPointZ, hardPointY,  hardPointBackX); 
   }

	const hkVector4 downDirection( 0.0f, -1.0f, 0.0f );
	suspension.m_wheelParams[0].m_directionCs = downDirection;
	suspension.m_wheelParams[1].m_directionCs = downDirection;
	suspension.m_wheelParams[2].m_directionCs = downDirection;
	suspension.m_wheelParams[3].m_directionCs = downDirection;
}
...
#else
	aerodynamics.m_airDensity = 0.0f;
	aerodynamics.m_frontalArea = 0.0f;	
	aerodynamics.m_dragCoefficient = 0.0f;
	aerodynamics.m_liftCoefficient = 0.0f;
	aerodynamics.m_extraGravityws.set( 0.0f, 0.0f, 0.0f); 
#endif
}
...
void VehicleSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultVelocityDamper& velocityDamper )
{
	...
	velocityDamper.m_collisionSpinDamping = 0.0f; 
	velocityDamper.m_collisionThreshold   = 0.0f; 
#endif
}
...
/*
* Havok SDK - DEMO RELEASE, BUILD(#20070406)
*
