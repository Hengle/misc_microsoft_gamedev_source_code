...
#ifdef SUPPORT_MMINTRIN
#include "mmintrin.h"
#endif
//------------------------------------------------------------------------------
#define FIX(x, b) ((long) ((x) * (1L<<(b)) + 0.5f))
//------------------------------------------------------------------------------
...
void jpeg_decoder::H2V2Convert(void)
{
  ...
  if (use_mmx)
  {
    ...
    // Mind-bending MMX intrinsics follow...
		// 4/10/03: This is far from the best implementation..
    for (int i = max_mcus_per_row; i > 0; i--)
    {
      for (int l = 0; l < 2; l++)
      {
        ...
        cb = _m_psllw(_m_punpcklbw(cb, zero), two);

        cr = m3;
        cr = _m_punpcklbw(cr, cr);
        cr = _m_psllw(_m_punpcklbw(cr, zero), two);
        ...
        y1 = _m_punpcklbw(_m_psrlq(yy, thirty_two), zero);
