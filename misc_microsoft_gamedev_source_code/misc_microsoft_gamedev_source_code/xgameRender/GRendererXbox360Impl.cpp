...
static const DWORD *VertexShaderTextTable[VS_Count - 1] =
{
    g_xvs_VShaderStrip,
    g_xvs_VShaderGlyph,
...
bool gEnableScaleformDraws = true;
...
public:
    ...
    bool    CreateVertexShader(GPtr<IDirect3DVertexShader9> *pshaderPtr, const DWORD* pshaderText)
    {
        if (!*pshaderPtr)
        {
            HRESULT hr = pDevice->CreateVertexShader( pshaderText, &pshaderPtr->GetRawRef() );
    ...
    bool    CreatePixelShader(GPtr<IDirect3DPixelShader9> *pshaderPtr, const DWORD* pshaderText)
    {
        if (!*pshaderPtr)
        {
            HRESULT hr = pDevice->CreatePixelShader( pshaderText, &pshaderPtr->GetRawRef() );
...
#ifndef BUILD_FINAL
                gFlashManager.mWorkerStats.mPSDrawCalls[PS_SolidColor]++;
#endif 
            }
...
#ifndef BUILD_FINAL
                    gFlashManager.mWorkerStats.mPSDrawCalls[PS_None]++;
#endif 
                }
                else
                {
...
#ifndef BUILD_FINAL
                        gFlashManager.mWorkerStats.mPSDrawCalls[PS_CxformTextureMultiply]++;
#endif 
                    }
                    else
                    {
...
#ifndef BUILD_FINAL
                        gFlashManager.mWorkerStats.mPSDrawCalls[PS_CxformTexture]++;
#endif 
                    }
    ...
    {
        SCOPEDSAMPLEID(GRendererXbox360Impl_BeginDisplay, 0xFFFF0000);
        if (!ModeSet)
        {
    ...
    {
        SCOPEDSAMPLEID(GRendererXBox360Impl_DrawIndexedTriList, 0xFFFF0000);
        if (!ModeSet)
            return;
    ...
    {
        SCOPEDSAMPLEID(GRendererXBox360Impl_DrawBitmaps, 0xFFFF0000);
...
void GTextureXbox360Impl::Update(int level, int n, const UpdateRect *rects, const GImageBase *pim)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        pD3DTexture->LockRect(level, &lr, &destr, lockFlags);
...
// Factory.
GRendererXbox360*   GRendererXbox360::CreateReferenceRenderer()
{
